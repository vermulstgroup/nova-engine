...
metadata: { "nova-engine": { "emoji": "🫧", "requires": { "config": ["channels.bluebubbles"] } } }
---
...
BlueBubbles is Nova Engine’s recommended iMessage integration. Use the `message` tool with `channel: "bluebubbles"` to send messages and manage iMessage conversations: send texts and attachments, react (tapbacks), edit/unsend, reply in threads, and manage group participants/names/icons.
...
```json
{
  ...
  "channel": "bluebubbles",
  "target": "+15551234567",
  "message": "hello from Nova Engine"
}
```
