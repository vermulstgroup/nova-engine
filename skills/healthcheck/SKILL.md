---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Nova Engine deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Nova Engine cron scheduling for periodic checks, or version status checks on a machine running Nova Engine (laptop, workstation, Pi, VPS).
---

# Nova Engine Host Hardening

## Overview

Assess and harden the host running Nova Engine, then align it to a user-defined risk tolerance without breaking access. Use Nova Engine security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Nova Engine changes the host firewall, SSH, or OS updates; it does not.
...
5. Nova Engine gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Nova Engine can access sensitive data.
...
### 2) Run Nova Engine security audits (read-only)

As part of the default read-only checks, run `nova-engine security audit --deep`. Only offer alternatives if the user requests them:

1. `nova-engine security audit` (faster, non-probing)
2. `nova-engine security audit --json` (structured output)

Offer to apply Nova Engine safe defaults (numbered):

1. `nova-engine security audit --fix`

Be explicit that `--fix` only tightens Nova Engine defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Nova Engine version/update status (read-only)

As part of the default read-only checks, run `nova-engine update status`.
...
- Credential hygiene notes (location of Nova Engine creds, prefer disk encryption)
...
- Nova Engine security audit (re-run)
...
After Nova Engine install or first hardening pass, run at least one baseline audit and version check:

- `nova-engine security audit`
- `nova-engine security audit --deep`
- `nova-engine update status`

Ongoing monitoring is recommended. Use the Nova Engine cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `nova-engine cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `nova-engine update status`
...
Before creating, `nova-engine cron list` and match on exact `name`. If found, `nova-engine cron edit <id> ...`.
If not found, `nova-engine cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `nova-engine update status` (preferred for source checkouts and channels)
2. `npm view nova-engine version` (published npm version)

## Nova Engine command accuracy

Use only supported commands and flags:

- `nova-engine security audit [--deep] [--fix] [--json]`
- `nova-engine status` / `nova-engine status --deep`
- `nova-engine health --json`
- `nova-engine update status`
- `nova-engine cron add|list|runs|run`

Do not invent CLI flags or imply Nova Engine enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Nova Engine compaction:
