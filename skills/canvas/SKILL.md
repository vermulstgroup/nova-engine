...
Display HTML content on connected Nova Engine nodes (Mac app, iOS, Android).
...
```
http://<tailscale-hostname>:18793/__nova-engine__/canvas/<file>.html
```
...
In `~/.nova-engine/nova-engine.json`:
...
```bash
cat ~/.nova-engine/nova-engine.json | jq '.gateway.bind'
```

Then construct the URL:

- **loopback**: `http://127.0.0.1:18793/__nova-engine__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:18793/__nova-engine__/canvas/<file>.html`
...
```
canvas action:present node:mac-63599bc4-b54d-4392-9048-b97abd58343a target:http://peters-mac-studio-1.sheep-coho.ts.net:18793/__nova-engine__/canvas/snake.html
```
...
1. Check server bind: `cat ~/.nova-engine/nova-engine.json | jq '.gateway.bind'`
2. Check what port canvas is on: `lsof -i :18793`
3. Test URL directly: `curl http://<hostname>:18793/__nova-engine__/canvas/<file>.html`
...
Node is offline. Use `nova-engine nodes list` to find online nodes.
...
The canvas host serves from `/__nova-engine__/canvas/` prefix:

```
http://<host>:18793/__nova-engine__/canvas/index.html  → ~/clawd/canvas/index.html
http://<host>:18793/__nova-engine__/canvas/games/snake.html → ~/clawd/canvas/games/snake.html
```

The `/__nova-engine__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
