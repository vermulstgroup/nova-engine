...
// NOTE: This extension is intended to be bundled with Nova Engine.
...
export function createLlmTaskTool(api: Nova EnginePluginApi) {
  return {
    ...
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via nova-engine.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-llm-task-"));
