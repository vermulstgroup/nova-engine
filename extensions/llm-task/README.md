...
`nova-engine.invoke --each`) without adding new Nova Engine code per workflow.
...
This extension depends on Nova Engine internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Nova Engine extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.nova-engine/extensions` as a standalone plugin directory.
