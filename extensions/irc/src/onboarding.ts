  ...
  configure: async ({
    ...
    if (useEnv) {
      ...
      const username = String(
        await prompter.text({
          ...
          initialValue: resolved.config.username || nick || "nova-engine",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
      ...
      const realname = String(
        await prompter.text({
          ...
          initialValue: resolved.config.realname || "Nova Engine",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
    ...
    await prompter.note(
      [
        ...
        "Command: nova-engine channels status --probe",
        `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
      ].join("\n"),
      "IRC next steps",
    );
