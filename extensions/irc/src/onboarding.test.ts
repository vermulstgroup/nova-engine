import type { RuntimeEnv, WizardPrompter } from "nova-engine/plugin-sdk";
...
describe("irc onboarding", () => {
  it("configures host and nick via onboarding prompts", async () => {
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("nova-engine-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#nova-engine", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#nova-engine", "#ops"]);
  });
