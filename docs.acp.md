# Nova Engine ACP Bridge

This document describes how the Nova Engine ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`nova-engine acp` exposes an ACP agent over stdio and forwards prompts to a running
Nova Engine Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Nova Engine Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `nova-engine acp` over stdio.

Example config:

```bash
nova-engine config set gateway.remote.url wss://gateway-host:18789
nova-engine config set gateway.remote.token <token>
```

Example run:

```bash
nova-engine acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
nova-engine acp --session agent:main:main
nova-engine acp --session agent:design:main
nova-engine acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Nova Engine ACP” to start a thread.

## Execution Model

- ACP client spawns `nova-engine acp` and speaks ACP messages over stdio.
...
```bash
nova-engine acp --session agent:main:main
nova-engine acp --session-label "support inbox"
nova-engine acp --reset-session
```
...
`nova-engine acp` resolves the Gateway URL and auth from CLI flags or config:
