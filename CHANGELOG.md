...
- Config/State: removed legacy `.moltbot` auto-detection/migration and `moltbot.json` config candidates. If you still have state/config under `~/.moltbot`, move it to `~/.openclaw` (recommended) or set `NOVA_STATE_DIR` / `NOVA_CONFIG_PATH` explicitly.
...
- Env: load global `$NOVA_STATE_DIR/.env` (`~/.openclaw/.env`) as a fallback after CWD `.env`.
