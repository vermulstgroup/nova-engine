...
import Foundation
import Testing
@testable import NovaEngineChatUI
...
private final class TestChatTransport: @unchecked Sendable, NovaEngineChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [NovaEngineChatHistoryPayload]
    private let sessionsResponses: [NovaEngineChatSessionsListResponse]

    private let stream: AsyncStream<NovaEngineChatTransportEvent>
    private let continuation: AsyncStream<NovaEngineChatTransportEvent>.Continuation

    init(
        historyResponses: [NovaEngineChatHistoryPayload],
        sessionsResponses: [NovaEngineChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<NovaEngineChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> NovaEngineChatHistoryPayload {
        ...
        return self.historyResponses.last ?? NovaEngineChatHistoryPayload(
        ...
        attachments _: [NovaEngineChatAttachmentPayload]) async throws -> NovaEngineChatSendResponse
    {
        ...
        return NovaEngineChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> NovaEngineChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? NovaEngineChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: NovaEngineChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = NovaEngineChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = NovaEngineChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { NovaEngineChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = NovaEngineChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = NovaEngineChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = NovaEngineChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { NovaEngineChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
