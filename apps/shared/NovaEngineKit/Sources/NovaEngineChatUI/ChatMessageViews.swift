...
struct ChatMessageBubble: View {
    let message: NovaEngineChatMessage
    let style: NovaEngineChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? NovaEngineChatTheme.userText : NovaEngineChatTheme.assistantText
    ...
    private var inlineAttachments: [NovaEngineChatMessageContent] {
    ...
    private var toolCalls: [NovaEngineChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [NovaEngineChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? NovaEngineChatTheme.userBubble
        }
        if self.style == .onboarding {
            return NovaEngineChatTheme.onboardingAssistantBubble
        }
        return NovaEngineChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return NovaEngineChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: NovaEngineChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? NovaEngineChatTheme.userText : NovaEngineChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: NovaEngineChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(NovaEngineChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(NovaEngineChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [NovaEngineChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: NovaEngineChatTheme.assistantText)
            }
        }
    }
}
