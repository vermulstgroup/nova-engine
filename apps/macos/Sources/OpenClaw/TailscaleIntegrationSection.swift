...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.nova-engine/nova-engine.json. Prefer OPENCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.nova-engine/nova-engine.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.nova-engine/nova-engine.json. Restart the gateway to apply."
        }
