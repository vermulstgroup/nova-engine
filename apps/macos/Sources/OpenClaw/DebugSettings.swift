    ...
    private var launchdSection: some View {
        GroupBox("Gateway startup") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(
                    "When enabled, NovaEngine won't install or manage \(gatewayLaunchdLabel). " +
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("Copy sample URL") {
                        ...
                        let url = "nova-engine://agent?message=\(encoded)&key=\(key)"
                        NSPasteboard.general.clearContents()
                ...
                Text("Deep links (nova-engine://…) are always enabled; the key controls unattended runs.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                ...
                GridRow {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Toggle("Write rolling diagnostics log (JSONL)", isOn: self.$diagnosticsFileLogEnabled)
                            ...
                            .help(
                                "Writes a rotating, local-only log under ~/Library/Logs/NovaEngine/. " +
    ...
    private var pathsSection: some View {
        GroupBox("Paths") {
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("NovaEngine project root")
                        .font(.caption.weight(.semibold))
                    HStack(spacing: 8) {
                        TextField("Path to nova-engine repo", text: self.$gatewayRootInput)
                        ...
                        Button("Reset") {
                            let def = FileManager().homeDirectoryForCurrentUser
                                .appendingPathComponent("Projects/nova-engine").path
                    ...
                    GridRow {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if let sessionStoreSaveError {
                                ...
                                Text("Used by the CLI session loader; stored in ~/.nova-engine/nova-engine.json.")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }
    ...
    private var quickActionsSection: some View {
        GroupBox("Quick actions") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(
                        "Note: macOS may require restarting NovaEngine after enabling Accessibility or Screen Recording.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)

                    Button {
                        LaunchdManager.startNovaEngine()
                    } label: {
                        Label("Restart NovaEngine", systemImage: "arrow.counterclockwise")
                    }
                    .buttonStyle(.bordered)
                    .controlSize(.small)
                }
...
extension DebugSettings {
    static func exerciseForTesting() async {
        ...
        view.gatewayRootInput = "/tmp/nova-engine"
