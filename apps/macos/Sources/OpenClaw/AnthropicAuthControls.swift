...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: NovaEngineOAuthStore.AnthropicOAuthStatus = NovaEngineOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: NovaEngineOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = NovaEngineOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = NovaEngineOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. NovaEngine can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: NovaEngineOAuthStore.AnthropicOAuthStatus,
