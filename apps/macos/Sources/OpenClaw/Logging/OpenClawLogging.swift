...
enum NovaEngineLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = NovaEngineOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = NovaEngineFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        NovaEngineLogging.bootstrapIfNeeded()
        let label = NovaEngineLogging.makeLabel(subsystem: subsystem, category: category)
...
struct NovaEngineOSLogHandler: LogHandler {
...
struct NovaEngineFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = NovaEngineLogging.parseLabel(self.label)
