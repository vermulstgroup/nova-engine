...
final class ScreenRecordService {
    ...
    private let logger = Logger(subsystem: "ai.nova-engine", category: "screenRecord")
        ...
        outPath: String?) async throws -> (path: String, hasAudio: Bool)
    {
        ...
        let outURL: URL = {
            ...
            return FileManager().temporaryDirectory
                .appendingPathComponent("nova-engine-screen-record-\(UUID().uuidString).mp4")
        }()
...
private final class StreamRecorder: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    let queue = DispatchQueue(label: "ai.nova-engine.screenRecord.writer")
