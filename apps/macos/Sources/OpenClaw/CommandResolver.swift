...
enum CommandResolver {
    private static let projectRootDefaultsKey = "nova-engine.gatewayProjectRootPath"
    private static let helperName = "nova-engine"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let nova-engineEntry = root.appendingPathComponent("nova-engine.mjs").path
        if FileManager().isReadableFile(atPath: nova-engineEntry) { return nova-engineEntry }
        let binEntry = root.appendingPathComponent("bin/nova-engine.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/nova-engine")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let nova-enginePaths = self.nova-engineManagedPaths(home: home)
        if !nova-enginePaths.isEmpty {
            extras.insert(contentsOf: nova-enginePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + nova-enginePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func nova-engineManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".nova-engine"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func nova-engineExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectNovaEngineExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("nova-engine.mjs").path,
            root.appendingPathComponent("bin/nova-engine.js").path,
        ]
    ...
    static func hasAnyNovaEngineInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.nova-engineExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func nova-engineNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let nova-enginePath = self.projectNovaEngineExecutable(projectRoot: root) {
                return [nova-enginePath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "nova-engine", subcommand] + extraArgs
            }
            if let nova-enginePath = self.nova-engineExecutable(searchPaths: searchPaths) {
                return [nova-enginePath, subcommand] + extraArgs
            }

            let missingEntry = """
            nova-engine entrypoint missing (looked for dist/index.js or nova-engine.mjs); run pnpm build.
            """
    ...
    static func nova-engineCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real nova-engine CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/nova-engine"
        ...
        \(cliSection)
        \(projectSection)
        if command -v nova-engine >/dev/null 2>&1; then
          CLI="$(command -v nova-engine)"
          nova-engine \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/nova-engine.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/nova-engine.mjs"
            node "$PRJ/nova-engine.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/nova-engine.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/nova-engine.js"
            node "$PRJ/bin/nova-engine.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent nova-engine"
          pnpm --silent nova-engine \(quotedArgs);
        else
          echo "nova-engine CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? NovaEngineConfigFile.loadDict()
