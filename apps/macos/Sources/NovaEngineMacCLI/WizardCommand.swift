...
func runWizardCommand(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        nova-engine-mac wizard

        Usage:
          nova-engine-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
...
actor GatewayWizardClient {
    ...
    private func sendConnect() async throws {
        ...
        let clientId = "nova-engine-macos"
        let clientMode = "ui"
        let role = "operator"
        ...
        let client: [String: ProtoAnyCodable] = [
            ...
            "displayName": ProtoAnyCodable(Host.current().localizedName ?? "NovaEngine macOS Wizard CLI"),
