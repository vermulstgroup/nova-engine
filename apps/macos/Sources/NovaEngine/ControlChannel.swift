...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: NovaEngineProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.nova-engine", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: NovaEngineKit.AnyCodable]()) {
                $0[$1.key] = NovaEngineKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: NovaEngineProtocol.AnyCodable?) -> [String: NovaEngineProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: NovaEngineProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: NovaEngineKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: NovaEngineProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("nova-engine.control.heartbeat")
    static let controlAgentEvent = Notification.Name("nova-engine.control.agent")
}
