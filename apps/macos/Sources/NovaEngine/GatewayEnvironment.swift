...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.nova-engine", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["NOVA_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = NovaEngineConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.nova-engineExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "nova-engine CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["NOVA_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "nova-engine@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "nova-engine@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "nova-engine@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "nova-engine@\(target)"])
            }

        statusHandler("Installing nova-engine@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed nova-engine@\(target)")
        } else {
