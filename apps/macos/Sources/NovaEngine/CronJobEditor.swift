...
struct CronJobEditor: View {
    ...
    static let introText =
        "Create a schedule that wakes NovaEngine via the Gateway. "
    ...
    static let sessionTargetNote =
        "Main jobs post a system event into the current main session. "
            + "Isolated jobs run NovaEngine in a dedicated session and can announce results to a channel."
        ...
        VStack(alignment: .leading, spacing: 10) {
            Grid(alignment: .leadingFirstTextBaseline, horizontalSpacing: 14, verticalSpacing: 10) {
                GridRow {
                    ...
                    TextField("What should NovaEngine do?", text: self.$agentMessage, axis: .vertical)
                        .textFieldStyle(.roundedBorder)
                        .lineLimit(3...7)
                        .frame(maxWidth: .infinity)
                }
