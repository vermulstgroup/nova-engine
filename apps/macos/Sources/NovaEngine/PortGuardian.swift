...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.nova-engine", category: "portguard")
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("NovaEngine", isDirectory: true)
    }()
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "nova-engine", "tsx", "pnpm", "bun"]
    ...
    private func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .remote:
            ...
            return false
        case .local:
            // The gateway daemon may listen as `nova-engine` or as its runtime (`node`, `bun`, etc).
            if full.contains("gateway-daemon") { return true }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("nova-engine"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
