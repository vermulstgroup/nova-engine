...
final class CanvasWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    init(sessionKey: String, root: URL, presentation: CanvasPresentation) throws {
        ...
        (() => {
          try {
            ...
            if (globalThis.__nova-engineA2UIBridgeInstalled) return;
            globalThis.__nova-engineA2UIBridgeInstalled = true;
            ...
            globalThis.addEventListener('a2uiaction', (evt) => {
              try {
                ...
                const handler = globalThis.webkit?.messageHandlers?.nova-engineCanvasA2UIAction;
                ...
                const hasBundledA2UIHost =
                  !!globalThis.nova-engineA2UI ||
                  !!document.querySelector('nova-engine-a2ui-host');
                ...
                location.href = 'nova-engine://agent?' + params.toString();
              } catch {}
            }, true);
          } catch {}
        })();
    ...
    func applyDebugStatusIfNeeded() {
        ...
        (() => {
          try {
            const api = globalThis.__nova-engine;
            if (!api) return;
    ...
    func snapshot(to outPath: String?) async throws -> String {
        ...
        if let outPath, !outPath.isEmpty {
            ...
            path = "/tmp/nova-engine-canvas-\(CanvasWindowController.sanitizeSessionKey(self.sessionKey))-\(ts).png"
        }
