...
final class HealthStore {
    ...
    private static let logger = Logger(subsystem: "ai.nova-engine", category: "health")
    ...
    var summaryLine: String {
        ...
        if link.summary.linked != true {
            if let fallback = self.resolveFallbackChannel(snap, excluding: link.id) {
                ...
                return "\(fallbackLabel) \(fallbackState) · Not linked — run nova-engine login"
            }
            return "Not linked — run nova-engine login"
        }
    ...
    var detailLine: String? {
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                ...
                return "The gateway control port (\(host)) isn’t listening — restart NovaEngine to bring it back."
            }
