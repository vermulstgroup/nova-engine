...
enum DebugActions {
    private static let verboseDefaultsKey = "nova-engine.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "nova-engine.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = NovaEnginePaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "NovaEngine", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = NovaEnginePaths.configURL
        guard
