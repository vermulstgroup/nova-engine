...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.nova-engine.mac`) even if the shipped
    ...
    {
        if environment["NOVA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "nova-engine.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "nova-engine.nixMode") { return true }
