...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.nova-engine", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous NovaEngine run already has an SSH listener on the expected port (common after restarts),
