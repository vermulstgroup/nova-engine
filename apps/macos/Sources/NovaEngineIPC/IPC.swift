...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/NovaEngine/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    return home
        .appendingPathComponent("Library/Application Support/NovaEngine/control.sock")
        .path
}()
