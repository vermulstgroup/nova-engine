...
enum WideAreaGatewayDiscovery {
        ...
        context: DiscoveryContext = .live) -> [WideAreaGatewayBeacon]
    {
        ...
        let probeName = "_nova-engine-gw._tcp.\(domainTrimmed)"
        ...
        for raw in ptrLines {
            ...
            let suffix = "._nova-engine-gw._tcp.\(domainTrimmed)"
    ...
    {
        guard let domain = NovaEngineBonjour.wideAreaGatewayServiceDomain else { return nil }
        let domainTrimmed = domain.trimmingCharacters(in: CharacterSet(charactersIn: "."))
        let probeName = "_nova-engine-gw._tcp.\(domainTrimmed)"
