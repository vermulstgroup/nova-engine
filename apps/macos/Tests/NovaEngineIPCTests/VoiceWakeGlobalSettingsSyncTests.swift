...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = NovaEngineProtocol.AnyCodable(["triggers": ["nova-engine", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["nova-engine", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = NovaEngineProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
