...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: NovaEngineProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: NovaEngineProtocol.AnyCodable] = [
            "presence": .init([NovaEngineProtocol.AnyCodable(entry)]),
        ]
        let payload = NovaEngineProtocol.AnyCodable(payloadMap)
