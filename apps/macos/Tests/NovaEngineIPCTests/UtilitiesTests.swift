...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func gatewayEntrypointPrefersDistOverBin() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/nova-engine.js")
    ...
    @Test func logLocatorPicksNewestLogFile() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/nova-engine", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("nova-engine-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("nova-engine-new-\(UUID().uuidString).log")
