...
struct GatewayDiscoveryModelTests {
    ...
    @Test func localGatewayMatchesDisplayName() {
        ...
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: "Peter's Mac Studio (NovaEngine)",
            serviceName: nil,
            local: local))
    }
    ...
    @Test func serviceNameDoesNotFalsePositiveOnSubstringHostToken() {
        ...
        #expect(!GatewayDiscoveryModel.isLocalGateway(
            ...
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipetacstudio (NovaEngine)",
            local: local))
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipete (NovaEngine)",
            local: local))
    }
    ...
    @Test func parsesGatewayTXTFields() {
        ...
        #expect(parsed.cliPath == "/opt/nova-engine")
    }
