...
final class MotionService: MotionServicing {
    func activities(params: NovaEngineMotionActivityParams) async throws -> NovaEngineMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[NovaEngineMotionActivityEntry], Error>) in
        ...
        return NovaEngineMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: NovaEnginePedometerParams) async throws -> NovaEnginePedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<NovaEnginePedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = NovaEnginePedometerPayload(
