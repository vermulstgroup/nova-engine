...
final class ContactsService: ContactsServicing {
    ...
    func search(params: NovaEngineContactsSearchParams) async throws -> NovaEngineContactsSearchPayload {
        ...
        return NovaEngineContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: NovaEngineContactsAddParams) async throws -> NovaEngineContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return NovaEngineContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return NovaEngineContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> NovaEngineContactPayload {
        NovaEngineContactPayload(
