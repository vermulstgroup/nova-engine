...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<NovaEngineTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = NovaEngineTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> NovaEngineTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return NovaEngineTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> NovaEngineTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> NovaEngineTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return NovaEngineTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> NovaEngineTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = NovaEngineTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: NovaEngineTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: NovaEngineAgentEventPayload.self) else {
                continue
            }
