...
import ai.nova-engine.android.ScreenCaptureRequester
...
class ScreenRecordManager(private val context: Context) {
  ...
  @Volatile private var permissionRequester: ai.nova-engine.android.PermissionRequester? = null

  fun attachScreenCaptureRequester(requester: ScreenCaptureRequester) {
    screenCaptureRequester = requester
  }

  fun attachPermissionRequester(requester: ai.nova-engine.android.PermissionRequester) {
  ...
  suspend fun record(paramsJson: String?): Payload =
    withContext(Dispatchers.Default) {
      ...
      val file = File.createTempFile("nova-engine-screen-", ".mp4")
