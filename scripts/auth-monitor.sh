...
#
# Suggested cron: */30 * * * * /home/admin/nova-engine/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Nova Engine notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., nova-engine-alerts)
...
STATE_FILE="$HOME/.nova-engine/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Nova Engine if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use nova-engine
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Nova Engine to $NOTIFY_PHONE..."
            nova-engine send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: Nova Engine Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Nova Engine is down. Run: ssh l36 '~/nova-engine/scripts/mobile-reauth.sh'" "urgent"
    exit 1
