...
const dangerous = hasFlag("--dangerous") || process.env.NOVA_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "nova-engine ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "nova-engine-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "Nova Engine E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
