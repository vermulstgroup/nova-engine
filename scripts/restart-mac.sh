#!/usr/bin/env bash
# Reset Nova Engine like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${NOVA_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Nova Engine.app/Contents/MacOS/Nova Engine"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Nova Engine"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Nova Engine"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Nova Engine"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.nova-engine.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/nova-engine-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${NOVA_RESTART_LOG:-/tmp/nova-engine-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${NOVA_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.nova-engine/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  NOVA_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node nova-engine.mjs daemon install --force --runtime node"
      log "  node nova-engine.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.nova-engine/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.nova-engine.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Nova Engine instances"
kill_all_nova-engine
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Nova Engine"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Nova Engine.app" ]]; then
    APP_BUNDLE="/Applications/Nova Engine.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Nova Engine.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Nova Engine.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Nova Engine.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set NOVA_APP_BUNDLE to your installed Nova Engine.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node nova-engine.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node nova-engine.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".nova-engine", "nova-engine.json"), "utf8");
