#!/bin/bash
# Setup Nova Engine Auth Management System
...
echo "=== Nova Engine Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'nova-engine-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Nova Engine message: Send warning via Nova Engine itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/nova-engine-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/nova-engine-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/nova-engine-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now nova-engine-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status nova-engine-auth-monitor.timer"
echo ""
echo "Quick commands:"
