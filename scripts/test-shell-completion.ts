 ...
 * `nova-engine update`. Use it to verify the completion installation flow
...
\`nova-engine update\`, \`nova-engine doctor\`, and \`nova-engine onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Nova Engine Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`nova-engine completion --install\` later to enable.`));
    return;
  }
