...
ZIP=${1:?"Usage: $0 Nova Engine-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/nova-engine/nova-engine/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Nova Engine-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/nova-engine/nova-engine/releases/download/v${VERSION}/"}
