...
const shouldBuild = (deps) => {
  if (deps.env.NOVA_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.NOVA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[nova-engine] ${message}\n`);
};

const runNova Engine = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["nova-engine.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runNova Engine(deps);
  }
