...
BASE_IMAGE="${BASE_IMAGE:-nova-engine-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-nova-engine-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=nova-engine.sandbox=1)
NOTE
