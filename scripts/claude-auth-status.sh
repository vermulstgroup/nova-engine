...
# Checks both Claude Code and Nova Engine auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
NOVA_AUTH="$HOME/.nova-engine/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    nova-engine models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    nova-engine_status=$(check_nova-engine_auth 2>/dev/null || true)

    claude_expires=0
    nova-engine_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        nova-engine_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        nova-engine_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$NOVA_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$nova-engine_status" \
        --arg be "$nova-engine_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            nova-engine: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$nova-engine_status" == EXPIRED* ]] || [[ "$nova-engine_status" == MISSING* ]]; then
        echo "NOVA_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$nova-engine_status" == EXPIRING* ]]; then
        echo "NOVA_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Nova Engine Auth (~/.nova-engine/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$NOVA_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$NOVA_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'nova-engine doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active nova-engine >/dev/null 2>&1; then
    echo -e "Nova Engine service: ${GREEN}running${NC}"
else
    echo -e "Nova Engine service: ${RED}NOT running${NC}"
fi
