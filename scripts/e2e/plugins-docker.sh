...
IMAGE_NAME="nova-engine-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  home_dir=$(mktemp -d "/tmp/nova-engine-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.nova-engine/extensions/demo-plugin"

  cat > "$HOME/.nova-engine/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.nova-engine/extensions/demo-plugin/nova-engine.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$NOVA_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/nova-engine-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@nova-engine/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/nova-engine.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$NOVA_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$NOVA_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/nova-engine-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@nova-engine/demo-plugin-dir",
  "version": "0.0.1",
  "nova-engine": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/nova-engine.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$NOVA_ENTRY" plugins install "$dir_plugin"
	  node "$NOVA_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/nova-engine-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@nova-engine/demo-plugin-file",
  "version": "0.0.1",
  "nova-engine": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/nova-engine.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$NOVA_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$NOVA_ENTRY" plugins list --json > /tmp/plugins4.json
