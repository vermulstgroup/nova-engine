...
IMAGE_NAME="nova-engine-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/nova-engine-bin:$PATH"
  mkdir -p /tmp/nova-engine-bin
  cat > /tmp/nova-engine-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/nova-engine-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/nova-engine-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/nova-engine-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/nova-engine-e2e-$1.XXXXXX"
  }
  ...
  send_skills_flow() {
    ...
    workspace_dir="$HOME/.nova-engine/workspace"
    config_path="$HOME/.nova-engine/nova-engine.json"
    sessions_dir="$HOME/.nova-engine/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$HOME/.nova-engine/nova-engine.json" <<'"'"'JSON'"'"'
{
