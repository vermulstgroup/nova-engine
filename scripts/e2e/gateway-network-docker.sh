...
IMAGE_NAME="nova-engine-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="nova-engine-net-e2e-$$"
GW_NAME="nova-engine-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "NOVA_GATEWAY_TOKEN=$TOKEN" \
	  -e "NOVA_SKIP_CHANNELS=1" \
	  -e "NOVA_SKIP_GMAIL_WATCHER=1" \
	  -e "NOVA_SKIP_CRON=1" \
	  -e "NOVA_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
