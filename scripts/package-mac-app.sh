...
# Build and bundle Nova Engine into a minimal .app we can open.
# Outputs to dist/Nova Engine.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Nova Engine.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Nova Engine"
BUNDLE_ID="${BUNDLE_ID:-ai.nova-engine.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/nova-engine/nova-engine/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Nova Engine/Resources/Info.plist"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Nova Engine"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Nova Engine"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Nova Engine"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Nova Engine" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Nova Engine/Resources/Nova Engine.icns" "$APP_ROOT/Contents/Resources/Nova Engine.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Nova Engine/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "⏹  Stopping any running Nova Engine"
killall -q Nova Engine 2>/dev/null || true
