...
show_usage() {
    cat << EOF
clawlog - Nova Engine Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Nova Engine logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Nova Engine logs flow through the macOS unified log (subsystem: ai.nova-engine).
