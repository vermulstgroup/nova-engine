...
read_when:
  - Designing workspace memory (~/.nova-engine/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Nova Engine integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.nova-engine/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.nova-engine/workspace` as canonical human-readable memory.
...
```
~/.nova-engine/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Nova Engine**, but keep a separable core library.

### Why integrate into Nova Engine?

- Nova Engine already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `nova-engine memory recall "…" --k 25 --since 30d`
  - `nova-engine memory reflect --since 7d`
...
Pragmatic take for `~/.nova-engine/workspace`:
