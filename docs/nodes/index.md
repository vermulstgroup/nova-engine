...
macOS can also run in **node mode**: the menubar app connects to the Gateway’s WS server and exposes its local canvas/camera commands as a node (so `nova-engine nodes …` works against this Mac).
...
- `node.pair.*` (CLI: `nova-engine nodes pending/approve/reject`) is a separate gateway-owned
...
- **Approvals**: enforced on the node host via `~/.nova-engine/exec-approvals.json`.
...
- The token is `gateway.auth.token` from the gateway config (`~/.nova-engine/nova-engine.json` on the gateway host).
- `nova-engine node run` reads `NOVA_GATEWAY_TOKEN` for auth.
...
- `--display-name` on `nova-engine node run` / `nova-engine node install` (persists in `~/.nova-engine/node.json` on the node).
- `nova-engine nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
Approvals live on the node host at `~/.nova-engine/exec-approvals.json`.
...
nova-engine nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
...
- On headless node host, `system.run` is gated by exec approvals (`~/.nova-engine/exec-approvals.json`).
...
Nova Engine can run a **headless node host** (no UI) that connects to the Gateway
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.nova-engine/node.json`.
- Exec approvals are enforced locally via `~/.nova-engine/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host prefers the companion app exec host when reachable and falls
  back to local execution if the app is unavailable. Set `NOVA_NODE_EXEC_HOST=app` to require
  the app, or `NOVA_NODE_EXEC_FALLBACK=0` to disable fallback.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `nova-engine nodes …` works against this Mac).
