...
- Send media with optional captions via `nova-engine message send --media`.
- Allow auto-replies from the web inbox to include media alongside text.
- Keep per-type limits sane and predictable.

## CLI Surface

- `nova-engine message send --media <path-or-url> [--message <caption>]`
...
- When media is present, the web sender resolves local paths or URLs using the same pipeline as `nova-engine message send`.
- Multiple media entries are sent sequentially if provided.

## Inbound Media to Commands (Pi)

- When inbound web messages include media, Nova Engine downloads to a temp file and exposes templating variables:
