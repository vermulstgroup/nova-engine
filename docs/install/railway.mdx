...
Deploy Nova Engine on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/nova-engine` — Control UI
...
- `NOVA_STATE_DIR=/data/.nova-engine` (recommended)
- `NOVA_WORKSPACE_DIR=/data/workspace` (recommended)
- `NOVA_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Nova Engine state + workspace so you can migrate to another host without losing config or memory.
