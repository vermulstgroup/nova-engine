...
Deploy Nova Engine on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/nova-engine/nova-engine)
...
repository configures everything needed to run Nova Engine:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: NOVA_STATE_DIR
        value: /data/.nova-engine
      - key: NOVA_WORKSPACE_DIR
        value: /data/workspace
      - key: NOVA_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: nova-engine-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/nova-engine`.
...
If you use the original Nova Engine repository, Render will not auto-deploy your Nova Engine. To update it, run a manual Blueprint sync from the dashboard.
...
For Nova Engine, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Nova Engine host.
