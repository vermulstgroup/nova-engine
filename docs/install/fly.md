...
| `internal_port = 3000`         | Must match `--port 3000` (or `NOVA_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `NOVA_STATE_DIR = "/data"` | Persists state on the volume                                                |
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `NOVA_GATEWAY_PORT=3000`).
...
**Fix:** Ensure `NOVA_STATE_DIR=/data` is set in `fly.toml` and redeploy.
