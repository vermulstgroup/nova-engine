...
- Plugin manifests (`nova-engine.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`nova-engine.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `nova-engine doctor --fix`.
...
Everything else must hard-fail with: “Config invalid. Run `nova-engine doctor --fix`.”
