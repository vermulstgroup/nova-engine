...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.nova-engine/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.nova-engine/node.json` (node id + pairing token).
...
Path: `~/.nova-engine/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.nova-engine/exec-approvals.sock",
...
- Unix socket at `~/.nova-engine/exec-approvals.sock` (0600).
