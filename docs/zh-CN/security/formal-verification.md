...
summary: 针对 Nova Engine 最高风险路径的机器检查安全模型。
title: 形式化验证（安全模型）
x-i18n:
...
本页跟踪 Nova Engine 的**形式化安全模型**（目前是 TLA+/TLC；根据需要会添加更多）。

> 注意：一些较旧的链接可能引用了以前的项目名称。

**目标（北极星）：** 提供机器检查的论证，证明 Nova Engine 在明确假设下执行其
...
**目前还不是什么：** 证明"Nova Engine 在所有方面都是安全的"或完整 TypeScript 实现是正确的。

## 模型存放位置

模型维护在一个单独的仓库中：[vignesh07/nova-engine-formal-models](https://github.com/vignesh07/nova-engine-formal-models)。
