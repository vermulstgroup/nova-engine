...
你可以在 `~/.nova-engine/nova-engine.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/nova-engine.log"
  }
}
```
...
```bash
nova-engine channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.nova-engine/nova-engine.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/nova-engine/nova-engine-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Nova Engine 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
NOVA_DIAGNOSTICS=telegram.http,telegram.payload
```
...
```json
{
  ...
  "diagnostics": {
    ...
    "otel": {
      ...
      "serviceName": "nova-engine-gateway",
      "traces": true,
      "metrics": true,
      "logs": true,
      "sampleRate": 0.2,
...
- 你也可以使用 `nova-engine plugins enable diagnostics-otel` 启用插件。
...
- `nova-engine.tokens`（计数器，属性：`nova-engine.token`、`nova-engine.channel`、`nova-engine.provider`、`nova-engine.model`）
- `nova-engine.cost.usd`（计数器，属性：`nova-engine.channel`、`nova-engine.provider`、`nova-engine.model`）
- `nova-engine.run.duration_ms`（直方图，属性：`nova-engine.channel`、`nova-engine.provider`、`nova-engine.model`）
- `nova-engine.context.tokens`（直方图，属性：`nova-engine.context`、`nova-engine.channel`、`nova-engine.provider`、`nova-engine.model`）

消息流：

- `nova-engine.webhook.received`（计数器，属性：`nova-engine.channel`、`nova-engine.webhook`）
- `nova-engine.webhook.error`（计数器，属性：`nova-engine.channel`、`nova-engine.webhook`）
- `nova-engine.webhook.duration_ms`（直方图，属性：`nova-engine.channel`、`nova-engine.webhook`）
- `nova-engine.message.queued`（计数器，属性：`nova-engine.channel`、`nova-engine.source`）
- `nova-engine.message.processed`（计数器，属性：`nova-engine.channel`、`nova-engine.outcome`）
- `nova-engine.message.duration_ms`（直方图，属性：`nova-engine.channel`、`nova-engine.outcome`）

队列 + 会话：

- `nova-engine.queue.lane.enqueue`（计数器，属性：`nova-engine.lane`）
- `nova-engine.queue.lane.dequeue`（计数器，属性：`nova-engine.lane`）
- `nova-engine.queue.depth`（直方图，属性：`nova-engine.lane` 或 `nova-engine.channel=heartbeat`）
- `nova-engine.queue.wait_ms`（直方图，属性：`nova-engine.lane`）
- `nova-engine.session.state`（计数器，属性：`nova-engine.state`、`nova-engine.reason`）
- `nova-engine.session.stuck`（计数器，属性：`nova-engine.state`）
- `nova-engine.session.stuck_age_ms`（直方图，属性：`nova-engine.state`）
- `nova-engine.run.attempt`（计数器，属性：`nova-engine.attempt`）

### 导出的 span（名称 + 关键属性）

- `nova-engine.model.usage`
  - `nova-engine.channel`、`nova-engine.provider`、`nova-engine.model`
  - `nova-engine.sessionKey`、`nova-engine.sessionId`
  - `nova-engine.tokens.*`（input/output/cache_read/cache_write/total）
- `nova-engine.webhook.processed`
  - `nova-engine.channel`、`nova-engine.webhook`、`nova-engine.chatId`
- `nova-engine.webhook.error`
  - `nova-engine.channel`、`nova-engine.webhook`、`nova-engine.chatId`、`nova-engine.error`
- `nova-engine.message.processed`
  - `nova-engine.channel`、`nova-engine.outcome`、`nova-engine.chatId`、`nova-engine.messageId`、`nova-engine.sessionKey`、`nova-engine.sessionId`、`nova-engine.reason`
- `nova-engine.session.stuck`
  - `nova-engine.state`、`nova-engine.ageMs`、`nova-engine.queueDepth`、`nova-engine.sessionKey`、`nova-engine.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `nova-engine doctor`。
