---
summary: 关于 Nova Engine 安装、配置和使用的常见问题
title: 常见问题
x-i18n:
...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [安装和设置 Nova Engine 的推荐方式是什么？](#whats-the-recommended-way-to-install-and-set-up-nova-engine)
  ...
  - [无法访问 docs.nova-engine.ai（SSL 错误），怎么办？](#i-cant-access-docsnova-engineai-ssl-error-what-now)
...
- [如何安装 beta 版本，beta 和 dev 有什么区别？](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  ...
  - [Windows 安装提示找不到 git 或无法识别 nova-engine](#windows-install-says-git-not-found-or-nova-engine-not-recognized)
  - [文档没有解答我的问题——如何获得更好的答案？](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [如何在 Linux 上安装 Nova Engine？](#how-do-i-install-nova-engine-on-linux)
  - [如何在 VPS 上安装 Nova Engine？](#how-do-i-install-nova-engine-on-a-vps)
  - [云/VPS 安装指南在哪里？](#where-are-the-cloudvps-install-guides)
  - [可以让 Nova Engine 自行更新吗？](#can-i-ask-nova-engine-to-update-itself)
  - [新手引导向导具体做了什么？](#what-does-the-onboarding-wizard-actually-do)
  - [运行 Nova Engine 需要 Claude 或 OpenAI 订阅吗？](#do-i-need-a-claude-or-openai-subscription-to-run-this)
  ...
  - [如果我买了 Mac mini 运行 Nova Engine，能连接到我的 MacBook Pro 吗？](#if-i-buy-a-mac-mini-to-run-nova-engine-can-i-connect-it-to-my-macbook-pro)
  - [可以使用 Bun 吗？](#can-i-use-bun)
  - [Telegram：`allowFrom` 填什么？](#telegram-what-goes-in-allowfrom)
  - [多人能否使用同一个 WhatsApp 号码配合不同的 Nova Engine 实例？](#can-multiple-people-use-one-whatsapp-number-with-different-nova-engine-instances)
  ...
  - [在专用机器上运行 Nova Engine 有多重要？](#how-important-is-it-to-run-nova-engine-on-a-dedicated-machine)
  - [VPS 的最低要求和推荐操作系统是什么？](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [可以在虚拟机中运行 Nova Engine 吗？有什么要求？](#can-i-run-nova-engine-in-a-vm-and-what-are-the-requirements)
- [什么是 Nova Engine？](#what-is-nova-engine)
  - [用一段话描述 Nova Engine？](#what-is-nova-engine-in-one-paragraph)
  - [价值主张是什么？](#whats-the-value-proposition)
  - [刚设置好，应该先做什么？](#i-just-set-it-up-what-should-i-do-first)
  - [Nova Engine 日常最常用的五个场景是什么？](#what-are-the-top-five-everyday-use-cases-for-nova-engine)
  - [Nova Engine 能否帮助 SaaS 进行获客、外联、广告和博客？](#can-nova-engine-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills 与自动化](#skills-and-automation)
  ...
  - [Nova Engine 能否按计划或在后台持续运行任务？](#can-nova-engine-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [磁盘上的文件位置](#where-things-live-on-disk)
  - [Nova Engine 使用的所有数据都保存在本地吗？](#is-all-data-used-with-nova-engine-saved-locally)
  - [Nova Engine 将数据存储在哪里？](#where-does-nova-engine-store-its-data)
  ...
  - [如何完全卸载 Nova Engine？](#how-do-i-completely-uninstall-nova-engine)
...
- [配置基础](#config-basics)
  ...
  - [Nova Engine 浏览器可以无头运行吗？](#can-the-nova-engine-browser-run-headless)
...
- [远程 Gateway 网关与节点](#remote-gateways-nodes)
  ...
  - [两个 Nova Engine 实例（本地 + VPS）可以互相通信吗？](#can-two-nova-engine-instances-talk-to-each-other-local-vps)
...
- [环境变量和 .env 加载](#env-vars-and-env-loading)
  - [Nova Engine 如何加载环境变量？](#how-does-nova-engine-load-environment-variables)
...
- [会话与多聊天](#sessions-multiple-chats)
  ...
  - [能否创建一个 Nova Engine 实例团队——一个 CEO 和多个智能体？](#is-there-a-way-to-make-a-team-of-nova-engine-instances-one-ceo-and-many-agents)
  - [为什么上下文在任务中途被截断了？如何防止？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 Nova Engine 但保留安装？](#how-do-i-completely-reset-nova-engine-but-keep-it-installed)
  ...
  - [为什么 Nova Engine 不在群组中回复？](#why-doesnt-nova-engine-reply-in-a-group)
...
- [模型：默认值、选择、别名、切换](#models-defaults-selection-aliases-switching)
  ...
  - [Nova Engine、Flawd 和 Krill 使用什么模型？](#what-do-nova-engine-flawd-and-krill-use-for-models)
...
- [Gateway 网关：端口、“已在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  ...
  - [为什么 `nova-engine gateway status` 显示 `Runtime: running` 但 `RPC probe: failed`？](#why-does-nova-engine-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [为什么 `nova-engine gateway status` 显示 `Config (cli)` 和 `Config (service)` 不同？](#why-does-nova-engine-gateway-status-show-config-cli-and-config-service-different)
  - ["another gateway instance is already listening"是什么意思？](#what-does-another-gateway-instance-is-already-listening-mean)
  - [如何以远程模式运行 Nova Engine（客户端连接到其他位置的 Gateway 网关）？](#how-do-i-run-nova-engine-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [日志与调试](#logging-and-debugging)
  ...
  - [我在 Windows 上关闭了终端——如何重启 Nova Engine？](#i-closed-my-terminal-on-windows-how-do-i-restart-nova-engine)
  ...
  - [通俗解释：`nova-engine gateway restart` 与 `nova-engine gateway`](#eli5-nova-engine-gateway-restart-vs-nova-engine-gateway)
...
- [安全与访问控制](#security-and-access-control)
  - [将 Nova Engine 暴露给入站私信安全吗？](#is-it-safe-to-expose-nova-engine-to-inbound-dms)
...
7. **Gateway 网关快照**
   ...
   nova-engine health --verbose   # 出错时显示目标 URL + 配置路径
   ```
...
这会从 **git checkout** 安装 Nova Engine，这样智能体可以读取代码 + 文档，并推理你正在运行的确切版本。你可以随时通过不带 `--install-method git` 重新运行安装程序切回稳定版。

提示：要求智能体**计划并监督**修复（逐步进行），然后只执行必要的命令。这样改动较小，更容易审查。

如果你发现了真正的 bug 或修复方案，请提交 GitHub issue 或发送 PR：
https://github.com/nova-engine/nova-engine/issues
https://github.com/nova-engine/nova-engine/pulls
...
- `nova-engine status`：Gateway 网关/智能体健康状况 + 基本配置的快速快照。
- `nova-engine models status`：检查提供商认证 + 模型可用性。
- `nova-engine doctor`：验证并修复常见的配置/状态问题。

其他有用的 CLI 检查：`nova-engine status --all`、`nova-engine logs --follow`、
...
如果你还没有全局安装，通过 `pnpm nova-engine onboard` 运行。
...
- 如果要求认证，运行 `nova-engine dashboard` 并使用带令牌的链接（`?token=...`）。
- 令牌与 `gateway.auth.token`（或 `NOVA_GATEWAY_TOKEN`）的值相同，UI 在首次加载后会存储它。

**非本地环境：**

- **Tailscale Serve**（推荐）：保持绑定 loopback，运行 `nova-engine gateway --tailscale serve`，打开 `https://<magicdns>/`。如果 `gateway.auth.allowTailscale` 为 `true`，身份标头满足认证要求（无需令牌）。
- **Tailnet 绑定**：运行 `nova-engine gateway --bind tailnet --token "<token>"`，打开 `http://<tailscale-ip>:18789/`，在仪表板设置中粘贴令牌。
- **SSH 隧道**：`ssh -N -L 18789:127.0.0.1:18789 user@host`，然后从 `nova-engine dashboard` 打开 `http://127.0.0.1:18789/?token=...`。
...
1. 在新机器上安装 Nova Engine。
2. 从旧机器复制 `$NOVA_STATE_DIR`（默认：`~/.nova-engine`）。
3. 复制你的工作区（默认：`~/.nova-engine/workspace`）。
4. 运行 `nova-engine doctor` 并重启 Gateway 网关服务。

这会保留配置、认证配置文件、WhatsApp 凭据、会话和记忆。如果你处于远程模式，请记住 Gateway 网关主机拥有会话存储和工作区。

**重要：** 如果你只将工作区提交/推送到 GitHub，你只备份了**记忆 + 引导文件**，但**不包括**会话历史或认证。它们位于 `~/.nova-engine/` 下（例如 `~/.nova-engine/agents/<agentId>/sessions/`）。

相关：[迁移](/install/migrating)、[磁盘上的文件位置](/help/faq#where-does-nova-engine-store-its-data)、
...
查看 GitHub 变更日志：
https://github.com/nova-engine/nova-engine/blob/main/CHANGELOG.md

最新条目在顶部。如果顶部部分标记为 **Unreleased**，则下一个带日期的部分是最新发布版本。条目按**亮点**、**变更**和**修复**分组（需要时还有文档/其他部分）。

### 无法访问 docs.nova-engine.ai（SSL 错误），怎么办

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.nova-engine.ai`。禁用该功能或将 `docs.nova-engine.ai` 加入白名单，然后重试。更多详情：[故障排除](/help/troubleshooting#docsnova-engineai-shows-an-ssl-error-comcastxfinity)。
请帮助我们在此处报告以解除封锁：https://spa.xfinity.com/check_url_status。

如果仍然无法访问该网站，文档在 GitHub 上有镜像：
https://github.com/nova-engine/nova-engine/tree/main/docs
...
https://nova-engine.ai/install.ps1
...
### Windows 安装提示找不到 git 或无法识别 nova-engine
...
**2) nova-engine is not recognized（安装后）**
...
`nova-engine onboard` 是推荐的设置路径。在**本地模式**下，它引导你完成：
...
### 运行 Nova Engine 需要 Claude 或 OpenAI 订阅吗

不需要。你可以使用 **API 密钥**（Anthropic/OpenAI/其他）或**纯本地模型**运行 Nova Engine，这样你的数据留在你的设备上。订阅（Claude Pro/Max 或 OpenAI Codex）是这些提供商的可选认证方式。
...
`claude setup-token` 通过 Claude Code CLI 生成一个**令牌字符串**（在 Web 控制台中不可用）。你可以在**任何机器**上运行它。在向导中选择 **Anthropic token (paste setup-token)** 或使用 `nova-engine models auth paste-token --provider anthropic` 粘贴。令牌作为 **anthropic** 提供商的认证配置文件存储，像 API 密钥一样使用（无自动刷新）。更多详情：[OAuth](/concepts/oauth)。
...
复制它打印的令牌，然后在向导中选择 **Anthropic token (paste setup-token)**。如果你想在 Gateway 网关主机上运行，使用 `nova-engine models auth setup-token --provider anthropic`。如果你在其他地方运行了 `claude setup-token`，在 Gateway 网关主机上使用 `nova-engine models auth paste-token --provider anthropic` 粘贴。参阅 [Anthropic](/providers/anthropic)。

### 是否支持 Claude 订阅认证（Claude Pro/Max）

是的——通过 **setup-token**。Nova Engine 不再复用 Claude Code CLI OAuth 令牌；请使用 setup-token 或 Anthropic API 密钥。在任何地方生成令牌并在 Gateway 网关主机上粘贴。参阅 [Anthropic](/providers/anthropic) 和 [OAuth](/concepts/oauth)。
...
提示：设置一个**备用模型**，这样 Nova Engine 在某个提供商被限速时仍能继续回复。
...
Nova Engine 通过 OAuth（ChatGPT 登录）支持 **OpenAI Code (Codex)**。向导可以运行 OAuth 流程，并在适当时将默认模型设置为 `openai-codex/gpt-5.2`。参阅[模型提供商](/concepts/model-providers)和[向导](/start/wizard)。

### 是否支持 OpenAI 订阅认证（Codex OAuth）

是的。Nova Engine 完全支持 **OpenAI Code (Codex) 订阅 OAuth**。新手引导向导可以为你运行 OAuth 流程。

参阅 [OAuth](/concepts/oauth)、[模型提供商](/concepts/model-providers)和[向导](/start/wizard)。

### 如何设置 Gemini CLI OAuth

Gemini CLI 使用**插件认证流程**，而不是 `nova-engine.json` 中的 client id 或 secret。

步骤：

1. 启用插件：`nova-engine plugins enable google-gemini-cli-auth`
2. 登录：`nova-engine models auth login --provider google-gemini-cli --set-default`

这会在 Gateway 网关主机上将 OAuth 令牌存储为认证配置文件。详情：[模型提供商](/concepts/model-providers)。

### 本地模型适合日常聊天吗

通常不适合。Nova Engine 需要大上下文 + 强安全性；小显卡会截断且泄漏。如果必须使用，请在本地运行你能运行的**最大** MiniMax M2.1 版本（LM Studio），参阅 [/gateway/local-models](/gateway/local-models)。较小/量化的模型会增加提示注入风险——参阅[安全](/gateway/security)。
...
不需要。Nova Engine 运行在 macOS 或 Linux 上（Windows 通过 WSL2）。Mac mini 是可选的——有些人买一台作为常开主机，但小型 VPS、家庭服务器或 Raspberry Pi 级别的设备也可以。
...
你需要**某台登录了 Messages 的 macOS 设备**。它**不一定**是 Mac mini——任何 Mac 都可以。Nova Engine 的 iMessage 集成在 macOS 上运行（BlueBubbles 或 `imsg`），而 Gateway 网关可以在其他地方运行。
...
### 如果我买了 Mac mini 运行 Nova Engine，能连接到我的 MacBook Pro 吗
...
- 使用 `nova-engine nodes status` / `nova-engine nodes list` 查看它。
...
- 给你的机器人发私信，然后运行 `nova-engine logs --follow` 并读取 `from.id`。
...
### 多人能否使用同一个 WhatsApp 号码配合不同的 Nova Engine 实例
...
如果你通过 systemd 运行 Nova Engine，确保服务 PATH 包含 `/home/linuxbrew/.linuxbrew/bin`（或你的 brew 前缀），以便 `brew` 安装的工具在非登录 shell 中可解析。
...
这**不会删除你的数据**——它只改变 Nova Engine 代码的安装位置。你的状态
（`~/.nova-engine`）和工作区（`~/.nova-engine/workspace`）保持不变。
...
**Nova Engine 特定说明：** WhatsApp/Telegram/Slack/Mattermost（插件）/Discord 在 VPS 上都能正常工作。唯一的真正权衡是**无头浏览器**与可见窗口。参阅[浏览器](/tools/browser)。

**推荐默认值：** 如果之前遇到过 Gateway 网关断连，使用 VPS。当你正在积极使用 Mac 并且需要本地文件访问或可见浏览器的 UI 自动化时，本地运行很好。

### 在专用机器上运行 Nova Engine 有多重要
...
Nova Engine 是轻量级的。对于基本的 Gateway 网关 + 一个聊天渠道：
...
Nova Engine 是一个运行在你自己设备上的个人 AI 助手。它在你已经使用的消息平台上回复（WhatsApp、Telegram、Slack、Mattermost（插件）、Discord、Google Chat、Signal、iMessage、WebChat），还可以在支持的平台上进行语音和实时 Canvas。**Gateway 网关** 是常开的控制平面；助手是产品。

### 价值主张是什么

Nova Engine 不是“只是一个 Claude 包装器”。它是一个**本地优先的控制平面**，让你在**自己的硬件**上运行强大的助手，可从你已经使用的聊天应用访问，具有有状态会话、记忆和工具——无需将工作流程的控制权交给托管 SaaS。
...
### Nova Engine 能否帮助 SaaS 进行获客、外联、广告和博客

可以用于**调研、筛选和起草**。它可以扫描网站、建立候选名单、总结潜在客户，并撰写外联或广告文案草稿。

对于**外联或广告投放**，请保持人工审核。避免垃圾邮件，遵守当地法律和平台政策，在发送之前审查所有内容。最安全的模式是让 Nova Engine 起草，由你批准。

文档：[安全](/gateway/security)。

### 相比 Claude Code，在 Web 开发方面有什么优势

Nova Engine 是一个**个人助手**和协调层，不是 IDE 替代品。使用 Claude Code 或 Codex 在仓库中进行最快的直接编码循环。当你需要持久记忆、跨设备访问和工具编排时，使用 Nova Engine。
...
展示：https://nova-engine.ai/showcase

## Skills 与自动化

### 如何自定义 Skills 而不弄脏仓库

使用托管覆盖而不是编辑仓库副本。将你的更改放在 `~/.nova-engine/skills/<name>/SKILL.md`（或通过 `~/.nova-engine/nova-engine.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级是 `<workspace>/skills` > `~/.nova-engine/skills` > 内置，所以托管覆盖优先生效而不会修改 git。只有值得上游合并的编辑才应该放在仓库中并作为 PR 提交。

### 可以从自定义文件夹加载 Skills 吗

可以。通过 `~/.nova-engine/nova-engine.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.nova-engine/skills` → 内置 → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，Nova Engine 将其视为 `<workspace>/skills`。
...
不能直接运行。macOS Skills 受 `metadata.nova-engine.os` 和所需二进制文件限制，Skills 只有在 **Gateway 网关主机**上符合条件时才会出现在系统提示中。在 Linux 上，`darwin` 专用 Skills（如 `apple-notes`、`apple-reminders`、`things-mac`）不会加载，除非你覆盖限制。

你有三种支持的模式：

**方案 A - 在 Mac 上运行 Gateway 网关（最简单）。**
在 macOS 二进制文件所在的地方运行 Gateway 网关，然后从 Linux 通过[远程模式](#how-do-i-run-nova-engine-in-remote-mode-client-connects-to-a-gateway-elsewhere)或 Tailscale 连接。Skills 正常加载，因为 Gateway 网关主机是 macOS。

**方案 B - 使用 macOS 节点（无需 SSH）。**
在 Linux 上运行 Gateway 网关，配对一个 macOS 节点（菜单栏应用），并在 Mac 上将**节点运行命令**设置为“始终询问”或“始终允许”。当所需二进制文件存在于节点上时，Nova Engine 可以将 macOS 专用 Skills 视为符合条件。智能体通过 `nodes` 工具运行这些 Skills。如果你选择“始终询问”，在提示中批准“始终允许”会将该命令添加到允许列表。
...
3. 覆盖 Skills 元数据（工作区或 `~/.nova-engine/skills`）以允许 Linux：
   ```markdown
   ---
   name: imsg
   description: iMessage/SMS CLI for listing chats, history, watch, and sending.
   metadata: { "nova-engine": { "os": ["darwin", "linux"], "requires": { "bins": ["imsg"] } } }
   ---
   ```
4. 开始新会话以刷新 Skills 快照。

对于 iMessage，你也可以将 `channels.imessage.cliPath` 指向 SSH 包装器（Nova Engine 只需要 stdio）。参阅 [iMessage](/channels/imessage)。
...
ClawHub 安装到当前目录下的 `./skills`（或回退到你配置的 Nova Engine 工作区）；Nova Engine 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.nova-engine/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [ClawHub](/tools/clawhub)。
...
Nova Engine 记忆就是智能体工作区中的 Markdown 文件：

- 每日笔记在 `memory/YYYY-MM-DD.md`
- 精选的长期笔记在 `MEMORY.md`（仅限主/私密会话）

Nova Engine 还会运行**静默的预压缩记忆刷新**，以提醒模型在自动压缩之前写入持久笔记。这只在工作区可写时运行（只读沙箱会跳过）。参阅[记忆](/concepts/memory)。
...
如果你没有明确设置提供商，Nova Engine 会在能解析 API 密钥（认证配置文件、`models.providers.*.apiKey` 或环境变量）时自动选择提供商。如果 OpenAI 密钥可解析则优先使用 OpenAI，否则如果 Gemini 密钥可解析则使用 Gemini。如果两个密钥都不可用，记忆搜索保持禁用直到你配置它。如果你配置了本地模型路径且存在，Nova Engine 优先使用 `local`。
...
### Nova Engine 使用的所有数据都保存在本地吗

不是——**Nova Engine 的状态是本地的**，但**外部服务仍然会看到你发送给它们的内容**。

- **默认本地：** 会话、记忆文件、配置和工作区位于 Gateway 网关主机上（`~/.nova-engine` + 你的工作区目录）。
...
### Nova Engine 将数据存储在哪里

所有内容位于 `$NOVA_STATE_DIR`（默认：`~/.nova-engine`）下：

| 路径                                                            | 用途                                                 |
| --------------------------------------------------------------- | ---------------------------------------------------- |
| `$NOVA_STATE_DIR/nova-engine.json`                             | 主配置（JSON5）                                      |
...
旧版单智能体路径：`~/.nova-engine/agent/*`（通过 `nova-engine doctor` 迁移）。

你的**工作区**（AGENTS.md、记忆文件、Skills 等）是独立的，通过 `agents.defaults.workspace` 配置（默认：`~/.nova-engine/workspace`）。

### AGENTS.md / SOUL.md / USER.md / MEMORY.md 应该放在哪里

这些文件位于**智能体工作区**中，而不是 `~/.nova-engine`。

- **工作区（按智能体）**：`AGENTS.md`、`SOUL.md`、`IDENTITY.md`、`USER.md`、
  `MEMORY.md`（或 `memory.md`）、`memory/YYYY-MM-DD.md`、可选的 `HEARTBEAT.md`。
- **状态目录（`~/.nova-engine`）**：配置、凭据、认证配置文件、会话、日志和共享 Skills（`~/.nova-engine/skills`）。

默认工作区是 `~/.nova-engine/workspace`，可通过以下方式配置：
...
**不要**提交 `~/.nova-engine` 下的任何内容（凭据、会话、令牌）。如果你需要完整恢复，将工作区和状态目录分别备份（参阅上面的迁移问题）。
...
可以。工作区是**默认 cwd** 和记忆锚点，不是硬沙箱。相对路径在工作区内解析，但绝对路径可以访问其他主机位置，除非启用了沙箱。如果你需要隔离，使用 [`agents.defaults.sandbox`](/gateway/sandboxing) 或按智能体的沙箱设置。如果你希望某个仓库作为默认工作目录，将该智能体的 `workspace` 指向仓库根目录。Nova Engine 仓库只是源代码；除非你有意要让智能体在其中工作，否则保持工作区独立。
...
Nova Engine 从 `$NOVA_CONFIG_PATH`（默认：`~/.nova-engine/nova-engine.json`）读取可选的 **JSON5** 配置：

```
$NOVA_CONFIG_PATH
```

如果文件不存在，使用安全的默认值（包括默认工作区 `~/.nova-engine/workspace`）。
...
如果你**确实**想要开放 local loopback，从配置中移除 `gateway.auth`。Doctor 可以随时为你生成令牌：`nova-engine doctor --generate-gateway-token`。
...
`web_fetch` 无需 API 密钥即可工作。`web_search` 需要 Brave Search API 密钥。**推荐：** 运行 `nova-engine configure --section web` 将其存储在 `tools.web.search.apiKey` 中。环境变量替代方案：为 Gateway 网关进程设置 `BRAVE_API_KEY`。
...
- 守护进程从 `~/.nova-engine/.env`（或服务环境）读取环境变量。
...
- 从备份恢复（git 或复制的 `~/.nova-engine/nova-engine.json`）。
- 如果没有备份，重新运行 `nova-engine doctor` 并重新配置渠道/模型。
- 如果这是意外情况，提交 bug 并附上你最后已知的配置或任何备份。
- 本地编码智能体通常可以从日志或历史中重建工作配置。

避免方法：

- 对小更改使用 `nova-engine config set`。
- 对交互式编辑使用 `nova-engine configure`。
...
- Gateway 网关正在运行：`nova-engine gateway status`
- Gateway 网关健康：`nova-engine status`
- 渠道健康：`nova-engine channels status`
...
### 两个 Nova Engine 实例（本地 + VPS）可以互相通信吗

可以。没有内置的“机器人对机器人”桥接，但你可以通过几种可靠的方式实现：

**最简单：** 使用两个机器人都能访问的普通聊天渠道（Telegram/Slack/WhatsApp）。让机器人 A 给机器人 B 发消息，然后让机器人 B 正常回复。

**CLI 桥接（通用）：** 运行一个脚本调用另一个 Gateway 网关，使用 `nova-engine agent --message ... --deliver`，定向到另一个机器人监听的聊天。如果一个机器人在远程 VPS 上，通过 SSH/Tailscale 将你的 CLI 指向该远程 Gateway 网关（参阅[远程访问](/gateway/remote)）。
...
### Nova Engine 如何加载环境变量

Nova Engine 从父进程（shell、launchd/systemd、CI 等）读取环境变量，并额外加载：

- 当前工作目录下的 `.env`
- `~/.nova-engine/.env`（即 `$NOVA_STATE_DIR/.env`）的全局回退 `.env`
...
1. 将缺失的密钥放在 `~/.nova-engine/.env` 中，这样即使服务不继承你的 shell 环境也能被获取。
...
`nova-engine models status` 报告的是 **shell 环境导入**是否启用。"Shell env: off"**不**意味着你的环境变量缺失——它只意味着 Nova Engine 不会自动加载你的登录 shell。

如果 Gateway 网关作为服务（launchd/systemd）运行，它不会继承你的 shell 环境。通过以下方式之一修复：

1. 将令牌放在 `~/.nova-engine/.env` 中：
   ```
...
### 能否创建一个 Nova Engine 实例团队——一个 CEO 和多个智能体
...
- 如果你使用了配置文件（`--profile` / `NOVA_PROFILE`），重置每个状态目录（默认为 `~/.nova-engine-<profile>`）。
- 开发重置：`nova-engine gateway --dev --reset`（仅限开发；清除开发配置 + 凭据 + 会话 + 工作区）。
...
如果 `HEARTBEAT.md` 存在但实际上为空（只有空行和 markdown 标题如 `# Heading`），Nova Engine 会跳过心跳运行以节省 API 调用。如果文件不存在，心跳仍然运行，由模型决定做什么。

按智能体覆盖使用 `agents.list[].heartbeat`。文档：[心跳](/gateway/heartbeat)。

### 需要在 WhatsApp 群组中添加“机器人账号”吗

不需要。Nova Engine 运行在**你自己的账户**上，所以如果你在群组中，Nova Engine 就能看到它。
...
- **磁盘增长：** 会话 + 记录位于 `~/.nova-engine/agents/<agentId>/sessions/` 下。
...
- 使用 `nova-engine doctor` 发现无用的工作区和配置文件不匹配。
...
Nova Engine 的默认模型是你设置的：

```
agents.defaults.model.primary
```

模型以 `provider/model` 引用（示例：`anthropic/claude-opus-4-5`）。如果你省略提供商，Nova Engine 目前假设 `anthropic` 作为临时弃用回退——但你仍然应该**明确**设置 `provider/model`。
...
- 聊天中的 `/model`（快速，按会话）
- `nova-engine models set ...`（只更新模型配置）
- `nova-engine configure --section models`（交互式）
- 编辑 `~/.nova-engine/nova-engine.json` 中的 `agents.defaults.model`

避免使用部分对象执行 `config.apply`，除非你打算替换整个配置。如果你确实覆盖了配置，从备份恢复或重新运行 `nova-engine doctor` 来修复。

文档：[模型](/concepts/models)、[Configure](/cli/configure)、[Config](/cli/config)、[Doctor](/gateway/doctor)。

### Nova Engine、Flawd 和 Krill 使用什么模型

- **Nova Engine + Flawd：** Anthropic Opus（`anthropic/claude-opus-4-5`）——参阅 [Anthropic](/providers/anthropic)。
...
是的。Nova Engine 内置了一些默认简写（仅在模型存在于 `agents.defaults.models` 中时应用）：
...
- 运行 `nova-engine agents add <id>` 并在向导中配置认证。
...
冷却期适用于失败的配置文件（指数退避），因此 Nova Engine 即使在提供商被限速或临时失败时也能继续响应。
...
- **确认认证配置文件的位置**（新路径 vs 旧路径）
  - 当前：`~/.nova-engine/agents/<agentId>/agent/auth-profiles.json`
  - 旧版：`~/.nova-engine/agent/*`（通过 `nova-engine doctor` 迁移）
- **确认环境变量被 Gateway 网关加载**
  - 如果你在 shell 中设置了 `ANTHROPIC_API_KEY` 但通过 systemd/launchd 运行 Gateway 网关，它可能不会继承。将其放在 `~/.nova-engine/.env` 中或启用 `env.shellEnv`。
- **确保你编辑的是正确的智能体**
  - 多智能体设置意味着可能有多个 `auth-profiles.json` 文件。
- **完整性检查模型/认证状态**
  - 使用 `nova-engine models status` 查看已配置的模型以及提供商是否已认证。
...
- **使用 setup-token**
  - 运行 `claude setup-token`，然后用 `nova-engine models auth setup-token --provider anthropic` 粘贴。
  - 如果令牌在另一台机器上创建，使用 `nova-engine models auth paste-token --provider anthropic`。
- **如果你想使用 API 密钥**
  - 在 **Gateway 网关主机**上将 `ANTHROPIC_API_KEY` 放入 `~/.nova-engine/.env`。
  - 清除任何强制缺失配置文件的固定顺序：
    ```bash
...
如果你的模型配置包含 Google Gemini 作为回退（或你切换到了 Gemini 简写），Nova Engine 会在模型回退期间尝试它。如果你没有配置 Google 凭据，你会看到 `No API key found for provider "google"`。
...
修复：Nova Engine 现在为 Google Antigravity Claude 剥离未签名的 thinking 块。如果仍然出现，开始**新会话**或为该智能体设置 `/thinking off`。
...
如果某个配置文件处于短期**冷却**（速率限制/超时/认证失败）或较长的**禁用**状态（计费/额度不足），Nova Engine 可能会临时跳过它。要检查这一点，运行 `nova-engine models status --json` 并查看 `auth.unusableProfiles`。调优：`auth.cooldowns.billingBackoffHours*`。
...
### 为什么 nova-engine gateway status 显示 Runtime: running 但 RPC probe: failed

因为"running"是 **supervisor** 的视角（launchd/systemd/schtasks）。RPC 探测是 CLI 实际连接到 Gateway 网关 WebSocket 并调用 `status`。

使用 `nova-engine gateway status` 并关注这些行：

- `Probe target:`（探测实际使用的 URL）
- `Listening:`（端口上实际绑定的内容）
- `Last gateway error:`（进程存活但端口未监听时的常见根因）

### 为什么 nova-engine gateway status 显示 Config (cli) 和 Config (service) 不同
...
Nova Engine 通过在启动时立即绑定 WebSocket 监听器来强制运行时锁（默认 `ws://127.0.0.1:18789`）。如果绑定因 `EADDRINUSE` 失败，它会抛出 `GatewayLockError` 表示另一个实例已在监听。

修复：停止另一个实例，释放端口，或使用 `nova-engine gateway --port <port>` 运行。

### 如何以远程模式运行 Nova Engine（客户端连接到其他位置的 Gateway 网关）
...
- `nova-engine gateway` 仅在 `gateway.mode` 为 `local` 时启动（或你传递覆盖标志）。
...
- 控制 UI 将令牌存储在浏览器 localStorage 键 `nova-engine.control.settings.v1` 中。
- UI 可以导入一次 `?token=...`（和/或 `?password=...`），然后从 URL 中剥离。

修复：

- 最快：`nova-engine dashboard`（打印 + 复制带令牌的链接，尝试打开；如果无头则显示 SSH 提示）。
- 如果你还没有令牌：`nova-engine doctor --generate-gateway-token`。
...
- 仍然卡住？运行 `nova-engine status --all` 并按[故障排除](/gateway/troubleshooting)操作。参阅[仪表板](/web/dashboard)了解认证详情。
...
- 每实例使用 `nova-engine --profile <name> …`（自动创建 `~/.nova-engine-<name>`）。
- 在每个配置文件配置中设置唯一的 `gateway.port`（或手动运行时传 `--port`）。
- 安装每配置文件的服务：`nova-engine --profile <name> gateway install`。

配置文件还会为服务名称添加后缀（`bot.molt.<profile>`；旧版 `com.nova-engine.*`、`nova-engine-gateway-<profile>.service`、`Nova Engine Gateway 网关 (<profile>)`）。
...
- macOS：`$NOVA_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`（默认：`~/.nova-engine/logs/...`；配置文件使用 `~/.nova-engine-<profile>/logs/...`）
- Linux：`journalctl --user -u nova-engine-gateway[-<profile>].service -n 200 --no-pager`
- Windows：`schtasks /Query /TN "Nova Engine Gateway 网关 (<profile>)" /V /FO LIST`
...
如果你手动运行 Gateway 网关，`nova-engine gateway --force` 可以回收端口。参阅 [Gateway 网关](/gateway)。

### 我在 Windows 上关闭了终端——如何重启 Nova Engine
...
1. Gateway 网关在运行吗？`nova-engine gateway status`
2. Gateway 网关健康吗？`nova-engine status`
3. UI 有正确的令牌吗？`nova-engine dashboard`
...
### 通俗解释：nova-engine gateway restart 与 nova-engine gateway

- `nova-engine gateway restart`：重启**后台服务**（launchd/systemd）。
- `nova-engine gateway`：在这个终端会话中**前台**运行 Gateway 网关。

如果你安装了服务，使用 Gateway 网关命令。想要一次性前台运行时使用 `nova-engine gateway`。
...
智能体的出站附件必须包含 `MEDIA:<path-or-url>` 行（独占一行）。参阅 [Nova Engine 助手设置](/start/nova-engine)和 [Agent send](/tools/agent-send)。
...
### 将 Nova Engine 暴露给入站私信安全吗

将入站私信视为不可信输入。默认设计旨在降低风险：

- 支持私信的渠道上的默认行为是**配对**：
  - 未知发送者会收到配对码；机器人不处理他们的消息。
  - 批准方式：`nova-engine pairing approve <channel> <code>`
  - 每个渠道的待处理请求上限为 **3 个**；如果没收到代码，检查 `nova-engine pairing list <channel>`。
- 公开开放私信需要明确选择加入（`dmPolicy: "open"` 且允许列表 `"*"`）。

运行 `nova-engine doctor` 以发现有风险的私信策略。
...
不会。WhatsApp 的默认私信策略是**配对**。未知发送者只会收到配对码，他们的消息**不会被处理**。Nova Engine 只回复它收到的聊天或你明确触发的发送。
...
Nova Engine 默认阻止**跨提供商**消息。如果工具调用绑定到 Telegram，除非你明确允许，否则不会发送到 Discord。
...
**答：** 在 Nova Engine 中，凭据和模型选择是分开的。设置 `ANTHROPIC_API_KEY`（或在认证配置文件中存储 Anthropic API 密钥）启用认证，但实际的默认模型是你在 `agents.defaults.model.primary` 中配置的（例如 `anthropic/claude-sonnet-4-5` 或 `anthropic/claude-opus-4-5`）。如果你看到 `No credentials found for profile "anthropic:default"`，意味着 Gateway 网关在正在运行的智能体的预期 `auth-profiles.json` 中找不到 Anthropic 凭据。

---

仍然卡住？在 [Discord](https://discord.com/invite/clawd) 中提问或发起 [GitHub 讨论](https://github.com/nova-engine/nova-engine/discussions)。
