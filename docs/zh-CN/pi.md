...
本文档描述了 Nova Engine 如何与 [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) 及其相关包（`pi-ai`、`pi-agent-core`、`pi-tui`）集成以实现其 AI 智能体能力。

## 概述

Nova Engine 使用 pi SDK 将 AI 编码智能体嵌入到其消息 Gateway 网关架构中。Nova Engine 不是将 pi 作为子进程生成或使用 RPC 模式，而是通过 `createAgentSession()` 直接导入并实例化 pi 的 `AgentSession`。这种嵌入式方法提供了：
...
| `pi-tui`          | 终端 UI 组件（用于 Nova Engine 的本地 TUI 模式）                                              |
...
2. **自定义替换**：Nova Engine 将 bash 替换为 `exec`/`process`，为沙箱自定义 read/edit/write
3. **Nova Engine 工具**：消息、浏览器、画布、会话、定时任务、Gateway 网关等
...
这确保 Nova Engine 的策略过滤、沙箱集成和扩展工具集在各提供商之间保持一致。

## 系统提示构建

系统提示在 `buildAgentSystemPrompt()`（`system-prompt.ts`）中构建。它组装一个完整的提示，包含工具、工具调用风格、安全护栏、Nova Engine CLI 参考、Skills、文档、工作区、沙箱、消息、回复标签、语音、静默回复、心跳、运行时元数据等部分，以及启用时的记忆和反应，还有可选的上下文文件和额外系统提示内容。部分内容在子智能体使用的最小提示模式下会被裁剪。
...
Nova Engine 用 `guardSessionManager()` 包装它以确保工具结果安全。
...
Nova Engine 维护一个认证配置文件存储，每个提供商有多个 API 密钥：
...
Nova Engine 加载自定义 pi 扩展以实现特殊行为：
...
Nova Engine 还有一个本地 TUI 模式，直接使用 pi-tui 组件：
...
| 方面     | Pi CLI                  | Nova Engine 嵌入式                                                                                 |
| -------- | ----------------------- | ----------------------------------------------------------------------------------------------- |
| 调用方式 | `pi` 命令 / RPC         | 通过 `createAgentSession()` 的 SDK                                                              |
| 工具     | 默认编码工具            | 自定义 Nova Engine 工具套件                                                                        |
| 系统提示 | AGENTS.md + prompts     | 按渠道/上下文动态生成                                                                           |
| 会话存储 | `~/.pi/agent/sessions/` | `~/.nova-engine/agents/<agentId>/sessions/`（或 `$NOVA_STATE_DIR/agents/<agentId>/sessions/`） |
| 认证     | 单一凭证                | 带轮换的多配置文件                                                                              |
| 扩展     | 从磁盘加载              | 编程方式 + 磁盘路径                                                                             |
