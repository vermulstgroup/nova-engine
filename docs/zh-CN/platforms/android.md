...
- 你可以在 Gateway 网关机器上运行 CLI（`nova-engine`）（或通过 SSH）。
...
- 在 Gateway 网关主机的 `~/.nova-engine/nova-engine.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `nova-engine.internal.`）并发布 `_nova-engine-gw._tcp` 记录。
...
1. 在 Gateway 网关主机上创建 `~/.nova-engine/workspace/canvas/index.html`。

2. 将节点导航到它（局域网）：

```bash
nova-engine nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__nova-engine__/canvas/"}'
```

Tailnet（可选）：如果两个设备都在 Tailscale 上，使用 MagicDNS 名称或 tailnet IP 而不是 `.local`，例如 `http://<gateway-magicdns>:18793/__nova-engine__/canvas/`。

此服务器将实时重载客户端注入 HTML 并在文件更改时重新加载。
A2UI 主机位于 `http://<gateway-host>:18793/__nova-engine__/a2ui/`。
