...
Nova Engine 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`nova-engine gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均支持）：

- 向导（推荐）：`nova-engine onboard --install-daemon`
- 直接安装：`nova-engine gateway install`
- 配置流程：`nova-engine configure` → 选择 **Gateway service**
- 修复/迁移：`nova-engine doctor`（提供安装或修复服务）

服务目标取决于操作系统：

- macOS：LaunchAgent（`bot.molt.gateway` 或 `bot.molt.<profile>`；旧版 `com.nova-engine.*`）
- Linux/WSL2：systemd 用户服务（`nova-engine-gateway[-<profile>].service`）
