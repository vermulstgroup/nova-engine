---
read_when:
  - 在 DigitalOcean 上设置 Nova Engine
  - 寻找便宜的 VPS 托管来运行 Nova Engine
summary: 在 DigitalOcean 上运行 Nova Engine（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Nova Engine

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 Nova Engine Gateway 网关。
...
- `~/.nova-engine/workspace/` — 工作区（SOUL.md、记忆等）
