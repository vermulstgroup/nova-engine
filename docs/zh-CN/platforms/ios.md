...
- 网络路径：
  ...
  - 通过单播 DNS-SD 的 Tailnet（示例域：`nova-engine.internal.`），**或**
  - 手动主机/端口（备选）。
...
Gateway 网关在 `local.` 上广播 `_nova-engine-gw._tcp`。iOS 应用会自动列出这些。

### Tailnet（跨网络）

如果 mDNS 被阻止，使用单播 DNS-SD 区域（选择一个域；示例：`nova-engine.internal.`）和 Tailscale 分割 DNS。
...
```bash
nova-engine nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18793/__nova-engine__/canvas/"}'
```

注意事项：

- Gateway 网关 canvas 主机服务于 `/__nova-engine__/canvas/` 和 `/__nova-engine__/a2ui/`。
...
- 配对提示从未出现：运行 `nova-engine nodes pending` 并手动批准。
