...
summary: Nova Engine macOS 配套应用（菜单栏 + Gateway 网关代理）
title: macOS 应用
x-i18n:
...
# Nova Engine macOS 配套应用（菜单栏 + Gateway 网关代理）

macOS 应用是 Nova Engine 的**菜单栏配套应用**。它拥有权限，在本地管理/附加到 Gateway 网关（launchd 或手动），并作为节点向智能体暴露 macOS 功能。
...
- 根据请求通过 npm/pnpm 安装全局 CLI（`nova-engine`）（不建议使用 bun 作为 Gateway 网关运行时）。

## 本地 vs 远程模式

- **本地**（默认）：如果存在运行中的本地 Gateway 网关，应用附加到它；否则通过 `nova-engine gateway install` 启用 launchd 服务。
...
应用管理一个标记为 `bot.molt.gateway` 的每用户 LaunchAgent（使用 `--profile`/`NOVA_PROFILE` 时为 `bot.molt.<profile>`；旧版 `com.nova-engine.*` 仍会卸载）。
...
如果 LaunchAgent 未安装，从应用中启用它或运行 `nova-engine gateway install`。
...
```
~/.nova-engine/exec-approvals.json
```
...
```bash
open 'nova-engine://agent?message=Hello%20from%20deep%20link'
```
...
```bash
cd apps/macos
swift run nova-engine-mac connect --json
swift run nova-engine-mac discover --timeout 3000 --json
```
...
提示：与 `nova-engine gateway discover --json` 比较，查看 macOS 应用的发现管道（NWBrowser + tailnet DNS-SD 回退）是否与 Node CLI 基于 `dns-sd` 的发现不同。
