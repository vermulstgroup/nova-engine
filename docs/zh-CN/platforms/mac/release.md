---
read_when:
  - 制作或验证 Nova Engine macOS 发布版本
  - 更新 Sparkle appcast 或订阅源资源
summary: Nova Engine macOS 发布清单（Sparkle 订阅源、打包、签名）
title: macOS 发布
x-i18n:
...
- 用于 `xcrun notarytool` 的公证凭据（钥匙串配置文件或 API 密钥），以实现通过 Gatekeeper 安全分发的 DMG/zip。
  - 我们使用名为 `nova-engine-notary` 的钥匙串配置文件，由 shell 配置文件中的 App Store Connect API 密钥环境变量创建：
...
ditto -c -k --sequesterRsrc --keepParent dist/Nova Engine.app dist/Nova Engine-2026.1.27-beta.1.zip

# 可选：同时构建适合用户使用的样式化 DMG（拖拽到 /Applications）
scripts/create-dmg.sh dist/Nova Engine.app dist/Nova Engine-2026.1.27-beta.1.dmg
...
# 可选：随发布一起提供 dSYM
ditto -c -k --keepParent apps/macos/.build/release/Nova Engine.app.dSYM dist/Nova Engine-2026.1.27-beta.1.dSYM.zip
```
...
```bash
SPARKLE_PRIVATE_KEY_FILE=/path/to/ed25519-private-key scripts/make_appcast.sh dist/Nova Engine-2026.1.27-beta.1.zip https://raw.githubusercontent.com/nova-engine/nova-engine/main/appcast.xml
```

从 `CHANGELOG.md`（通过 [`scripts/changelog-to-html.sh`](https://github.com/nova-engine/nova-engine/blob/main/scripts/changelog-to-html.sh)）生成 HTML 发布说明，并将其嵌入 appcast 条目。
发布时，将更新后的 `appcast.xml` 与发布资源（zip + dSYM）一起提交。

## 发布与验证

- 将 `Nova Engine-2026.1.27-beta.1.zip`（和 `Nova Engine-2026.1.27-beta.1.dSYM.zip`）上传到标签 `v2026.1.27-beta.1` 对应的 GitHub 发布。
- 确保原始 appcast URL 与内置的订阅源匹配：`https://raw.githubusercontent.com/nova-engine/nova-engine/main/appcast.xml`。
- 完整性检查：
  - `curl -I https://raw.githubusercontent.com/nova-engine/nova-engine/main/appcast.xml` 返回 200。
