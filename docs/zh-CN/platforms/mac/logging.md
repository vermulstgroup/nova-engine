...
read_when:
  ...
  - 调试语音唤醒/会话生命周期问题
summary: Nova Engine 日志：滚动诊断文件日志 + 统一日志隐私标志
title: macOS 日志
x-i18n:
...
Nova Engine 通过 swift-log（默认使用统一日志）路由 macOS 应用日志，并且在需要持久化捕获时可以将本地轮转文件日志写入磁盘。

- 详细级别：**Debug 面板 → Logs → App logging → Verbosity**
- 启用：**Debug 面板 → Logs → App logging → "Write rolling diagnostics log (JSONL)"**
- 位置：`~/Library/Logs/Nova Engine/diagnostics.jsonl`（自动轮转；旧文件以 `.1`、`.2`、… 为后缀）
