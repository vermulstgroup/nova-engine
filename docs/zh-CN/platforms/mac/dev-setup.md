...
read_when:
  - 设置 macOS 开发环境
summary: 为在 Nova Engine macOS 应用上工作的开发者提供的设置指南
title: macOS 开发设置
x-i18n:
...
本指南涵盖从源代码构建和运行 Nova Engine macOS 应用程序的必要步骤。
...
要构建 macOS 应用并将其打包到 `dist/Nova Engine.app`，运行：
...
https://github.com/nova-engine/nova-engine/blob/main/apps/macos/README.md

> **注意**：Ad-hoc 签名的应用可能会触发安全提示。如果应用立即崩溃并显示"Abort trap 6"，请参阅[故障排除](#troubleshooting)部分。

## 3. 安装 CLI

macOS 应用期望全局安装 `nova-engine` CLI 来管理后台任务。
...
1. 重置 TCC 权限：
   ...
   ```
2. 如果这不起作用，在 [`scripts/package-mac-app.sh`](https://github.com/nova-engine/nova-engine/blob/main/scripts/package-mac-app.sh) 中临时更改 `BUNDLE_ID` 以强制 macOS 从"全新状态"开始。
