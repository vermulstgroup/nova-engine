...
Gateway 网关；如果无法访问，它会通过外部 `nova-engine` CLI（无嵌入式运行时）启用 launchd
...
- 应用安装标记为 `bot.molt.gateway` 的按用户 LaunchAgent
  （使用 `--profile`/`NOVA_PROFILE` 时为 `bot.molt.<profile>`；支持旧版 `com.nova-engine.*`）。
...
- 写入 `~/.nova-engine/disable-launchagent`。
...
`--attach-only`（或 `--no-launchd`）启动它。这会设置 `~/.nova-engine/disable-launchagent`，
