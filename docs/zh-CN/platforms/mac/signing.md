...
- 将构建元数据注入 Info.plist：`NovaEngineBuildTimestamp`（UTC）和 `NovaEngineGitCommit`（短哈希），以便"关于"面板可以显示构建信息、git 信息和调试/发布渠道。
...
- `NovaEngineBuildTimestamp`：打包时的 ISO8601 UTC 时间
- `NovaEngineGitCommit`：短 git 哈希（不可用时为 `unknown`）
