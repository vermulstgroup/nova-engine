...
Nova Engine.app 不再捆绑 Node/Bun 或 Gateway 网关运行时。macOS 应用期望有一个**外部**的 `nova-engine` CLI 安装，不会将 Gateway 网关作为子进程启动，而是管理一个每用户的 launchd 服务来保持 Gateway 网关运行（或者如果已有本地 Gateway 网关正在运行，则连接到现有的）。

## 安装 CLI（本地模式必需）

你需要在 Mac 上安装 Node 22+，然后全局安装 `nova-engine`：
...
- `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.nova-engine.*` 可能仍然存在）
...
- CLI 也可以安装它：`nova-engine gateway install`。

行为：

- "Nova Engine Active"启用/禁用 LaunchAgent。
