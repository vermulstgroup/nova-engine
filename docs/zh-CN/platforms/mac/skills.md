...
macOS 应用通过 Gateway 网关展示 Nova Engine Skills；它不会在本地解析 Skills。

## 数据来源

- `skills.status`（Gateway 网关）返回所有 Skills 以及资格和缺失的要求
  （包括内置 Skills 的允许列表阻止情况）。
- 要求来源于每个 `SKILL.md` 中的 `metadata.nova-engine.requires`。

## 安装操作

- `metadata.nova-engine.install` 定义安装选项（brew/node/go/uv）。
...
- 应用将密钥存储在 `~/.nova-engine/nova-engine.json` 的 `skills.entries.<skillKey>` 下。
