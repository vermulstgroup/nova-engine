...
read_when:
  - 编辑 IPC 合约或菜单栏应用 IPC
summary: Nova Engine 应用的 macOS IPC 架构、Gateway 网关节点传输和 PeekabooBridge
title: macOS IPC
x-i18n:
...
# Nova Engine macOS IPC 架构

**当前模型：** 一个本地 Unix 套接字将**节点主机服务**连接到 **macOS 应用**，用于 exec 审批 + `system.run`。存在一个 `nova-engine-mac` 调试 CLI 用于发现/连接检查；智能体操作仍通过 Gateway 网关 WebSocket 和 `node.invoke` 流转。UI 自动化使用 PeekabooBridge。
...
- 主机优先顺序（客户端侧）：Peekaboo.app → Claude.app → Nova Engine.app → 本地执行。
