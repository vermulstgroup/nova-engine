...
read_when:
  - 向新用户介绍 Nova Engine
summary: Nova Engine 是一个多渠道 AI 智能体 Gateway 网关，可在任何操作系统上运行。
title: Nova Engine
x-i18n:
...
<Columns>
  <Card title="入门指南" href="/start/getting-started" icon="rocket">
    安装 Nova Engine 并在几分钟内启动 Gateway 网关。
  </Card>
  <Card title="运行向导" href="/start/wizard" icon="sparkles">
    通过 `nova-engine onboard` 和配对流程进行引导式设置。
  </Card>
...
Nova Engine 通过单个 Gateway 网关进程将聊天应用连接到 Pi 等编程智能体。它为 Nova Engine 助手提供支持，并支持本地或远程部署。
...
配置文件位于 `~/.nova-engine/nova-engine.json`。

- 如果你**不做任何修改**，Nova Engine 将使用内置的 Pi 二进制文件以 RPC 模式运行，并按发送者创建独立会话。
