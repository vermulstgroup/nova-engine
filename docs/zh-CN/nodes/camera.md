...
- **设置 → 通用 → 允许相机**（`nova-engine.cameraEnabled`）
  - 默认：**关闭**
  - 关闭时：相机请求返回"用户已禁用相机"。

### CLI 辅助工具（节点调用）

使用主 `nova-engine` CLI 在 macOS 节点上调用相机命令。
...
- `nova-engine nodes camera snap` 默认 `maxWidth=1600`，除非被覆盖。
