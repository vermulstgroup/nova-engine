...
- 默认滚动日志文件位于 `/tmp/nova-engine/` 下（每天一个文件）：`nova-engine-YYYY-MM-DD.log`
  - 日期使用 Gateway 网关主机的本地时区。
- 日志文件路径和级别可以通过 `~/.nova-engine/nova-engine.json` 配置：
  - `logging.file`
  - `logging.level`
...
`nova-engine gateway` 支持每个 Gateway 网关的样式切换：
