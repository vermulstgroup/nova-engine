...
Nova Engine 的 Gateway 网关可以提供兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
底层实现中，请求作为正常的 Gateway 网关智能体运行执行（与 `nova-engine agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- `model: "nova-engine:<agentId>"`（示例：`"nova-engine:main"`、`"nova-engine:beta"`）
- `model: "agent:<agentId>"`（别名）

或通过头指定特定的 Nova Engine 智能体：

- `x-nova-engine-agent-id: <agentId>`（默认：`main`）

高级：

- `x-nova-engine-session-key: <sessionKey>` 完全控制会话路由。
