...
status: active
summary: Nova Engine 沙箱隔离的工作原理：模式、作用域、工作区访问和镜像
title: 沙箱隔离
x-i18n:
...
Nova Engine 可以**在 Docker 容器内运行工具**以减少影响范围。
...
- `"none"`（默认）：工具看到 `~/.nova-engine/sandboxes` 下的沙箱工作区。
- `"ro"`：以只读方式在 `/agent` 挂载智能体工作区（禁用 `write`/`edit`/`apply_patch`）。
- `"rw"`：以读写方式在 `/workspace` 挂载智能体工作区。

入站媒体被复制到活动沙箱工作区（`media/inbound/*`）。
Skills 注意事项：`read` 工具以沙箱为根。使用 `workspaceAccess: "none"` 时，Nova Engine 将符合条件的 Skills 镜像到沙箱工作区（`.../skills`）以便可以读取。使用 `"rw"` 时，工作区 Skills 可从 `/workspace/skills` 读取。
...
默认镜像：`nova-engine-sandbox:bookworm-slim`
...
- 使用 `nova-engine sandbox explain` 检查生效的沙箱模式、工具策略和修复配置键。
