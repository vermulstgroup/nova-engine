...
Nova Engine 的 Gateway 网关暴露了一个简单的 HTTP 端点用于直接调用单个工具。它始终启用，但受 Gateway 网关认证和工具策略限制。
...
- `x-nova-engine-message-channel: <channel>`（示例：`slack`、`telegram`）
- `x-nova-engine-account-id: <accountId>`（当存在多个账户时）
