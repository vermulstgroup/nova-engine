...
Nova Engine 有两个表面上看起来相似的不同问题：

1. **操作员远程控制**：macOS 菜单栏应用控制运行在其他地方的 Gateway 网关。
2. **节点配对**：iOS/Android（以及未来的节点）发现 Gateway 网关并安全配对。

设计目标是将所有网络发现/广播保留在 **Node Gateway 网关**（`nova-engine gateway`）中，并让客户端（mac 应用、iOS）作为消费者。
...
- 服务类型：
  - `_nova-engine-gw._tcp`（Gateway 网关传输信标）
- TXT 键（非机密）：
  - `role=gateway`
  ...
  - `canvasPort=18793`（默认画布主机端口；服务于 `/__nova-engine__/canvas/`）
  - `cliPath=<path>`（可选；可运行的 `nova-engine` 入口点或二进制文件的绝对路径）
  - `tailnetDns=<magicdns>`（可选提示；当 Tailscale 可用时自动检测）

禁用/覆盖：

- `NOVA_DISABLE_BONJOUR=1` 禁用广播。
- `~/.nova-engine/nova-engine.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `NOVA_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（默认为 22）。
- `NOVA_TAILNET_DNS` 发布 `tailnetDns` 提示（MagicDNS）。
- `NOVA_CLI_PATH` 覆盖广播的 CLI 路径。
