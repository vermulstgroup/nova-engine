...
Nova Engine 的 Gateway 网关可以提供一个小型的 OpenAI 兼容 Chat Completions 端点。
...
底层实现中，请求作为普通的 Gateway 网关智能体运行执行（与 `nova-engine agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- `model: "nova-engine:<agentId>"`（例如：`"nova-engine:main"`、`"nova-engine:beta"`）
