...
summary: 在同一主机上运行多个 Nova Engine Gateway 网关（隔离、端口和配置文件）
title: 多 Gateway 网关
x-i18n:
...
- `NOVA_CONFIG_PATH` — 每个实例的配置文件
- `NOVA_STATE_DIR` — 每个实例的会话、凭证、缓存
...
配置文件自动限定 `NOVA_STATE_DIR` + `NOVA_CONFIG_PATH` 范围并为服务名称添加后缀。
