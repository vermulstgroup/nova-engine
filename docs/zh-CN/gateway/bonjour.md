...
Nova Engine 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_nova-engine-gw._tcp` 的 DNS‑SD 记录
   （示例：`nova-engine.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端（包括 iOS）解析。

Nova Engine 支持任何发现域名；`nova-engine.internal.` 只是一个示例。
...
- 从 `~/.nova-engine/dns/<domain>.db` 提供你选择的域名服务（示例：`nova-engine.internal.`）
...
你的发现域名中浏览 `_nova-engine-gw._tcp`，无需多播。
...
- 在 `~/.nova-engine/nova-engine.json` 中设置 `gateway.bind: "tailnet"`。
- 重启 Gateway 网关（或重启 macOS 菜单栏应用）。

## 什么在广播

只有 Gateway 网关广播 `_nova-engine-gw._tcp`。

## 服务类型

- `_nova-engine-gw._tcp` — Gateway 网关传输信标（被 macOS/iOS/Android 节点使用）。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `nova-engine` 入口点的绝对路径）
...
iOS 节点使用 `NWBrowser` 来发现 `_nova-engine-gw._tcp`。
...
- `NOVA_DISABLE_BONJOUR=1` 禁用广播（旧版：`NOVA_DISABLE_BONJOUR`）。
- `~/.nova-engine/nova-engine.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `NOVA_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`NOVA_SSH_PORT`）。
- `NOVA_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`NOVA_TAILNET_DNS`）。
- `NOVA_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`NOVA_CLI_PATH`）。
