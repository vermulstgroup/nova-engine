...
read_when:
  - 添加或修改配置字段时
summary: ~/.nova-engine/nova-engine.json 的所有配置选项及示例
title: 配置
x-i18n:
...
Nova Engine 从 `~/.nova-engine/nova-engine.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，Nova Engine 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.nova-engine/workspace`）。通常只在以下情况需要配置：
...
Nova Engine 只接受完全匹配 schema 的配置。
未知键、类型错误或无效值会导致 Gateway 网关 **拒绝启动**以确保安全。

验证失败时：

- Gateway 网关不会启动。
- 只允许诊断命令（例如：`nova-engine doctor`、`nova-engine logs`、`nova-engine health`、`nova-engine status`、`nova-engine service`、`nova-engine help`）。
- 运行 `nova-engine doctor` 查看具体问题。
- 运行 `nova-engine doctor --fix`（或 `--yes`）应用迁移/修复。
...
请使用 `config.patch` 或 `nova-engine config set`。请备份 `~/.nova-engine/nova-engine.json`。
...
```bash
nova-engine gateway call config.get --params '{}' # capture payload.hash
nova-engine gateway call config.apply --params '{
...
```bash
nova-engine gateway call config.get --params '{}' # capture payload.hash
nova-engine gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.nova-engine/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@nova-engine", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.nova-engine/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.nova-engine/workspace" }],
}
```
...
{ "$include": "/etc/nova-engine/base.json5" } // 绝对路径
...
```json5
// ~/.nova-engine/clients/mueller/agents.json5
[
...
```json5
// ~/.nova-engine/clients/mueller/broadcast.json5
{
...
Nova Engine 从父进程（shell、launchd/systemd、CI 等）读取环境变量。

此外，它还会加载：

- 当前工作目录中的 `.env`（如果存在）
- `~/.nova-engine/.env`（即 `$NOVA_STATE_DIR/.env`）作为全局回退 `.env`
...
可选便利功能：如果启用且预期键均未设置，Nova Engine 会运行你的登录 shell 并仅导入缺失的预期键（不会覆盖）。
...
Nova Engine 在以下位置存储**每个智能体的**认证配置文件（OAuth + API 密钥）：

- `<agentDir>/auth-profiles.json`（默认：`~/.nova-engine/agents/<agentId>/agent/auth-profiles.json`）

另请参阅：[/concepts/oauth](/concepts/oauth)

旧版 OAuth 导入：

- `~/.nova-engine/credentials/oauth.json`（或 `$NOVA_STATE_DIR/credentials/oauth.json`）
...
- `~/.nova-engine/agent/*`（由 `nova-engine doctor` 迁移到 `~/.nova-engine/agents/<defaultAgentId>/agent/*`）
...
首次使用时，Nova Engine 会将 `oauth.json` 条目导入到 `auth-profiles.json` 中。
...
如果设置了，Nova Engine 会推导默认值（仅在你未明确设置时）：
...
- 默认日志文件：`/tmp/nova-engine/nova-engine-YYYY-MM-DD.log`
- 如需稳定路径，将 `logging.file` 设为 `/tmp/nova-engine/nova-engine.log`。
- 控制台输出可通过以下方式单独调整：
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/nova-engine/nova-engine.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // 可选覆盖。默认：~/.nova-engine/credentials/whatsapp/biz
...
- 旧版单账号 Baileys 认证目录由 `nova-engine doctor` 迁移到 `whatsapp/default`。
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@nova-engine", "nova-engine"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          // 仅这些文本模式会触发响应
          mentionPatterns: ["reisponde", "@nova-engine"],
        },
      },
    ],
  },
...
- `agents.list[]`：每智能体覆盖。
  ...
  - `name`：智能体的显示名称。
  - `workspace`：默认 `~/.nova-engine/workspace-<agentId>`（对于 `main`，回退到 `agents.defaults.workspace`）。
  - `agentDir`：默认 `~/.nova-engine/agents/<agentId>/agent`。
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.nova-engine/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.nova-engine/workspace-home" },
      { id: "work", workspace: "~/.nova-engine/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` 启用 `/config`（读写 `nova-engine.json`）。
...
Nova Engine 仅在存在 `channels.telegram` 配置段时启动 Telegram。机器人 token 从 `channels.telegram.botToken`（或 `channels.telegram.tokenFile`）解析，`TELEGRAM_BOT_TOKEN` 作为默认账号的回退。
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["nova-engine-dm"], // 可选群组私聊白名单
      },
      guilds: {
        "123456789012345678": {
          // 服务器 id（推荐）或 slug
          slug: "friends-of-nova-engine",
...
Nova Engine 仅在存在 `channels.discord` 配置段时启动 Discord。token 从 `channels.discord.token` 解析，`DISCORD_BOT_TOKEN` 作为默认账号的回退（除非 `channels.discord.enabled` 为 `false`）。在为 cron/CLI 命令指定投递目标时，使用 `user:<id>`（私聊）或 `channel:<id>`（服务器频道）；裸数字 ID 有歧义会被拒绝。
...
Nova Engine 在提供商启用且两个 token 都已设置时启动 Slack（通过配置或 `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`）。在为 cron/CLI 命令指定投递目标时使用 `user:<id>`（私聊）或 `channel:<id>`。
...
请先安装：`nova-engine plugins install @nova-engine/mattermost`（或从 git checkout 使用 `./extensions/mattermost`）。
...
Nova Engine 在账号已配置（机器人 token + 基础 URL）且已启用时启动 Mattermost。token + 基础 URL 从 `channels.mattermost.botToken` + `channels.mattermost.baseUrl` 或默认账号的 `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` 解析（除非 `channels.mattermost.enabled` 为 `false`）。
...
Nova Engine 会生成 `imsg rpc`（通过 stdio 的 JSON-RPC）。无需守护进程或端口。
...
在系统提示的 Runtime 行中显示的可选仓库根目录。如果未设置，Nova Engine 会从工作区（和当前工作目录）向上查找 `.git` 目录进行检测。路径必须存在才能使用。

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/nova-engine" } },
}
```
...
当文件超过此限制时，Nova Engine 会记录警告并注入带标记的头尾截断内容。
...
设置用户时区用于**系统提示上下文**（不用于消息信封中的时间戳）。如果未设置，Nova Engine 在运行时使用主机时区。
...
如果未设置 `messages.responsePrefix`，默认不应用前缀。WhatsApp 自聊天回复是例外：它们在设置时默认为 `[{identity.name}]`，否则为 `[nova-engine]`，以保持同一手机上的对话可读性。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[nova-engine]"`，否则为 `""`（无前缀）。使用 `"[nova-engine]"` 时，如果路由的智能体设置了 `identity.name`，Nova Engine 会改用 `[{identity.name}]`。
...
为出站回复启用文字转语音。开启后，Nova Engine 使用 ElevenLabs 或 OpenAI 生成音频并附加到回复中。Telegram 使用 Opus 语音消息；其他渠道发送 MP3 音频。
...
Nova Engine 还内置了一些别名快捷方式。默认值仅在模型已存在于 `agents.defaults.models` 中时才应用：
...
如果省略提供商，Nova Engine 目前假定 `anthropic` 作为临时弃用回退。
...
- `tools.web.search.apiKey`（推荐：通过 `nova-engine configure --section web` 设置，或使用 `BRAVE_API_KEY` 环境变量）
...
- `group:nodes`：`nodes`
- `group:nova-engine`：所有内置 Nova Engine 工具（不包含提供商插件）
...
- 智能体工作区访问：`workspaceAccess: "none"`（默认）
  - `"none"`：在 `~/.nova-engine/sandboxes` 下使用每范围的沙箱工作区
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "nova-engine-sandbox-browser:bookworm-slim",
          containerPrefix: "nova-engine-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Nova Engine 使用 **pi-coding-agent** 模型目录。你可以通过编写
`~/.nova-engine/agents/<agentId>/agent/models.json` 或在 Nova Engine 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
按提供商的概述 + 示例：[/concepts/model-providers](/concepts/model-providers)。

当存在 `models.providers` 时，Nova Engine 在启动时将 `models.json` 写入/合并到
`~/.nova-engine/agents/<agentId>/agent/`：
...
OpenCode Zen 是一个具有每模型端点的多模型网关。Nova Engine 使用
...
- 快捷方式：`nova-engine onboard --auth-choice opencode-zen`。
...
快捷方式：`nova-engine onboard --auth-choice zai-api-key`。
...
- 在环境中设置 `MOONSHOT_API_KEY` 或使用 `nova-engine onboard --auth-choice moonshot-api-key`。
...
- 在环境中设置 `KIMI_API_KEY` 或使用 `nova-engine onboard --auth-choice kimi-code-api-key`。
...
- 设置 `SYNTHETIC_API_KEY` 或使用 `nova-engine onboard --auth-choice synthetic-api-key`。
...
- 设置 `MINIMAX_API_KEY` 环境变量或使用 `nova-engine onboard --auth-choice minimax-api`。
...
- 如果你希望 `models.json` 存储在其他位置，请使用 `NOVA_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.nova-engine/agents/main/agent`）。
...
```json5
{
  session: {
    ...
    // 默认已按智能体存储在 ~/.nova-engine/agents/<agentId>/sessions/sessions.json
...
- `resetByType`：`dm`、`group` 和 `thread` 的每会话覆盖。
  - 如果你只设置了旧版 `session.idleMinutes` 而没有任何 `reset`/`resetByType`，Nova Engine 保持仅空闲模式以向后兼容。
...
控制内置白名单、安装偏好、额外 Skills 文件夹和每 Skills 覆盖。适用于**内置**Skills 和 `~/.nova-engine/skills`（工作区 Skills 在名称冲突时仍然优先）。
...
控制插件发现、允许/拒绝和每插件配置。插件从 `~/.nova-engine/extensions`、`<workspace>/.nova-engine/extensions` 以及任何 `plugins.load.paths` 条目加载。**配置更改需要重启 Gateway 网关。**
...
### `browser`（Nova Engine 管理的浏览器）

Nova Engine 可以为 Nova Engine 启动一个**专用、隔离的** Chrome/Brave/Edge/Chromium 实例并暴露一个小型 local loopback 控制服务。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（Nova Engine.app 菜单栏或 `nova-engine gateway`）启动。
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/nova-engine" }
...
- 示例：`"/ui"`、`"/nova-engine"`、`"/apps/nova-engine"`。
- 默认：根路径（`/`）（不变）。
...
- 当连接来自这些 IP 之一时，Nova Engine 使用 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP，用于本地配对检查和 HTTP 认证/本地检查。
- 仅列出你完全控制的代理，并确保它们**覆盖**传入的 `x-forwarded-for`。

说明：

- `nova-engine gateway` 拒绝启动，除非 `gateway.mode` 设为 `local`（或你传递了覆盖标志）。
...
- `gateway.auth.allowTailscale` 允许 Tailscale Serve 身份头部
  ...
  `x-forwarded-proto` 和 `x-forwarded-host` 时满足认证。Nova Engine 在接受之前
...
- Nova Engine.app 监视 `~/.nova-engine/nova-engine.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.nova-engine/nova-engine.json`（或 `NOVA_CONFIG_PATH`）并自动应用更改。
...
- `~/.nova-engine/nova-engine.json`（或 `NOVA_CONFIG_PATH`）
...
- `nova-engine --dev …` → 使用 `~/.nova-engine-dev` + 端口从基础 `19001` 偏移
- `nova-engine --profile <name> …` → 使用 `~/.nova-engine-<name>`（端口通过配置/环境变量/标志）
...
```bash
NOVA_CONFIG_PATH=~/.nova-engine/a.json \
NOVA_STATE_DIR=~/.nova-engine-a \
nova-engine gateway --port 19001
```
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.nova-engine/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Gmail 辅助配置（由 `nova-engine webhooks gmail setup` / `run` 使用）：
...
注意：当 `tailscale.mode` 开启时，Nova Engine 将 `serve.path` 默认为 `/`，以便
...
默认根目录：`~/.nova-engine/workspace/canvas`
默认端口：`18793`（选择此端口以避免 Nova Engine 浏览器 CDP 端口 `18792`）
...
- 监视目录并通过 `/__nova-engine__/ws` 的 WebSocket 端点广播重载
- 目录为空时自动创建起始 `index.html`（以便你立即看到内容）
- 同时在 `/__nova-engine__/a2ui/` 提供 A2UI，并作为 `canvasHostUrl` 通告给节点
...
{
  canvasHost: {
    root: "~/.nova-engine/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // 省略时使用 ~/.nova-engine/bridge/tls/bridge-{cert,key}.pem。
      // certPath: "~/.nova-engine/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.nova-engine/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
控制 LAN mDNS 发现广播（`_nova-engine-gw._tcp`）。

- `minimal`（默认）：从 TXT 记录中省略 `cliPath` + `sshPort`
- `full`：在 TXT 记录中包含 `cliPath` + `sshPort`
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `nova-engine`（通告 `nova-engine.local`）。通过 `NOVA_MDNS_HOSTNAME` 覆盖。
...
启用后，Gateway 网关在 `~/.nova-engine/dns/` 下使用配置的发现域（示例：`nova-engine.internal.`）为 `_nova-engine-gw._tcp` 写入单播 DNS-SD 区域。
