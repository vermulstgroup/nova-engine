...
- 在心跳运行期间，当 `HEARTBEAT_OK` 出现在回复的**开头或结尾**时，Nova Engine 将其视为确认。该标记会被移除，如果剩余内容 **≤ `ackMaxChars`**（默认：300），则回复被丢弃。
...
- `session`：心跳运行的可选会话键。
  - `main`（默认）：智能体主会话。
  - 显式会话键（从 `nova-engine sessions --json` 或 [sessions CLI](/cli/sessions) 复制）。
...
如果**所有三个**都为 false，Nova Engine 会完全跳过心跳运行（不调用模型）。
...
如果 `HEARTBEAT.md` 存在但实际上是空的（只有空行和 markdown 标题如 `# Heading`），Nova Engine 会跳过心跳运行以节省 API 调用。如果文件不存在，心跳仍会运行，由模型决定做什么。
