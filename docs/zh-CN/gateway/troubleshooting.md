...
read_when:
  - 调查运行时问题或故障
summary: Nova Engine 常见故障的快速故障排除指南
title: 故障排除
x-i18n:
...
| 命令                               | 它告诉你什么                                                                          | 何时使用                              |
| ---------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------- |
| `nova-engine status`                  | 本地摘要：操作系统 + 更新、Gateway 网关可达性/模式、服务、智能体/会话、提供商配置状态 | 首次检查，快速概览                    |
| `nova-engine status --all`            | 完整本地诊断（只读、可粘贴、相对安全）包括日志尾部                                    | 当你需要分享调试报告时                |
| `nova-engine status --deep`           | 运行 Gateway 网关健康检查（包括提供商探测；需要可达的 Gateway 网关）                  | 当"已配置"不意味着"正常工作"时        |
| `nova-engine gateway probe`           | Gateway 网关发现 + 可达性（本地 + 远程目标）                                          | 当你怀疑正在探测错误的 Gateway 网关时 |
| `nova-engine channels status --probe` | 向运行中的 Gateway 网关查询渠道状态（并可选探测）                                     | 当 Gateway 网关可达但渠道异常时       |
| `nova-engine gateway status`          | 监管程序状态（launchd/systemd/schtasks）、运行时 PID/退出、最后的 Gateway 网关错误    | 当服务"看起来已加载"但没有运行时      |
| `nova-engine logs --follow`           | 实时日志（运行时问题的最佳信号）                                                      | 当你需要实际的故障原因时              |

**分享输出：** 优先使用 `nova-engine status --all`（它会隐藏令牌）。如果你粘贴 `nova-engine status`，考虑先设置 `NOVA_SHOW_SECRETS=0`（令牌预览）。
...
- 优先：`nova-engine logs --follow`
- 文件日志（始终）：`/tmp/nova-engine/nova-engine-YYYY-MM-DD.log`（或你配置的 `logging.file`）
- macOS LaunchAgent（如果已安装）：`$NOVA_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`
- Linux systemd（如果已安装）：`journalctl --user -u nova-engine-gateway[-<profile>].service -n 200 --no-pager`
...
- 或直接设置：
  ```bash
  nova-engine config set gateway.mode local
  ```
...
- 设置远程 URL 并保持 `gateway.mode=remote`：
  ```bash
  nova-engine config set gateway.mode remote
  nova-engine config set gateway.remote.url "wss://gateway.example.com"
  ```

**仅临时/开发使用：** 传递 `--allow-unconfigured` 以在没有
`gateway.mode=local` 的情况下启动 Gateway 网关。

**还没有配置文件？** 运行 `nova-engine setup` 创建初始配置，然后重新运行
Gateway 网关。
...
变量如 `DISPLAY` 应该放在 `~/.nova-engine/.env` 中（由 Gateway 网关早期加载）。
...
服务是用 Bun 或版本管理的 Node 路径安装的，运行 `nova-engine doctor`
迁移到系统 Node 安装。
...
- 或将密钥烘焙到你的自定义沙箱镜像中
- 然后运行 `nova-engine sandbox recreate --agent <id>`（或 `--all`）
...
- `gateway.mode` 必须为 `local` 才能运行 `nova-engine gateway` 和服务。
- 如果你设置了 `gateway.mode=remote`，**CLI 默认**使用远程 URL。服务可能仍在本地运行，但你的 CLI 可能在探测错误的位置。使用 `nova-engine gateway status` 查看服务解析的端口 + 探测目标（或传递 `--url`）。
- `nova-engine gateway status` 和 `nova-engine doctor` 在服务看起来正在运行但端口关闭时会显示日志中的**最后 Gateway 网关错误**。
- 非本地回环绑定（`lan`/`tailnet`/`custom`，或本地回环不可用时的 `auto`）需要认证：
  `gateway.auth.token`（或 `NOVA_GATEWAY_TOKEN`）。
- `gateway.remote.token` 仅用于远程 CLI 调用；它**不**启用本地认证。
- `gateway.token` 被忽略；使用 `gateway.auth.token`。

**如果 `nova-engine gateway status` 显示配置不匹配**

- `Config (cli): ...` 和 `Config (service): ...` 通常应该匹配。
- 如果不匹配，你几乎肯定是在编辑一个配置而服务运行的是另一个。
- 修复：从你希望服务使用的相同 `--profile` / `NOVA_STATE_DIR` 重新运行 `nova-engine gateway install --force`。

**如果 `nova-engine gateway status` 报告服务配置问题**

- 监管程序配置（launchd/systemd/schtasks）缺少当前默认值。
- 修复：运行 `nova-engine doctor` 更新它（或 `nova-engine gateway install --force` 完全重写）。
...
- 修复：设置 `gateway.auth.mode` + `gateway.auth.token`（或导出 `NOVA_GATEWAY_TOKEN`）并重启服务。

**如果 `nova-engine gateway status` 显示 `bind=tailnet` 但未找到 tailnet 接口**
...
如果你从旧版本升级，你的磁盘上可能仍有 `~/nova-engine`。
...
症状：`pwd` 或文件工具显示 `~/.nova-engine/sandboxes/...` 即使你
期望的是主机工作区。
...
Nova Engine 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。

**修复：**

- 为提供商选择**最新**模型并更新你的配置或模型别名。
- 如果你不确定哪些模型可用，运行 `nova-engine models list` 或
  `nova-engine models scan` 并选择一个支持的模型。
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${NOVA_CONFIG_PATH:-$HOME/.nova-engine/nova-engine.json}"
```
...
# 或者如果你想快速过滤：
tail -f "$(ls -t /tmp/nova-engine/nova-engine-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
nova-engine logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.nova-engine/agents/<agentId>/sessions/
```
...
# 查看最近的连接事件
nova-engine logs --limit 200 | grep "connection\\|disconnect\\|logout"
```

**修复：** 通常在 Gateway 网关运行后会自动重连。如果卡住，重启 Gateway 网关进程（无论你如何监管它），或使用详细输出手动运行：

```bash
nova-engine gateway --verbose
```

如果你已登出/取消关联：

```bash
nova-engine channels logout
trash "${NOVA_STATE_DIR:-$HOME/.nova-engine}/credentials" # 如果 logout 无法完全清除所有内容
nova-engine channels login --verbose       # 重新扫描二维码
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/nova-engine/nova-engine-*.log | head -1)" | tail -20
```

### 高内存使用

Nova Engine 在内存中保留对话历史。
...
当配置包含未知键、格式错误的值或无效类型时，Nova Engine 现在拒绝启动。
这是为了安全而故意设计的。
...
- `nova-engine doctor` 报告每个无效条目。
- `nova-engine doctor --fix` 应用迁移/修复并重写配置。
- 诊断命令如 `nova-engine logs`、`nova-engine health`、`nova-engine status`、`nova-engine gateway status` 和 `nova-engine gateway probe` 即使配置无效也能运行。

### "All models failed" — 我应该首先检查什么？

- **凭证**存在于正在尝试的提供商（认证配置文件 + 环境变量）。
- **模型路由**：确认 `agents.defaults.model.primary` 和回退是你可以访问的模型。
- `/tmp/nova-engine/…` 中的 **Gateway 网关日志**以获取确切的提供商错误。
- **模型状态**：使用 `/model status`（聊天）或 `nova-engine models status`（CLI）。
...
```bash
nova-engine channels login
```

### `main` 上的构建错误 — 标准修复路径是什么？

1. `git pull origin main && pnpm install`
2. `nova-engine doctor`
...
5. 运行 `nova-engine channels status --probe` 获取审计提示。
...
端点接受 JSON Schema 的严格子集。Nova Engine 在当前 `main` 中清理/规范化工具
...
如果重置不起作用，在 [`scripts/package-mac-app.sh`](https://github.com/nova-engine/nova-engine/blob/main/scripts/package-mac-app.sh) 中更改 `BUNDLE_ID`（例如，添加 `.test` 后缀）并重新构建。这会强制 macOS 将其视为新应用。
...
# 或：launchctl bootout gui/$UID/bot.molt.gateway（用 bot.molt.<profile> 替换；旧版 com.nova-engine.* 仍然有效）
```
...
**修复 3：检查 CLI 安装**
确保全局 `nova-engine` CLI 已安装且与应用版本匹配：

```bash
nova-engine --version
npm install -g nova-engine@<version>
```
...
```bash
# 在配置中打开跟踪日志：
#   ${NOVA_CONFIG_PATH:-$HOME/.nova-engine/nova-engine.json} -> { logging: { level: "trace" } }
#
# 然后运行详细命令将调试输出镜像到标准输出：
nova-engine gateway --verbose
nova-engine channels login --verbose
```
...
| 日志                             | 位置                                                                                                                                                                                                                                                                                                                      |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway 网关文件日志（结构化）   | `/tmp/nova-engine/nova-engine-YYYY-MM-DD.log`（或 `logging.file`）                                                                                                                                                                                                                                                              |
| Gateway 网关服务日志（监管程序） | macOS：`$NOVA_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.nova-engine/logs/...`；配置文件使用 `~/.nova-engine-<profile>/logs/...`）<br />Linux：`journalctl --user -u nova-engine-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "Nova Engine Gateway (<profile>)" /V /FO LIST` |
| 会话文件                         | `$NOVA_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                          |
| 媒体缓存                         | `$NOVA_STATE_DIR/media/`                                                                                                                                                                                                                                                                                              |
| 凭证                             | `$NOVA_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                        |
...
nova-engine logs --follow
# 如果 RPC 宕机的备用方案
tail -20 /tmp/nova-engine/nova-engine-*.log
```
...
trash "${NOVA_STATE_DIR:-$HOME/.nova-engine}"
nova-engine channels login         # 重新配对 WhatsApp
nova-engine gateway restart           # 或：nova-engine gateway
```
...
1. 首先检查日志：`/tmp/nova-engine/`（默认：`nova-engine-YYYY-MM-DD.log`，或你配置的 `logging.file`）
2. 在 GitHub 上搜索现有问题
3. 提交新问题时包含：
   - Nova Engine 版本
   - 相关日志片段
