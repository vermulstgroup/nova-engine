...
- **询问默认：** `on-miss`。
- **批准存储：** `~/.nova-engine/exec-approvals.json`（JSON，无遗留迁移）。
- **运行器：** 无头系统服务；UI 应用托管 Unix socket 用于批准。
- **节点身份：** 使用现有 `nodeId`。
- **Socket 认证：** Unix socket + token（跨平台）；如需要稍后拆分。
- **节点主机状态：** `~/.nova-engine/node.json`（节点 id + 配对 token）。
...
路径：`~/.nova-engine/exec-approvals.json`
...
- Unix socket 位于 `~/.nova-engine/exec-approvals.sock`（0600）。
