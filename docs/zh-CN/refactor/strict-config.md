...
- 所有插件都需要插件清单（`nova-engine.plugin.json`）。

## 插件 schema 强制执行

- 每个插件为其配置提供严格的 JSON Schema（内联在清单中）。
- 插件加载流程：
  1. 解析插件清单 + schema（`nova-engine.plugin.json`）。
  2. 根据 schema 验证配置。
...
其他所有命令必须硬失败并显示："Config invalid. Run `nova-engine doctor --fix`."
