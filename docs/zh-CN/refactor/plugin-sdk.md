...
- 以 `nova-engine/plugin-sdk` 发布（或从核心以 `nova-engine/plugin-sdk` 导出）。
- 使用语义化版本控制，提供明确的稳定性保证。

### 2）插件运行时（执行层，注入式）

范围：所有涉及核心运行时行为的内容。
通过 `Nova EnginePluginApi.runtime` 访问，确保插件永远不会导入 `src/**`。
...
- 引入 `nova-engine/plugin-sdk`。
- 在 `Nova EnginePluginApi` 中添加带有上述接口的 `api.runtime`。
...
- 插件声明所需的运行时版本范围（例如 `nova-engineRuntime: ">=2026.2.0"`）。
