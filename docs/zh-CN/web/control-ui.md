...
- 默认：`http://<host>:18789/`
- 可选前缀：设置 `gateway.controlUi.basePath`（例如 `/nova-engine`）
...
如果页面加载失败，请先启动 Gateway 网关：`nova-engine gateway`。
...
一旦批准，设备会被记住，除非你使用 `nova-engine devices revoke --device <id> --role <role>` 撤销它，否则不需要重新批准。参见 [Devices CLI](/cli/devices) 了解 token 轮换和撤销。
...
- 配置：查看/编辑 `~/.nova-engine/nova-engine.json`（`config.get`、`config.set`）
...
默认情况下，当 `gateway.auth.allowTailscale` 为 `true` 时，Serve 请求可以通过 Tailscale 身份头（`tailscale-user-login`）进行认证。Nova Engine 通过使用 `tailscale whois` 解析 `x-forwarded-for` 地址并与头匹配来验证身份，并且只在请求通过 Tailscale 的 `x-forwarded-*` 头到达 loopback 时接受这些。如果你想即使对于 Serve 流量也要求 token/密码，请设置 `gateway.auth.allowTailscale: false`（或强制 `gateway.auth.mode: "password"`）。

### 绑定到 tailnet + token

```bash
nova-engine gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
如果你通过普通 HTTP 打开仪表板（`http://<lan-ip>` 或 `http://<tailscale-ip>`），浏览器在**非安全上下文**中运行并阻止 WebCrypto。默认情况下，Nova Engine **阻止**没有设备身份的控制 UI 连接。
...
```bash
NOVA_CONTROL_UI_BASE_PATH=/nova-engine/ pnpm ui:build
```
