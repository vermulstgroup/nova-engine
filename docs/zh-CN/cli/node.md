...
summary: "`nova-engine node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
使用 `nova-engine node run` 运行前台节点主机（无服务）。
...
- `nova-engine approvals --node <id|name|ip>`（从 Gateway 网关编辑）
