...
read_when:
  - 添加或修改 CLI 命令或选项
  - 为新命令界面编写文档
summary: Nova Engine `nova-engine` 命令、子命令和选项的 CLI 参考
title: CLI 参考
x-i18n:
...
- `--dev`：将状态隔离到 `~/.nova-engine-dev` 下并调整默认端口。
- `--profile <name>`：将状态隔离到 `~/.nova-engine-<name>` 下。
- `--no-color`：禁用 ANSI 颜色。
- `--update`：`nova-engine update` 的简写（仅限源码安装）。
- `-V`、`--version`、`-v`：打印版本并退出。
...
注意：插件可以添加额外的顶级命令（例如 `nova-engine voicecall`）。

## 安全

- `nova-engine security audit` — 审计配置 + 本地状态中常见的安全隐患。
- `nova-engine security audit --deep` — 尽力进行实时 Gateway 网关探测。
- `nova-engine security audit --fix` — 收紧安全默认值并 chmod 状态/配置。

## 插件

管理扩展及其配置：

- `nova-engine plugins list` — 发现插件（使用 `--json` 获取机器可读输出）。
- `nova-engine plugins info <id>` — 显示插件详情。
- `nova-engine plugins install <path|.tgz|npm-spec>` — 安装插件（或将插件路径添加到 `plugins.load.paths`）。
- `nova-engine plugins enable <id>` / `disable <id>` — 切换 `plugins.entries.<id>.enabled`。
- `nova-engine plugins doctor` — 报告插件加载错误。
...
- `nova-engine memory status` — 显示索引统计。
- `nova-engine memory index` — 重新索引记忆文件。
- `nova-engine memory search "<query>"` — 对记忆进行语义搜索。
...
- `--workspace <dir>`：智能体工作区路径（默认 `~/.nova-engine/workspace`）。
- `--wizard`：运行新手引导向导。
...
非交互式配置辅助工具（get/set/unset）。不带子命令运行 `nova-engine config` 会启动向导。
...
- `channels status`：检查 Gateway 网关可达性和渠道健康状况（`--probe` 运行额外检查；使用 `nova-engine health` 或 `nova-engine status --deep` 进行 Gateway 网关健康探测）。
- 提示：`channels status` 在检测到常见配置错误时会打印带有建议修复的警告（然后指向 `nova-engine doctor`）。
...
当 OAuth/API 凭证可用时，Nova Engine 可以显示提供商用量/配额。

显示位置：

- `/status`（可用时添加简短的提供商用量行）
- `nova-engine status --usage`（打印完整的提供商明细）
- macOS 菜单栏（上下文下的用量部分）
...
- `gateway status` 在检测到旧版或额外的 Gateway 网关服务时也会显示（`--deep` 添加系统级扫描）。配置文件命名的 Nova Engine 服务被视为一等公民，不会被标记为"额外"。
...
`nova-engine models` 是 `models status` 的别名。
...
`node` 运行**无头节点主机**或将其作为后台服务管理。参见 [`nova-engine node`](/cli/node)。
...
浏览器控制 CLI（专用 Chrome/Brave/Edge/Chromium）。参见 [`nova-engine browser`](/cli/browser) 和[浏览器工具](/tools/browser)。
