...
read_when:
  - 你想管理智能体钩子
  - 你想安装或更新钩子
summary: CLI 参考：`nova-engine hooks`（智能体钩子）
title: hooks
x-i18n:
...
通过将特定钩子添加到配置（`~/.nova-engine/config.json`）来启用它。

**注意：** 由插件管理的钩子在 `nova-engine hooks list` 中显示 `plugin:<id>`，
...
**输出：** `~/.nova-engine/workspace/memory/YYYY-MM-DD-slug.md`
