...
read_when:
  - 你想安全地更新源码检出
  - 你需要了解 `--update` 简写行为
summary: "`nova-engine update` 的 CLI 参考（相对安全的源码更新 + Gateway 网关自动重启）"
title: update
x-i18n:
...
安全更新 Nova Engine 并在 stable/beta/dev 渠道之间切换。
...
当你显式切换渠道（`--channel ...`）时，Nova Engine 也会保持安装方式一致：

- `dev` → 确保存在 git 检出（默认：`~/nova-engine`，可通过 `NOVA_GIT_DIR` 覆盖），更新它，并从该检出安装全局 CLI。
...
7. 构建 + 构建控制界面。
8. 运行 `nova-engine doctor` 作为最终的"安全更新"检查。
9. 将插件同步到当前渠道（dev 使用捆绑的扩展；stable/beta 使用 npm）并更新 npm 安装的插件。

## `--update` 简写

`nova-engine --update` 会重写为 `nova-engine update`（便于 shell 和启动脚本使用）。

## 另请参阅

- `nova-engine doctor`（在 git 检出上会提供先运行更新的选项）
