...
summary: "`nova-engine models` 的 CLI 参考（status/list/set/scan、别名、回退、认证）"
title: models
x-i18n:
...
`nova-engine models status` 显示已解析的默认模型/回退配置以及认证概览。
...
命令会使用 `NOVA_AGENT_DIR`/`PI_CODING_AGENT_DIR`（如已设置），否则使用
已配置的默认智能体。
...
- 如果省略提供商，Nova Engine 会将输入视为别名或**默认提供商**的模型（仅在模型 ID 不包含 `/` 时有效）。
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>`（已配置的智能体 ID；覆盖 `NOVA_AGENT_DIR`/`PI_CODING_AGENT_DIR`）
...
`nova-engine plugins list` 查看已安装的提供商。
