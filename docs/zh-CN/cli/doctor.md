...
read_when:
  - 你遇到连接/认证问题，需要引导式修复
  - 你更新后想进行完整性检查
summary: "`nova-engine doctor` 的 CLI 参考（健康检查 + 引导式修复）"
title: doctor
x-i18n:
...
- `--fix`（`--repair` 的别名）会将备份写入 `~/.nova-engine/nova-engine.json.bak`，并删除未知的配置键，同时列出每个删除项。

## macOS：`launchctl` 环境变量覆盖

如果你之前运行过 `launchctl setenv NOVA_GATEWAY_TOKEN ...`（或 `...PASSWORD`），该值会覆盖你的配置文件，并可能导致持续的"未授权"错误。
