...
read_when:
  ...
  - 你想检查渠道状态或跟踪渠道日志
summary: "`nova-engine channels` 的 CLI 参考（账户、状态、登录/登出、日志）"
title: channels
x-i18n:
...
提示：`nova-engine channels add --help` 显示每个渠道的标志（token、app token、signal-cli 路径等）。
...
- 运行 `nova-engine status --deep` 进行全面探测。
- 使用 `nova-engine doctor` 获取引导式修复。
- `nova-engine channels list` 输出 `Claude: HTTP 403 ... user:profile` → 用量快照需要 `user:profile` 权限范围。使用 `--no-usage`，或提供 claude.ai 会话密钥（`CLAUDE_WEB_SESSION_KEY` / `CLAUDE_WEB_COOKIE`），或通过 Claude Code CLI 重新授权。
