...
运行与 Nova Engine Gateway 网关通信的 ACP（Agent Client Protocol）桥接器。
...
当 IDE（或其他客户端）使用 Agent Client Protocol 并且你希望它驱动 Nova Engine Gateway 网关会话时，请使用 ACP。

1. 确保 Gateway 网关正在运行（本地或远程）。
2. 配置 Gateway 网关目标（配置或标志）。
3. 将你的 IDE 配置为通过 stdio 运行 `nova-engine acp`。
...
在 Zed 中，打开 Agent 面板并选择"Nova Engine ACP"来开始一个会话。
...
- `--cwd <dir>`：ACP 会话的工作目录。
- `--server <command>`：ACP 服务器命令（默认：`nova-engine`）。
