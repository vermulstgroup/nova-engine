...
summary: Nova Engine Gateway 网关 CLI（`nova-engine gateway`）— 运行、查询和发现 Gateway 网关
title: gateway
x-i18n:
...
Gateway 网关是 Nova Engine 的 WebSocket 服务器（渠道、节点、会话、hooks）。

本页中的子命令位于 `nova-engine gateway …` 下。
...
- 默认情况下，除非在 `~/.nova-engine/nova-engine.json` 中设置了 `gateway.mode=local`，否则 Gateway 网关将拒绝启动。使用 `--allow-unconfigured` 进行临时/开发运行。
...
- `--token <token>`：令牌覆盖（同时为进程设置 `NOVA_GATEWAY_TOKEN`）。
- `--password <password>`：密码覆盖（同时为进程设置 `NOVA_GATEWAY_PASSWORD`）。
...
`gateway discover` 扫描 Gateway 网关信标（`_nova-engine-gw._tcp`）。

- 组播 DNS-SD：`local.`
- 单播 DNS-SD（广域 Bonjour）：选择一个域（示例：`nova-engine.internal.`）并设置分割 DNS + DNS 服务器；参见 [/gateway/bonjour](/gateway/bonjour)
