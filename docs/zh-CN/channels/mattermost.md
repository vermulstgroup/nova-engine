...
read_when:
  - 设置 Mattermost
  - 调试 Mattermost 路由
summary: Mattermost 机器人设置和 Nova Engine 配置
title: Mattermost
x-i18n:
...
如果你在配置/新手引导期间选择 Mattermost 并检测到 git 检出，Nova Engine 会自动提供本地安装路径。
...
在 `nova-engine message send` 或 cron/webhooks 中使用这些目标格式：
