...
通过 IRC 连接支持 Twitch 聊天。Nova Engine 以 Twitch 用户（机器人账户）身份连接，在频道中接收和发送消息。
...
4. 配置令牌：
   - 环境变量：`NOVA_TWITCH_ACCESS_TOKEN=...`（仅限默认账户）
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "nova-engine", // 机器人的 Twitch 账户
      accessToken: "oauth:abc123...", // OAuth Access Token（或使用 NOVA_TWITCH_ACCESS_TOKEN 环境变量）
