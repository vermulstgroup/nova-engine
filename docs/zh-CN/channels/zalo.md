...
- 通过 CLI 安装：`nova-engine plugins install @nova-engine/zalo`
...
1. 安装 Zalo 插件：
   - 从源代码检出：`nova-engine plugins install ./extensions/zalo`
   - 从 npm（如果已发布）：`nova-engine plugins install @nova-engine/zalo`
...
- 使用聊天 id 作为目标。
- 示例：`nova-engine message send --channel zalo --target 123456789 --message "hi"`。

## 故障排除

**Bot 不响应：**

- 检查 token 是否有效：`nova-engine channels status --probe`
- 验证发送者已被批准（配对或 allowFrom）
- 检查 Gateway 网关日志：`nova-engine logs --follow`
