...
"配对"是 Nova Engine 的显式**所有者批准**步骤。它用于两个地方：
...
- 旧版 `node.pair.*` API（CLI：`nova-engine nodes pending/approve`）是一个单独的 Gateway 网关拥有的配对存储。WS 节点仍然需要设备配对。
