...
3. 创建并下载 **JSON 密钥**：
   ...
   - 选择 **JSON** 并点击 **Create**。
4. 将下载的 JSON 文件存储在 Gateway 网关主机上（例如 `~/.nova-engine/googlechat-service-account.json`）。
5. 在 [Google Cloud Console Chat Configuration](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat) 中创建一个 Google Chat 应用：
   - 填写 **Application info**：
     - **App name**：（例如 `Nova Engine`）
     - **Avatar URL**：（例如 `https://nova-engine.ai/logo.png`）
   ...
   - 在 **Triggers** 下，选择 **Use a common HTTP endpoint URL for all triggers** 并将其设置为你的 Gateway 网关公网 URL 后加 `/googlechat`。
     - _提示：运行 `nova-engine status` 查看你的 Gateway 网关公网 URL。_
...
6. **启用应用状态**：
   ...
   - 再次点击 **Save**。
7. 使用服务账号路径和 webhook audience 配置 Nova Engine：
...
Google Chat webhooks 需要一个公网 HTTPS 端点。为了安全起见，**只将 `/googlechat` 路径暴露到互联网**。将 Nova Engine 仪表板和其他敏感端点保留在你的私有网络上。
...
使用此配置，任何发往 `your-domain.com/` 的请求将被忽略或返回 404，而 `your-domain.com/googlechat` 会安全地路由到 Nova Engine。
...
2. Nova Engine 根据配置的 `audienceType` + `audience` 验证令牌：
...
- 检查 `nova-engine channels status --probe` 以查看认证错误或缺少 audience 配置。
- 如果没有收到消息，请确认 Chat 应用的 webhook URL 和事件订阅。
- 如果提及门控阻止了回复，请将 `botUser` 设置为应用的用户资源名称并验证 `requireMention`。
- 在发送测试消息时使用 `nova-engine logs --follow` 查看请求是否到达 Gateway 网关。
