...
在 `~/.nova-engine/nova-engine.json` 中添加 `groupChat` 块，以便在 WhatsApp 剥离文本正文中的可视 `@` 时，显示名称提及仍能正常工作：
...
- 正则表达式不区分大小写；它们涵盖了像 `@nova-engine` 这样的显示名称提及，以及带或不带 `+`/空格的原始号码。
...
1. 将你的 WhatsApp 账号（运行 Nova Engine 的账号）添加到群组。
2. 说 `@nova-engine …`（或包含号码）。只有允许列表中的发送者才能触发，除非你设置 `groupPolicy: "open"`。
...
- 手动冒烟测试：
  - 在群组中发送 `@nova-engine` 提及，确认收到引用发送者名称的回复。
...
- 会话存储条目将在会话存储中显示为 `agent:<agentId>:whatsapp:group:<jid>`（默认为 `~/.nova-engine/agents/<agentId>/sessions/sessions.json`）；缺失条目只是意味着该群组尚未触发运行。
