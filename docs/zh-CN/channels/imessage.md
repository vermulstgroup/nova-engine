...
3. 配置 Nova Engine 的 `channels.imessage.cliPath` 和 `channels.imessage.dbPath`。
...
- 已登录"信息"的 macOS。
- Nova Engine + `imsg` 的完全磁盘访问权限（访问"信息"数据库）。
- 发送时需要自动化权限。
...
2. 创建 macOS 用户（例如：`nova-enginehome`）并登录。
...
如果你想在另一台 Mac 上使用 iMessage，请将 `channels.imessage.cliPath` 设置为通过 SSH 在远程 macOS 主机上运行 `imsg` 的包装脚本。Nova Engine 只需要 stdio。
...
**远程附件：** 当 `cliPath` 通过 SSH 指向远程主机时，"信息"数据库中的附件路径引用的是远程机器上的文件。Nova Engine 可以通过设置 `channels.imessage.remoteHost` 自动通过 SCP 获取这些文件：
...
如果未设置 `remoteHost`，Nova Engine 会尝试通过解析包装脚本中的 SSH 命令自动检测。建议显式配置以提高可靠性。
...
│ - nova-engine gateway           │          SCP (attachments)        │ - Messages signed in     │
...
示例包装脚本（`~/.nova-engine/scripts/imsg-ssh`）：
...
多账户支持：使用 `channels.imessage.accounts` 配置每个账户及可选的 `name`。参见 [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) 了解共享模式。不要提交 `~/.nova-engine/nova-engine.json`（它通常包含令牌）。
...
如果你在 `channels.imessage.groups` 下显式配置了 `chat_id`，Nova Engine 会将该会话视为"群组"用于：
