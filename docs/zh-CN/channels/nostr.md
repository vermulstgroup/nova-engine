...
Nostr 是一个去中心化的社交网络协议。此渠道使 Nova Engine 能够通过 NIP-04 接收和回复加密私信（DMs）。

## 安装（按需）

### 新手引导（推荐）

- 新手引导向导（`nova-engine onboard`）和 `nova-engine channels add` 会列出可选的渠道插件。
- 选择 Nostr 会提示你按需安装插件。
