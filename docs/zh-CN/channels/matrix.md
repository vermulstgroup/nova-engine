...
Matrix 是一个开放的去中心化消息协议。Nova Engine 以 Matrix **用户**身份连接到任意主服务器，因此你需要为机器人创建一个 Matrix 账户。登录后，你可以直接私信机器人或邀请它加入房间（Matrix"群组"）。Beeper 也是一个有效的客户端选项，但它需要启用 E2EE。
...
如果你在配置/新手引导期间选择 Matrix 并检测到 git 检出，Nova Engine 将自动提供本地安装路径。
...
1. 安装 Matrix 插件：
   - 从 npm：`nova-engine plugins install @nova-engine/matrix`
   - 从本地检出：`nova-engine plugins install ./extensions/matrix`
...
3. 获取机器人账户的访问令牌：
   ...
   - 或设置 `channels.matrix.userId` + `channels.matrix.password`：Nova Engine 会调用相同的登录端点，将访问令牌存储在 `~/.nova-engine/credentials/matrix/credentials.json`，并在下次启动时重用。
...
- 发送到加密房间时，出站媒体会被加密。
- 首次连接时，Nova Engine 会向你的其他会话请求设备验证。
- 在另一个 Matrix 客户端（Element 等）中验证设备以启用密钥共享。
- 如果无法加载加密模块，E2EE 将被禁用，加密房间将无法解密；Nova Engine 会记录警告。
...
加密状态按账户 + 访问令牌存储在 `~/.nova-engine/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`（SQLite 数据库）。同步状态存储在同目录的 `bot-storage.json` 中。如果访问令牌（设备）更改，将创建新的存储，机器人必须重新验证才能访问加密房间。
...
- 启动时，Nova Engine 将允许列表中的房间/用户名称解析为 ID 并记录映射；未解析的条目不会参与允许列表匹配。
