...
3. 链接 bot 设备并启动守护进程：
   - `signal-cli link -n "Nova Engine"`
4. 配置 Nova Engine 并启动 Gateway 网关。
...
2. 链接 bot 账户：
   - `signal-cli link -n "Nova Engine"` 然后在 Signal 中扫描二维码。
...
如果你想自己管理 `signal-cli`（JVM 冷启动慢、容器初始化或共享 CPU），请单独运行守护进程并将 Nova Engine 指向它：
...
这会跳过自动启动和 Nova Engine 内部的启动等待。对于自动启动时的慢启动，请设置 `channels.signal.startupTimeoutMs`。
...
- **输入指示器**：Nova Engine 通过 `signal-cli sendTyping` 发送输入信号，并在回复运行时刷新它们。
- **已读回执**：当 `channels.signal.sendReadReceipts` 为 true 时，Nova Engine 为允许的私信转发已读回执。
