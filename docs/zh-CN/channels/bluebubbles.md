...
- Nova Engine 通过其 REST API 与之通信（`GET /api/v1/ping`、`POST /message/text`、`POST /chat/:id/*`）。
...
3. 运行 `nova-engine onboard` 并选择 BlueBubbles，或手动配置：
   ```json5
   {
     channels: {
       bluebubbles: {
...
- **输入指示器**：Nova Engine 发送输入开始事件；BlueBubbles 在发送或超时时自动清除输入状态（通过 DELETE 手动停止不可靠）。
...
Nova Engine 可能会显示*短*消息 ID（例如 `1`、`2`）以节省 token。
...
- 直接句柄：`+15555550123`、`user@example.com`
  - 如果直接句柄没有现有的私信聊天，Nova Engine 将通过 `POST /api/v1/chat/new` 创建一个。这需要启用 BlueBubbles Private API。
...
- 配对码在一小时后过期；使用 `nova-engine pairing list bluebubbles` 和 `nova-engine pairing approve bluebubbles <code>`。
...
- Nova Engine 会根据 BlueBubbles 服务器的 macOS 版本自动隐藏已知不可用的操作。如果在 macOS 26（Tahoe）上编辑仍然显示，请使用 `channels.bluebubbles.actions.edit=false` 手动禁用。
- 查看状态/健康信息：`nova-engine status --all` 或 `nova-engine status --deep`。
