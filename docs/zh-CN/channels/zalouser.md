---
read_when:
  - 为 Nova Engine 设置 Zalo Personal
...
- 通过 CLI 安装：`nova-engine plugins install @nova-engine/zalouser`
- 或从源码检出安装：`nova-engine plugins install ./extensions/zalouser`
- 详情：[插件](/tools/plugin)
...
- 配置向导可以提示输入群组允许列表。
- 启动时，Nova Engine 将允许列表中的群组/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
...
- 重新登录：`nova-engine channels logout --channel zalouser && nova-engine channels login --channel zalouser`
