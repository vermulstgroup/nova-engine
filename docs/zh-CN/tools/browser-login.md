...
当网站需要登录时，请在**主机**浏览器配置文件（nova-engine 浏览器）中**手动登录**。
...
Nova Engine 控制一个**专用的 Chrome 配置文件**（名为 `nova-engine`，橙色调 UI）。这与你的日常浏览器配置文件是分开的。
...
如果你有多个配置文件，传入 `--browser-profile <name>`（默认是 `nova-engine`）。
