...
read_when:
  - 添加或修改智能体 CLI 入口点
summary: 直接 `nova-engine agent` CLI 运行（带可选投递）
title: Agent Send
x-i18n:
...
# `nova-engine agent`（直接智能体运行）

`nova-engine agent` 运行单个智能体回合，无需入站聊天消息。
...
- 输出：
  ...
  - `--json`：打印结构化负载 + 元数据
- 可选使用 `--deliver` + `--channel` 将回复投递回渠道（目标格式与 `nova-engine message --target` 匹配）。
