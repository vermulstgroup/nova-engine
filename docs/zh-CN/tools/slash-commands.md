...
- `commands.config`（默认 `false`）启用 `/config`（读写 `nova-engine.json`）。
...
- 要获取完整的提供商使用量分解，使用 `nova-engine status --usage`。
- `/allowlist add|remove` 需要 `commands.config=true` 并遵循渠道 `configWrites`。
- `/usage` 控制每响应使用量页脚；`/usage cost` 从 Nova Engine 会话日志打印本地成本摘要。
...
- 覆盖立即应用于新的配置读取，但**不会**写入 `nova-engine.json`。
- 使用 `/debug reset` 清除所有覆盖并返回到磁盘上的配置。

## 配置更新

`/config` 写入你的磁盘配置（`nova-engine.json`）。仅所有者。默认禁用；使用 `commands.config: true` 启用。
...
- **Slack：** `channels.slack.slashCommand` 仍然支持单个 `/nova-engine` 风格的命令。如果你启用 `commands.native`，你必须为每个内置命令创建一个 Slack 斜杠命令（与 `/help` 相同的名称）。Slack 的命令参数菜单以临时 Block Kit 按钮形式发送。
