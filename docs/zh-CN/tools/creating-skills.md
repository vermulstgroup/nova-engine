...
Nova Engine 被设计为易于扩展。"Skills"是为你的助手添加新功能的主要方式。
...
Skills 位于你的工作区中，通常是 `~/.nova-engine/workspace/skills/`。为你的 Skill 创建一个新文件夹：
...
让你的智能体"刷新 skills"或重启 Gateway 网关。Nova Engine 将发现新目录并索引 `SKILL.md`。

## 最佳实践

- **简洁明了**：指示模型*做什么*，而不是如何成为一个 AI。
- **安全第一**：如果你的 Skill 使用 `bash`，确保提示词不允许来自不受信任用户输入的任意命令注入。
- **本地测试**：使用 `nova-engine agent --message "use my new skill"` 进行测试。
