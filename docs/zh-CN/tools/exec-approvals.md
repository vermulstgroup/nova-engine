...
- **gateway 主机** → gateway 机器上的 `nova-engine` 进程
...
如果节点尚未通告执行审批，请直接编辑其本地的 `~/.nova-engine/exec-approvals.json`。

CLI：`nova-engine approvals` 支持 gateway 或 node 编辑（参见 [Approvals CLI](/cli/approvals)）。
