...
read_when:
  - 添加或修改智能体工具
  - 停用或更改 `nova-engine-*` Skills
summary: Nova Engine 的智能体工具接口（browser、canvas、nodes、message、cron），替代旧版 `nova-engine-*` Skills
title: 工具
x-i18n:
...
Nova Engine 为 browser、canvas、nodes 和 cron 暴露**一流的智能体工具**。
这些工具取代了旧的 `nova-engine-*` Skills：工具是类型化的，无需调用 shell，
智能体应该直接依赖它们。

## 禁用工具

你可以通过 `nova-engine.json` 中的 `tools.allow` / `tools.deny` 全局允许/拒绝工具
...
- 匹配不区分大小写。
- 支持 `*` 通配符（`"*"` 表示所有工具）。
- 如果 `tools.allow` 仅引用未知或未加载的插件工具名称，Nova Engine 会记录警告并忽略允许列表，以确保核心工具保持可用。
...
- `group:nodes`：`nodes`
- `group:nova-engine`：所有内置 Nova Engine 工具（不包括提供商插件）
...
- `host=node` 可以针对 macOS 配套应用或无头节点主机（`nova-engine node run`）。
...
- 需要 Brave API 密钥（推荐：`nova-engine configure --section web`，或设置 `BRAVE_API_KEY`）。
...
- 快速冒烟测试：`nova-engine nodes canvas a2ui push --node <id> --text "Hello from A2UI"`。
...
- `restart`（授权 + 发送 `SIGUSR1` 进行进程内重启；`nova-engine gateway` 就地重启）
