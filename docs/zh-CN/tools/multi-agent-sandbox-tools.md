...
有关调试"为什么这被阻止了？"，请参见[沙箱 vs 工具策略 vs 提权](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `nova-engine sandbox explain`。
...
旧版 `agent.*` 配置由 `nova-engine doctor` 迁移；今后请优先使用 `agents.defaults` + `agents.list`。
