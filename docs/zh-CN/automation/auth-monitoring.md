...
Nova Engine 通过 `nova-engine models status` 提供 OAuth 过期健康状态。请使用该命令进行自动化和告警；脚本是为手机工作流程提供的可选附加功能。
...
- `scripts/claude-auth-status.sh` 现在使用 `nova-engine models status --json` 作为数据来源（如果 CLI 不可用则回退到直接读取文件），因此请确保 `nova-engine` 在定时器的 `PATH` 中。
- `scripts/auth-monitor.sh`：cron/systemd 定时器目标；发送告警（ntfy 或手机）。
- `scripts/systemd/nova-engine-auth-monitor.{service,timer}`：systemd 用户定时器。
- `scripts/claude-auth-status.sh`：Claude Code + Nova Engine 认证检查器（完整/json/简洁模式）。
...
- `scripts/termux-sync-widget.sh`：同步 Claude Code 凭证 → Nova Engine。
