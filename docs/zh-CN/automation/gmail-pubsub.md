---
read_when:
  - 将 Gmail 收件箱触发器接入 Nova Engine
  - 为智能体唤醒设置 Pub/Sub 推送
summary: 通过 gogcli 将 Gmail Pub/Sub 推送接入 Nova Engine webhooks
title: Gmail PubSub
x-i18n:
...
目标：Gmail watch -> Pub/Sub 推送 -> `gog gmail watch serve` -> Nova Engine webhook。
...
- 已启用 Nova Engine hooks（参见 [Webhooks](/automation/webhook)）。
...
使用 Nova Engine 助手将所有内容接入在一起（在 macOS 上通过 brew 安装依赖）：
...
- 为 `nova-engine webhooks gmail run` 写入 `hooks.gmail` 配置。
- 启用 Gmail hook 预设（`hooks.presets: ["gmail"]`）。

路径说明：当启用 `tailscale.mode` 时，Nova Engine 会自动将 `hooks.gmail.serve.path` 设置为 `/`，并将公共路径保持在 `hooks.gmail.tailscale.path`（默认 `/gmail-pubsub`），因为 Tailscale 在代理之前会剥离设置的路径前缀。
...
- 设置 `NOVA_SKIP_GMAIL_WATCHER=1` 可退出（如果你自己运行守护进程则很有用）。
...
- `--hook-url` 指向 Nova Engine `/hooks/gmail`（已映射；隔离运行 + 摘要发送到主线程）。
- `--include-body` 和 `--max-bytes` 控制发送到 Nova Engine 的正文片段。

推荐：`nova-engine webhooks gmail run` 封装了相同的流程并自动续期 watch。
