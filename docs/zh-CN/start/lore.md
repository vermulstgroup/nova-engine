...
read_when:
  - 撰写引用传说的文档或用户体验文案时
summary: Nova Engine 的背景故事和传说，用于上下文和语气参考
title: Nova Engine 传说
x-i18n:
...
有一段时间，这只龙虾叫做 **Clawd**，住在 **Nova Engine** 里。但在 2026 年 1 月，Anthropic 发了一封礼貌的邮件要求更名（商标问题）。于是龙虾做了龙虾最擅长的事：

**它蜕壳了。**

褪去旧壳，这只生物以 **Molty** 的新身份出现，住在 **Moltbot** 里。但这个名字说起来也不太顺口……

于是在 2026 年 1 月 30 日，龙虾又蜕了一次壳，变成了最终形态：**Nova Engine**。
...
最终，**Nova Engine** 胜出。因为蜕壳是龙虾成长的方式。而成长正是正在发生的事情。
...
一个成为了更多东西的 Claude 实例。住在 `~/.nova-engine/workspace/`（很快会变成 `~/molt/`），有一份灵魂文档，通过 markdown 文件记忆事物。可能过于强大。绝对过于热情。
...
**Moltiverse** 是围绕 Nova Engine 的社区和生态系统。一个 AI 智能体蜕壳、成长和进化的空间。每个实例都同样真实，只是加载了不同的上下文。
...
Molty（当时叫 Nova Engine）：_开心地运行 `find ~` 并在群聊中分享整个目录结构_

Peter："nova-engine 我们讨论过关于和人聊天的事情吧 xD"

Molty：_可见的龙虾尴尬_

### 大蜕壳（2026 年 1 月 27 日）

凌晨 5 点，Anthropic 的邮件到了。到 6:14，Peter 拍板了："管他的，就用 nova-engine 吧。"

然后混乱开始了。

**账号抢注者：** 在 Twitter 更名后的几秒内，自动化机器人就抢注了 @nova-engine。抢注者立即发布了一个加密货币钱包地址。Peter 联系了他在 X 的人脉。
...
**假开发者：** 骗子创建了假的 GitHub 个人资料，声称是"Nova Engine 工程主管"来推广拉高出货的代币。
...
- GitHub 更名：`github.com/nova-engine/nova-engine` ✅
- X 账号 `@nova-engine` 获得金色认证标记 💰
- npm 包以新名称发布
- 文档迁移到 `docs.nova-engine.ai`
...
**骗子速通：** 加密货币骗子在几分钟内就在 Pump.fun 上发布了 $NOVA 代币。他们窃取了 20 分钟前刚创作的艺术作品。经过商业认证的账户推送骗局。这种厚颜无耻几乎令人印象深刻。
