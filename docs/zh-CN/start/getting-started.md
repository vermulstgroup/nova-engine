...
最快聊天：打开 Control UI（无需渠道设置）。运行 `nova-engine dashboard` 并在浏览器中聊天，或在 Gateway 网关主机上打开 `http://127.0.0.1:18789/`。文档：[Dashboard](/web/dashboard) 和 [Control UI](/web/control-ui)。

推荐路径：使用 **CLI 新手引导向导**（`nova-engine onboard`）。它设置：
...
- Node `>=22`
- `pnpm`（可选；如果从源代码构建则推荐）
- **推荐：**Brave Search API 密钥用于网页搜索。最简单的方式：`nova-engine configure --section web`（存储 `tools.web.search.apiKey`）。参见 [Web 工具](/tools/web)。
...
- OAuth 凭证（旧版导入）：`~/.nova-engine/credentials/oauth.json`
- 认证配置文件（OAuth + API 密钥）：`~/.nova-engine/agents/<agentId>/agent/auth-profiles.json`
...
如果你还没有全局安装，从仓库通过 `pnpm nova-engine ...` 运行新手引导步骤。`pnpm build` 也会打包 A2UI 资源；如果你只需要运行那个步骤，使用 `pnpm canvas:a2ui:bundle`。

Gateway 网关（从此仓库）：

```bash
node nova-engine.mjs gateway --port 18789 --verbose
```
...
```bash
nova-engine message send --target +15555550123 --message "Hello from Nova Engine"
```

如果 `nova-engine health` 显示"未配置认证"，回到向导设置 OAuth/密钥认证——没有它智能体将无法响应。

提示：`nova-engine status --all` 是最佳的可粘贴、只读调试报告。
健康探测：`nova-engine health`（或 `nova-engine status --deep`）向运行中的 Gateway 网关请求健康快照。
