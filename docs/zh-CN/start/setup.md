...
summary: 设置指南：在保持最新的同时保持你的 Nova Engine 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.nova-engine/workspace`（工作区）+ `~/.nova-engine/nova-engine.json`（配置）。
...
- **配置：** `~/.nova-engine/nova-engine.json`（JSON/JSON5 格式）
- **工作区：** `~/.nova-engine/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm nova-engine setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **Nova Engine.app**（菜单栏）。
...
- 运行 `nova-engine setup`，然后 `nova-engine channels login`，然后手动启动 Gateway 网关（`nova-engine gateway`）。
...
- **状态存储位置：**
  ...
  - 会话：`~/.nova-engine/agents/<agentId>/sessions/`
  - 日志：`/tmp/nova-engine/`
...
- **配对允许列表**：`~/.nova-engine/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.nova-engine/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.nova-engine/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.nova-engine/workspace` 和 `~/.nova-engine/` 保持为"你的东西"；不要将个人提示/配置放入 `nova-engine` 仓库。
...
- [Nova Engine 助手设置](/start/nova-engine)
