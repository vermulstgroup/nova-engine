...
新手引导向导是在 macOS、Linux 或 Windows（通过 WSL2；强烈推荐）上设置 Nova Engine 的**推荐**方式。
...
最快开始聊天的方式：打开控制界面（无需设置渠道）。运行 `nova-engine dashboard` 并在浏览器中聊天。文档：[控制面板](/web/dashboard)。
...
推荐：设置 Brave Search API 密钥，以便智能体可以使用 `web_search`（`web_fetch` 无需密钥即可使用）。最简单的方式：`nova-engine configure --section web`，它会存储 `tools.web.search.apiKey`。文档：[Web 工具](/tools/web)。
...
1. **现有配置检测**
   - 如果 `~/.nova-engine/nova-engine.json` 存在，选择**保留 / 修改 / 重置**。
   - 重新运行向导**不会**清除任何内容，除非你明确选择**重置**（或传递 `--reset`）。
   - 如果配置无效或包含遗留键名，向导会停止并要求你在继续之前运行 `nova-engine doctor`。
...
2. **模型/认证**
   ...
   - **OpenAI API 密钥**：如果存在则使用 `OPENAI_API_KEY`，否则提示输入密钥，然后保存到 `~/.nova-engine/.env` 以便 launchd 可以读取。
...
- OAuth 凭证存储在 `~/.nova-engine/credentials/oauth.json`；认证配置文件存储在 `~/.nova-engine/agents/<agentId>/agent/auth-profiles.json`（API 密钥 + OAuth）。
- 更多详情：[/concepts/oauth](/concepts/oauth)

3. **工作区**
   - 默认 `~/.nova-engine/workspace`（可配置）。
   - 为智能体引导仪式播种所需的工作区文件。
...
5. **渠道**
   ...
   - 私信安全：默认为配对。第一条私信发送验证码；通过 `nova-engine pairing approve <channel> <code>` 批准或使用允许列表。
...
7. **健康检查**
   - 启动 Gateway 网关（如果需要）并运行 `nova-engine health`。
   - 提示：`nova-engine status --deep` 在状态输出中添加 Gateway 网关健康探测（需要可达的 Gateway 网关）。
...
使用 `nova-engine agents add <name>` 创建一个具有独立工作区、会话和认证配置文件的单独智能体。不带 `--workspace` 运行会启动向导。
...
- 默认工作区遵循 `~/.nova-engine/workspace-<agentId>`。
...
- 下载适当的发布资源。
- 存储在 `~/.nova-engine/tools/signal-cli/<version>/` 下。
...
`~/.nova-engine/nova-engine.json` 中的典型字段：
...
`nova-engine agents add` 写入 `agents.list[]` 和可选的 `bindings`。

WhatsApp 凭证存储在 `~/.nova-engine/credentials/whatsapp/<accountId>/` 下。
会话存储在 `~/.nova-engine/agents/<agentId>/sessions/` 下。
