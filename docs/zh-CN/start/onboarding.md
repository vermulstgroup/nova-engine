...
read_when:
  - 设计 macOS 新手引导助手
  - 实现认证或身份设置
summary: Nova Engine 的首次运行新手引导流程（macOS 应用）
title: 新手引导
x-i18n:
...
- 打开浏览器进行 OAuth（PKCE）
- 要求用户粘贴 `code#state` 值
- 将凭证写入 `~/.nova-engine/credentials/oauth.json`
...
应用可以通过 npm/pnpm 安装全局 `nova-engine` CLI，以便终端工作流和 launchd 任务开箱即用。
...
在首次智能体运行时，Nova Engine 会引导一个工作区（默认 `~/.nova-engine/workspace`）：
...
```bash
nova-engine webhooks gmail setup --account you@gmail.com
```
...
- `~/.nova-engine/credentials/oauth.json`
