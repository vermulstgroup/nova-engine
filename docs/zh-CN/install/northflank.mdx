...
通过一键模板在 Northflank 上部署 Nova Engine，然后在浏览器中完成设置。
这是最简单的"无需在服务器上使用终端"的方式：Northflank 为你运行 Gateway网关，
你通过 `/setup` 网页向导配置一切。

## 如何开始

1. 点击 [Deploy Nova Engine](https://northflank.com/stacks/deploy-nova-engine) 打开模板。
2. 如果你还没有账户，请创建一个 [Northflank 账户](https://app.northflank.com/signup)。
3. 点击 **Deploy Nova Engine now**。
4. 设置必需的环境变量：`SETUP_PASSWORD`。
5. 点击 **Deploy stack** 构建并运行 Nova Engine 模板。
6. 等待部署完成，然后点击 **View resources**。
7. 打开 Nova Engine 服务。
8. 打开公开的 Nova Engine URL，在 `/setup` 完成设置。
9. 在 `/nova-engine` 打开控制面板 UI。

## 你将获得

- 托管的 Nova Engine Gateway网关 + 控制面板 UI
...
4. 点击 **Run setup**。
5. 在 `https://<your-northflank-domain>/nova-engine` 打开控制面板 UI。
