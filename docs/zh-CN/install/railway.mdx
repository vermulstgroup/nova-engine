...
通过一键模板在 Railway 上部署 Nova Engine，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/nova-engine` — 控制面板 UI
...
- `NOVA_STATE_DIR=/data/.nova-engine`（推荐）
- `NOVA_WORKSPACE_DIR=/data/workspace`（推荐）
- `NOVA_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 Nova Engine 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
