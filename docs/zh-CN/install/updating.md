...
read_when:
  - 更新 Nova Engine
  - 更新后出现问题
summary: 安全更新 Nova Engine（全局安装或源码），以及回滚策略
title: 更新
x-i18n:
...
Nova Engine 发展迅速（尚未到"1.0"）。将更新视为发布基础设施：更新 → 运行检查 → 重启（或使用会重启的 `nova-engine update`）→ 验证。

## 推荐：重新运行网站安装程序（原地升级）

**首选**的更新路径是重新运行网站上的安装程序。它会检测现有安装、原地升级，并在需要时运行 `nova-engine doctor`。
...
- 对于**全局安装**，脚本底层使用 `npm install -g nova-engine@latest`。
...
- 快照你的定制内容：
  - 配置：`~/.nova-engine/nova-engine.json`
  - 凭证：`~/.nova-engine/credentials/`
  - 工作区：`~/.nova-engine/workspace`
...
- 如果你的 Gateway 网关作为服务运行，`nova-engine gateway restart` 优于杀死 PID。
- 如果你固定在特定版本，参见下面的"回滚/固定"。

## 更新（`nova-engine update`）
...
- 安装依赖、构建、构建控制 UI，并运行 `nova-engine doctor`。
- 默认重启 Gateway 网关（使用 `--no-restart` 跳过）。

如果你通过 **npm/pnpm** 安装（没有 git 元数据），`nova-engine update` 将尝试通过你的包管理器更新。如果无法检测到安装，请改用"更新（全局安装）"。

## 更新（控制 UI / RPC）

控制 UI 有**更新并重启**（RPC：`update.run`）。它：

1. 运行与 `nova-engine update` 相同的源码更新流程（仅限 git checkout）。
...
- 当你运行打包的 `nova-engine` 二进制文件（[`nova-engine.mjs`](https://github.com/nova-engine/nova-engine/blob/main/nova-engine.mjs)）或使用 Node 运行 `dist/` 时，`pnpm build` 很重要。
- 如果你从仓库 checkout 运行而没有全局安装，CLI 命令使用 `pnpm nova-engine ...`。
- 如果你直接从 TypeScript 运行（`pnpm nova-engine ...`），通常不需要重新构建，但**配置迁移仍然适用** → 运行 doctor。
- 在全局和 git 安装之间切换很容易：安装另一种方式，然后运行 `nova-engine doctor` 以便将 Gateway 网关服务入口点重写为当前安装。

## 始终运行：`nova-engine doctor`

Doctor 是"安全更新"命令。它故意很无聊：修复 + 迁移 + 警告。

注意：如果你是**源码安装**（git checkout），`nova-engine doctor` 会提供先运行 `nova-engine update`。
...
- 检测并将旧版 Gateway 网关服务（launchd/systemd；旧版 schtasks）迁移到当前 Nova Engine 服务。
...
- macOS launchd（应用捆绑的 LaunchAgent）：`launchctl kickstart -k gui/$UID/bot.molt.gateway`（使用 `bot.molt.<profile>`；旧版 `com.nova-engine.*` 仍然有效）
- Linux systemd 用户服务：`systemctl --user restart nova-engine-gateway[-<profile>].service`
- Windows（WSL2）：`systemctl --user restart nova-engine-gateway[-<profile>].service`
  - `launchctl`/`systemctl` 仅在服务已安装时有效；否则运行 `nova-engine gateway install`。
...
提示：要查看当前发布的版本，运行 `npm view nova-engine version`。
...
- 再次运行 `nova-engine doctor` 并仔细阅读输出（它通常会告诉你修复方法）。
