---
read_when:
  - 你想在 GCP 上 24/7 运行 Nova Engine
  - 你想要在自己的 VM 上运行生产级、常驻的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上 24/7 运行 Nova Engine Gateway 网关并持久化状态
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 Nova Engine（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 Nova Engine Gateway 网关，具有持久状态、内置二进制文件和安全的重启行为。

如果你想要"Nova Engine 24/7 大约 $5-12/月"，这是在 Google Cloud 上的可靠设置。
...
- 在主机上持久化 `~/.nova-engine` + `~/.nova-engine/workspace`（重启/重建后仍保留）
...
Nova Engine 在 Docker 中运行，但 Docker 不是真实来源。
...
| WhatsApp 会话    | `/home/node/.nova-engine/`           | 主机卷挂载    | 保留 QR 登录                |
