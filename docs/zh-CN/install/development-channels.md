...
当你使用 `--channel` **显式**切换渠道时，Nova Engine 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/nova-engine`，可通过 `NOVA_GIT_DIR` 覆盖），
...
当你使用 `nova-engine update` 切换渠道时，Nova Engine 还会同步插件来源：
