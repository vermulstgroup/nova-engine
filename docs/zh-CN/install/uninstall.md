---
read_when:
  - 你想从机器上移除 Nova Engine
  - 卸载后 Gateway 网关服务仍在运行
summary: 完全卸载 Nova Engine（CLI、服务、状态、工作区）
title: 卸载
x-i18n:
...
如果你将 `NOVA_CONFIG_PATH` 设置为状态目录外的自定义位置，也请删除该文件。
...
- 如果你使用了配置文件（`--profile` / `NOVA_PROFILE`），对每个状态目录重复步骤 3（默认为 `~/.nova-engine-<profile>`）。
- 在远程模式下，状态目录位于 **Gateway 网关主机**上，因此也需要在那里运行步骤 1-4。

## 手动服务移除（CLI 未安装）

如果 Gateway 网关服务持续运行但 `nova-engine` 缺失，请使用此方法。

### macOS（launchd）

默认标签是 `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.nova-engine.*` 可能仍然存在）：
...
如果你使用了配置文件，请将标签和 plist 名称替换为 `bot.molt.<profile>`。如果存在任何旧版 `com.nova-engine.*` plist，请将其移除。

### Linux（systemd 用户单元）

默认单元名称是 `nova-engine-gateway.service`（或 `nova-engine-gateway-<profile>.service`）：
...
默认任务名称是 `Nova Engine Gateway`（或 `Nova Engine Gateway (<profile>)`）。
任务脚本位于你的状态目录下。
...
如果你使用了配置文件，请删除匹配的任务名称和 `~\.nova-engine-<profile>\gateway.cmd`。

## 普通安装 vs 源码检出

### 普通安装（install.sh / npm / pnpm / bun）

如果你使用了 `https://nova-engine.ai/install.sh` 或 `install.ps1`，CLI 是通过 `npm install -g nova-engine@latest` 安装的。
使用 `npm rm -g nova-engine` 移除（或 `pnpm remove -g` / `bun remove -g`，如果你是用那种方式安装的）。

### 源码检出（git clone）

如果你从仓库检出运行（`git clone` + `nova-engine ...` / `bun run nova-engine ...`）：
