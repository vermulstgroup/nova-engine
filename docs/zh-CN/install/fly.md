...
**目标：** Nova Engine Gateway 网关运行在 [Fly.io](https://fly.io) 机器上，具有持久存储、自动 HTTPS 和 Discord/渠道访问。
...
| `internal_port = 3000`         | 必须与 `--port 3000`（或 `NOVA_GATEWAY_PORT`）匹配以进行 Fly 健康检查 |
| `memory = "2048mb"`            | 512MB 太小；推荐 2GB                                                      |
| `NOVA_STATE_DIR = "/data"` | 在卷上持久化状态                                                          |

## 3）设置密钥

```bash
# Required: Gateway token (for non-loopback binding)
fly secrets set NOVA_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- 非 loopback 绑定（`--bind lan`）出于安全需要 `NOVA_GATEWAY_TOKEN`。
- 像对待密码一样对待这些 token。
- **优先使用环境变量而不是配置文件**来存储所有 API 密钥和 token。这可以避免密钥出现在 `nova-engine.json` 中，防止意外暴露或记录。
...
**注意：** 使用 `NOVA_STATE_DIR=/data` 时，配置路径是 `/data/nova-engine.json`。
...
或访问 `https://my-nova-engine.fly.dev/`

粘贴你的 Gateway 网关 token（来自 `NOVA_GATEWAY_TOKEN` 的那个）进行认证。
...
**修复：** 确保 `internal_port` 与 Gateway 网关端口匹配（设置 `--port 3000` 或 `NOVA_GATEWAY_PORT=3000`）。
...
如果使用 `--allow-unconfigured`，Gateway 网关会创建最小配置。你在 `/data/nova-engine.json` 的自定义配置应该在重启时被读取。
...
**修复：** 确保 `fly.toml` 中设置了 `NOVA_STATE_DIR=/data` 并重新部署。
