...
当设置 `NOVA_NIX_MODE=1` 时（nix-openclaw 会自动设置）：
...
OpenClaw 从 `NOVA_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `NOVA_STATE_DIR` 中。

- `NOVA_STATE_DIR`（默认：`~/.openclaw`）
- `NOVA_CONFIG_PATH`（默认：`$NOVA_STATE_DIR/openclaw.json`）
