---
read_when:
  - 你想让 Nova Engine 与你的主 macOS 环境隔离
  - 你想在沙箱中集成 iMessage（BlueBubbles）
  - 你想要一个可重置、可克隆的 macOS 环境
  - 你想比较本地与托管 macOS VM 选项
summary: 在沙箱隔离的 macOS VM（本地或托管）中运行 Nova Engine，当你需要隔离或 iMessage 时
title: macOS 虚拟机
x-i18n:
...
# 在 macOS 虚拟机上运行 Nova Engine（沙箱隔离）
...
使用 [Lume](https://cua.ai/docs/lume) 在你现有的 Apple Silicon Mac 上的沙箱 macOS VM 中运行 Nova Engine。
...
VM 在后台运行。Nova Engine 的守护进程保持 Gateway 网关运行。
...
这是在 macOS 上运行的杀手级功能。使用 [BlueBubbles](https://bluebubbles.app) 将 iMessage 添加到 Nova Engine。
...
| VM IP 未显示            | 等待 VM 完全启动，再次运行 `lume get nova-engine`                   |
| 找不到 Lume 命令        | 将 `~/.local/bin` 添加到你的 PATH                                |
| WhatsApp 二维码扫描失败 | 确保运行 `nova-engine channels login` 时你是登录到 VM（而不是主机） |
