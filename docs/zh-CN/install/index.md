...
read_when:
  - 安装 Nova Engine
  - 你想从 GitHub 安装
summary: 安装 Nova Engine（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
通过 npm 全局安装 `nova-engine` 并运行新手引导。
...
pnpm approve-builds -g                # 批准 nova-engine、node-llama-cpp、sharp 等
pnpm add -g nova-engine@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm nova-engine ...` 运行仓库命令。
...
- 运行新手引导：`nova-engine onboard --install-daemon`
- 快速检查：`nova-engine doctor`
- 检查 Gateway 网关健康状态：`nova-engine status` + `nova-engine health`
- 打开仪表板：`nova-engine dashboard`
...
```bash
# 显式 npm
curl -fsSL https://nova-engine.ai/install.sh | bash -s -- --install-method npm
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `nova-engine`）。
