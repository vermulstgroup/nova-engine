...
使用基础设施即代码方式在 Render 上部署 Nova Engine。内置的 `render.yaml` Blueprint 以声明式方式定义了你的整个技术栈——服务、磁盘、环境变量，让你只需一键即可完成部署，并将基础设施与代码一同进行版本管理。
...
<a
  href="https://render.com/deploy?repo=https://github.com/nova-engine/nova-engine"
  target="_blank"
  rel="noreferrer"
>
  部署到 Render
...
Render Blueprint 是定义基础设施的 YAML 文件。本仓库中的 `render.yaml` 配置了运行 Nova Engine 所需的一切：
...
Web 管理面板位于 `https://<your-service>.onrender.com/nova-engine`。
...
如果你使用的是原始 Nova Engine 仓库，Render 不会自动部署你的 Nova Engine。要更新它，请在仪表盘中手动执行 Blueprint 同步。
...
对于 Nova Engine，垂直扩展通常就足够了。水平扩展需要粘性会话或外部状态管理。
...
这将下载一个可移植的备份文件，你可以在任何 Nova Engine 主机上恢复。
