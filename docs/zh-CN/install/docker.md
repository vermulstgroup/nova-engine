...
read_when:
  ...
  - 你正在验证 Docker 流程
summary: Nova Engine 的可选 Docker 设置和新手引导
title: Docker
x-i18n:
...
- `NOVA_DOCKER_APT_PACKAGES` — 在构建期间安装额外的 apt 包
- `NOVA_EXTRA_MOUNTS` — 添加额外的主机绑定挂载
- `NOVA_HOME_VOLUME` — 在命名卷中持久化 `/home/node`

完成后：

- 在浏览器中打开 `http://127.0.0.1:18789/`。
- 将令牌粘贴到控制 UI（设置 → token）。
- 需要再次获取带令牌的 URL？运行 `docker compose run --rm nova-engine-cli dashboard --no-open`。
...
注意：从仓库根目录运行 `docker compose ...`。如果你启用了 `NOVA_EXTRA_MOUNTS` 或 `NOVA_HOME_VOLUME`，设置脚本会写入 `docker-compose.extra.yml`；在其他地方运行 Compose 时包含它：
...
如果你想将额外的主机目录挂载到容器中，在运行 `docker-setup.sh` 之前设置 `NOVA_EXTRA_MOUNTS`。这接受逗号分隔的 Docker 绑定挂载列表，并通过生成 `docker-compose.extra.yml` 将它们应用到 `nova-engine-gateway` 和 `nova-engine-cli`。
...
- 如果你编辑 `NOVA_EXTRA_MOUNTS`，重新运行 `docker-setup.sh` 以重新生成额外的 compose 文件。
- `docker-compose.extra.yml` 是生成的。不要手动编辑它。

### 持久化整个容器 home（可选）

如果你想让 `/home/node` 在容器重建后持久化，通过 `NOVA_HOME_VOLUME` 设置一个命名卷。这会创建一个 Docker 卷并将其挂载到 `/home/node`，同时保持标准的配置/工作区绑定挂载。这里使用命名卷（不是绑定路径）；对于绑定挂载，使用 `NOVA_EXTRA_MOUNTS`。
...
- 如果你更改 `NOVA_HOME_VOLUME`，重新运行 `docker-setup.sh` 以重新生成额外的 compose 文件。
- 命名卷会持久化直到使用 `docker volume rm <name>` 删除。

### 安装额外的 apt 包（可选）

如果你需要镜像内的系统包（例如构建工具或媒体库），在运行 `docker-setup.sh` 之前设置 `NOVA_DOCKER_APT_PACKAGES`。这会在镜像构建期间安装包，因此即使容器被删除它们也会持久化。
...
- 这接受空格分隔的 apt 包名称列表。
- 如果你更改 `NOVA_DOCKER_APT_PACKAGES`，重新运行 `docker-setup.sh` 以重建镜像。
...
如果你需要 Playwright 安装系统依赖，使用 `NOVA_DOCKER_APT_PACKAGES` 重建镜像，而不是在运行时使用 `--with-deps`。

4. **持久化 Playwright 浏览器下载**：

- 在 `docker-compose.yml` 中设置 `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright`。
- 确保 `/home/node` 通过 `NOVA_HOME_VOLUME` 持久化，或通过 `NOVA_EXTRA_MOUNTS` 挂载 `/home/node/.cache/ms-playwright`。

### 权限 + EACCES

镜像以 `node`（uid 1000）运行。如果你在 `/home/node/.nova-engine` 上看到权限错误，确保你的主机绑定挂载由 uid 1000 拥有。
...
- Gateway 网关容器是会话的真实来源（`~/.nova-engine/agents/<agentId>/sessions/`）。
...
- 镜像：`nova-engine-sandbox:bookworm-slim`
- 每个智能体一个容器
- 智能体工作区访问：`workspaceAccess: "none"`（默认）使用 `~/.nova-engine/sandboxes`
...
- `user` 必须是 root 才能运行 `apt-get`（省略 `user` 或设置 `user: "0:0"`）。
  当 `setupCommand`（或 docker 配置）更改时，Nova Engine 会自动重建容器，除非容器是**最近使用的**（在约 5 分钟内）。热容器会记录警告，包含确切的 `nova-engine sandbox recreate ...` 命令。
...
这使用 `Dockerfile.sandbox` 构建 `nova-engine-sandbox:bookworm-slim`。
...
这构建 `nova-engine-sandbox-common:bookworm-slim`。要使用它：
...
这使用 `Dockerfile.sandbox-browser` 构建 `nova-engine-sandbox-browser:bookworm-slim`。容器运行启用 CDP 的 Chromium 和可选的 noVNC 观察器（通过 Xvfb 有头）。
...
- 镜像缺失：使用 [`scripts/sandbox-setup.sh`](https://github.com/nova-engine/nova-engine/blob/main/scripts/sandbox-setup.sh) 构建或设置 `agents.defaults.sandbox.docker.image`。
- 容器未运行：它会按需为每个会话自动创建。
- 沙箱中的权限错误：将 `docker.user` 设置为与你挂载的工作区所有权匹配的 UID:GID（或 chown 工作区文件夹）。
- 找不到自定义工具：Nova Engine 使用 `sh -lc`（登录 shell）运行命令，这会 source `/etc/profile` 并可能重置 PATH。设置 `docker.env.PATH` 以在前面添加你的自定义工具路径（例如 `/custom/bin:/usr/local/share/npm-global/bin`），或在你的 Dockerfile 中在 `/etc/profile.d/` 下添加脚本。
