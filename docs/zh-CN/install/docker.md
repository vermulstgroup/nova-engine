...
如果你想将额外的主机目录挂载到容器中，在运行 `docker-setup.sh` 之前设置 `NOVA_EXTRA_MOUNTS`。这接受逗号分隔的 Docker 绑定挂载列表，并通过生成 `docker-compose.extra.yml` 将它们应用到 `openclaw-gateway` 和 `openclaw-cli`。
...
```bash
export NOVA_HOME_VOLUME="openclaw_home"
./docker-setup.sh
```
...
```bash
docker compose exec openclaw-gateway node dist/index.js health --token "$NOVA_GATEWAY_TOKEN"
```
