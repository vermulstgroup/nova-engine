---
read_when:
  - 设计超越每日 Markdown 日志的工作区记忆（~/.nova-engine/workspace）
...
目标：Clawd 风格的工作区（`agents.defaults.workspace`，默认 `~/.nova-engine/workspace`），其中"记忆"以每天一个 Markdown 文件（`memory/YYYY-MM-DD.md`）加上一小组稳定文件（例如 `memory.md`、`SOUL.md`）的形式存储。
...
保持 `~/.nova-engine/workspace` 作为规范的人类可读记忆。
...
建议：**深度集成到 Nova Engine**，但保持可分离的核心库。
...
对于 `~/.nova-engine/workspace` 的务实观点：
