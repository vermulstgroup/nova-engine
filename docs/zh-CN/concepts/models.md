...
- `agents.defaults.models` 是 Nova Engine 可使用的模型白名单/目录（加上别名）。
...
如果设置了 `agents.defaults.models`，它将成为 `/model` 和会话覆盖的**白名单**。当用户选择不在该白名单中的模型时，Nova Engine 返回：
...
- 如果省略提供商，Nova Engine 将输入视为别名或**默认提供商**的模型（仅在模型 ID 中没有 `/` 时有效）。
...
`nova-engine models`（无子命令）是 `models status` 的快捷方式。
...
`nova-engine models scan` 检查 OpenRouter 的**免费模型目录**，并可选择性地探测模型的工具和图像支持。
...
`models.providers` 中的自定义提供商会写入智能体目录下的 `models.json`（默认 `~/.nova-engine/agents/<agentId>/models.json`）。除非 `models.mode` 设置为 `replace`，否则此文件默认会被合并。
