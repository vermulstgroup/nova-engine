...
- CLI 辅助工具：`nova-engine onboard`、`nova-engine models list`、`nova-engine models set <provider/model>`。

## 内置提供商（pi-ai 目录）

Nova Engine 附带 pi-ai 目录。这些提供商**不需要** `models.providers` 配置；只需设置认证 + 选择模型。
...
- CLI：`nova-engine onboard --auth-choice token`（粘贴 setup-token）或 `nova-engine models auth paste-token --provider anthropic`
...
- CLI：`nova-engine onboard --auth-choice openai-codex` 或 `nova-engine models auth login --provider openai-codex`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`nova-engine plugins enable google-antigravity-auth`
  - 登录：`nova-engine models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`nova-engine plugins enable google-gemini-cli-auth`
  - 登录：`nova-engine models auth login --provider google-gemini-cli --set-default`
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `nova-engine.json` 中。CLI 登录流程将令牌存储在 Gateway 网关主机的认证配置文件中。
...
nova-engine models set opencode/claude-opus-4-5
