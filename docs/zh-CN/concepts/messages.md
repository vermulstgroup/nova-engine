...
本页汇总了 Nova Engine 如何处理入站消息、会话、队列、流式传输和推理可见性。
...
渠道可能在重新连接后重复投递同一消息。Nova Engine 保持一个短期缓存，以渠道/账户/对端/会话/消息 ID 为键，因此重复投递不会触发另一次智能体运行。
