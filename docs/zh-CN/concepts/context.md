...
"上下文"是 **Nova Engine 在一次运行中发送给模型的所有内容**。它受模型的**上下文窗口**（token 限制）约束。

新手心智模型：

- **系统提示词**（Nova Engine 构建）：规则、工具、Skills 列表、时间/运行时，以及注入的工作区文件。
...
## Nova Engine 如何构建系统提示词

系统提示词由 **Nova Engine 拥有**，每次运行时重建。它包括：
...
默认情况下，Nova Engine 注入一组固定的工作区文件（如果存在）：
