---
read_when:
  - 你想全面了解 Nova Engine 的 OAuth 流程
  - 你遇到了令牌失效/登出问题
  - 你想了解 setup-token 或 OAuth 认证流程
  - 你想使用多账户或配置文件路由
summary: Nova Engine 中的 OAuth：令牌交换、存储和多账户模式
title: OAuth
x-i18n:
...
Nova Engine 支持通过 OAuth 进行"订阅认证"，适用于提供此功能的提供商（特别是 **OpenAI Codex（ChatGPT OAuth）**）。对于 Anthropic 订阅，请使用 **setup-token** 流程。本页说明：

- OAuth **令牌交换**的工作原理（PKCE）
- 令牌**存储**在哪里（以及原因）
- 如何处理**多账户**（配置文件 + 按会话覆盖）

Nova Engine 还支持**提供商插件**，它们自带 OAuth 或 API 密钥流程。通过以下命令运行：
...
- 你通过 Nova Engine _和_ Claude Code / Codex CLI 登录 → 其中一个稍后会随机"登出"

为减少这种情况，Nova Engine 将 `auth-profiles.json` 视为**令牌汇聚点**：
...
- 运行时缓存（自动管理；请勿编辑）：`~/.nova-engine/agents/<agentId>/agent/auth.json`

仅用于导入的旧版文件（仍然支持，但不是主存储）：

- `~/.nova-engine/credentials/oauth.json`（首次使用时导入到 `auth-profiles.json`）

以上所有路径也遵循 `$NOVA_STATE_DIR`（状态目录覆盖）。完整参考：[/gateway/configuration](/gateway/configuration#auth-storage-oauth--api-keys)

## Anthropic setup-token（订阅认证）

在任意机器上运行 `claude setup-token`，然后将其粘贴到 Nova Engine 中：
...
Nova Engine 的交互式登录流程在 `@mariozechner/pi-ai` 中实现，并集成到向导/命令中。
...
2. 将令牌粘贴到 Nova Engine
3. 作为令牌认证配置文件存储（无刷新）

向导路径为 `nova-engine onboard` → 认证选择 `setup-token`（Anthropic）。
...
向导路径为 `nova-engine onboard` → 认证选择 `openai-codex`。
...
- `nova-engine channels list --json`（显示 `auth[]`）
