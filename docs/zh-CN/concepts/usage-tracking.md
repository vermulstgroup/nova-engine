...
- 聊天中的 `/usage cost`：从 Nova Engine 会话日志汇总的本地费用摘要。
- CLI：`nova-engine status --usage` 打印完整的按提供商分类的详细信息。
- CLI：`nova-engine channels list` 在提供商配置旁打印相同的使用量快照（使用 `--no-usage` 跳过）。
