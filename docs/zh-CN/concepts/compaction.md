...
read_when:
  - 你想了解自动压缩和 /compact
  - 你正在调试长会话触及上下文限制的问题
summary: 上下文窗口 + 压缩：Nova Engine 如何将会话保持在模型限制内
title: 压缩
x-i18n:
...
每个模型都有一个**上下文窗口**（可见的最大 token 数）。长时间运行的对话会累积消息和工具结果；一旦窗口空间紧张，Nova Engine 会**压缩**较早的历史记录以保持在限制范围内。
...
当会话接近或超过模型的上下文窗口时，Nova Engine 会触发自动压缩，并可能使用压缩后的上下文重试原始请求。
...
在压缩之前，Nova Engine 可以运行一次**静默记忆刷写**轮次，将持久化笔记写入磁盘。详情及配置请参阅[记忆](/concepts/memory)。
...
上下文窗口因模型而异。Nova Engine 使用已配置提供商目录中的模型定义来确定限制。
