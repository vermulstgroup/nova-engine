...
- 以原始记录格式返回消息数组。
- 当给定 `sessionId` 时，Nova Engine 将其解析为相应的会话键（缺失的 id 会报错）。
...
- 完成后，Nova Engine 运行子智能体**通告步骤**并将结果发布到请求者聊天渠道。
