...
Nova Engine 围绕一个拥有会话状态的单一 **Gateway 网关进程**设计。
...
- 存储：`~/.nova-engine/agents/<agentId>/sessions/sessions.json`
- 记录：`~/.nova-engine/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram 话题会话：`.../<sessionId>-topic-<threadId>.jsonl`

Nova Engine 通过 `src/config/sessions.ts` 解析这些位置。
...
Nova Engine 有意**不**"修复"记录；Gateway 网关使用 `SessionManager` 来读/写它们。
...
这些是 Pi 运行时语义（Nova Engine 消费事件，但 Pi 决定何时压缩）。
...
Nova Engine 还为嵌入式运行强制执行安全下限：

- 如果 `compaction.reserveTokens < reserveTokensFloor`，Nova Engine 会提升它。
- 默认下限是 `20000` 个 token。
- 设置 `agents.defaults.compaction.reserveTokensFloor: 0` 以禁用下限。
- 如果它已经更高，Nova Engine 不会改变它。
...
Nova Engine 支持用于后台任务的"静默"回合，用户不应该看到中间输出。

约定：

- 助手以 `NO_REPLY` 开始其输出，表示"不要向用户发送回复"。
- Nova Engine 在投递层剥离/抑制此内容。

从 `2026.1.10` 开始，当部分块以 `NO_REPLY` 开头时，Nova Engine 还会抑制**草稿/打字流式输出**，因此静默操作不会在回合中途泄漏部分输出。
...
Pi 还在扩展 API 中公开了 `session_before_compact` 钩子，但 Nova Engine 的刷新逻辑目前位于 Gateway 网关端。
...
- 存储 vs 记录不匹配？从 `nova-engine status` 确认 Gateway 网关主机和存储路径。
- 压缩过于频繁？检查：
  - 模型上下文窗口（太小）
