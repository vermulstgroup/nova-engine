---
read_when:
  - 启动新的 Nova Engine 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 Nova Engine 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — Nova Engine 个人助手（默认）

## 首次运行（推荐）

Nova Engine 为智能体使用专用的工作区目录。默认：`~/.nova-engine/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
- macOS 应用管理权限（屏幕录制、通知、麦克风）并通过其内置二进制文件暴露 `nova-engine` CLI。
...
- 脚本编写优先使用 `nova-engine` CLI；mac 应用处理权限。
...
- 对于浏览器驱动的验证，使用带有 Nova Engine 管理的 Chrome 配置文件的 `nova-engine browser`（tabs/status/screenshot）。
- 对于 DOM 检查，使用 `nova-engine browser eval|query|dom|snapshot`（需要机器输出时使用 `--json`/`--out`）。
- 对于交互，使用 `nova-engine browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（click/type 需要 snapshot 引用；CSS 选择器使用 `evaluate`）。
