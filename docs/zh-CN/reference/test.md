...
脚本：[`scripts/bench-model.ts`](https://github.com/nova-engine/nova-engine/blob/main/scripts/bench-model.ts)
...
此脚本通过伪终端驱动交互式向导，验证配置/工作区/会话文件，然后启动 Gateway 网关并运行 `nova-engine health`。
