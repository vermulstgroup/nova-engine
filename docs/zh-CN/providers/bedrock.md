---
read_when:
  - 你想在 Nova Engine 中使用 Amazon Bedrock 模型
  - 你需要为模型调用配置 AWS 凭证/区域
summary: 在 Nova Engine 中使用 Amazon Bedrock（Converse API）模型
...
Nova Engine 可以通过 pi‑ai 的 **Bedrock Converse** 流式提供商使用 **Amazon Bedrock** 模型。Bedrock 认证使用 **AWS SDK 默认凭证链**，而非 API 密钥。
...
如果检测到 AWS 凭证，Nova Engine 可以自动发现支持**流式传输**和**文本输出**的 Bedrock 模型。发现功能使用 `bedrock:ListFoundationModels`，并会被缓存（默认：1 小时）。
...
当在附加了 IAM 角色的 EC2 实例上运行 Nova Engine 时，AWS SDK 会自动使用实例元数据服务（IMDS）进行认证。但是，Nova Engine 的凭证检测目前只检查环境变量，不检查 IMDS 凭证。
...
# 3. 在 EC2 实例上启用发现功能
nova-engine config set models.bedrockDiscovery.enabled true
nova-engine config set models.bedrockDiscovery.region us-east-1
...
- Nova Engine 按以下顺序获取凭证来源：`AWS_BEARER_TOKEN_BEDROCK`，然后是 `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`，然后是 `AWS_PROFILE`，最后是默认的 AWS SDK 链。
