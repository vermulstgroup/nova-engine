---
read_when:
  - 你想通过 Ollama 使用本地模型运行 Nova Engine
  - 你需要 Ollama 的安装和配置指导
summary: 通过 Ollama（本地 LLM 运行时）运行 Nova Engine
title: Ollama
x-i18n:
...
Ollama 是一个本地 LLM 运行时，可以轻松在你的机器上运行开源模型。Nova Engine 通过 Ollama 的 OpenAI 兼容 API 进行集成，并且当你通过 `OLLAMA_API_KEY`（或认证配置）启用且未定义显式的 `models.providers.ollama` 条目时，可以**自动发现支持工具调用的模型**。
...
3. 为 Nova Engine 启用 Ollama（任意值即可；Ollama 不需要真实密钥）：
...
当你设置了 `OLLAMA_API_KEY`（或认证配置）且**未**定义 `models.providers.ollama` 时，Nova Engine 会从本地 Ollama 实例 `http://127.0.0.1:11434` 发现模型：
...
如果设置了 `OLLAMA_API_KEY`，你可以在提供商条目中省略 `apiKey`，Nova Engine 会自动填充以进行可用性检查。
...
当 Ollama 在 `/api/show` 中报告 `thinking` 时，Nova Engine 会将模型标记为具有推理能力：
...
对于自动发现的模型，Nova Engine 会使用 Ollama 报告的上下文窗口（如果可用），否则默认为 `8192`。你可以在显式提供商配置中覆盖 `contextWindow` 和 `maxTokens`。
...
Nova Engine 仅自动发现报告了工具支持的模型。如果你的模型未列出，可以：
