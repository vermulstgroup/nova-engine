---
read_when:
  - 你想在 Nova Engine 中使用注重隐私的推理服务
  - 你需要 Venice AI 设置指导
summary: 在 Nova Engine 中使用 Venice AI 注重隐私的模型
title: Venice AI
x-i18n:
...
设置完成后，Nova Engine 会显示所有可用的 Venice 模型。根据你的需求选择：
...
## 通过 `nova-engine configure` 配置

1. 运行 `nova-engine configure`
2. 选择 **Model/auth**
3. 选择 **Venice AI**
...
当设置了 `VENICE_API_KEY` 时，Nova Engine 会自动从 Venice API 发现模型。如果 API 不可达，则回退到静态目录。
...
Venice 模型目录会动态更新。运行 `nova-engine models list` 查看当前可用的模型。部分模型可能暂时离线。
