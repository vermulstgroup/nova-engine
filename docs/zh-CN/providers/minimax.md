---
read_when:
  - 你想在 Nova Engine 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Nova Engine 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
nova-engine plugins enable minimax-portal-auth  # 如果已加载则跳过
nova-engine gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/nova-engine/nova-engine/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `nova-engine models list` 和 `nova-engine models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `nova-engine configure` 并选择 **MiniMax M2.1**，或
