...
read_when:
  ...
  - 排查 Nova Engine 中 tsx/esbuild 加载器崩溃问题
...
通过 Node 使用 `tsx` 运行 Nova Engine 时，启动阶段报错：
...
- 在此之前（Bun 路径），`nova-engine status` 和 `gateway:watch` 均正常工作。
...
- 使用 Node + tsc watch，然后运行编译产物：
  ...
  ```
- 已在本地确认：`pnpm exec tsc -p tsconfig.json` + `node nova-engine.mjs status` 在 Node 25 上可正常运行。
