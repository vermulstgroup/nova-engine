...
# 插件清单（nova-engine.plugin.json）

每个插件都**必须**在**插件根目录**下提供一个 `nova-engine.plugin.json` 文件。Nova Engine 使用此清单来**在不执行插件代码的情况下**验证配置。缺失或无效的清单将被视为插件错误，并阻止配置验证。
