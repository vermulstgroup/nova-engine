---
summary: "Nova Engine on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Nova Engine on DigitalOcean
  - Looking for cheap VPS hosting for Nova Engine
title: "DigitalOcean"
---

# Nova Engine on DigitalOcean

## Goal

Run a persistent Nova Engine Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status nova-engine-gateway.service

# View logs
journalctl --user -u nova-engine-gateway.service -f
```
...
```bash
nova-engine channels login whatsapp
# Scan QR code
```
...
- `~/.nova-engine/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u nova-engine --no-pager -n 50
```
