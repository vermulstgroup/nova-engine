...
This app is usually built from [`scripts/package-mac-app.sh`](https://github.com/nova-engine/nova-engine/blob/main/scripts/package-mac-app.sh), which now:

- sets a stable debug bundle identifier: `ai.nova-engine.mac.debug`
...
TCC permissions are tied to the bundle identifier _and_ code signature. Unsigned debug builds with changing UUIDs were causing macOS to forget grants after each rebuild. Signing the binaries (ad‑hoc by default) and keeping a fixed bundle id/path (`dist/Nova Engine.app`) preserves the grants between builds, matching the VibeTunnel approach.
