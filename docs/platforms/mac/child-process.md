...
service via the external `nova-engine` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`NOVA_PROFILE`; legacy `com.nova-engine.*` is supported).
