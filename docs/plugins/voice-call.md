...
read_when:
  - You want to place an outbound voice call from Nova Engine
...
Voice calls for Nova Engine via a plugin. Supports outbound notifications and
...
- Use `nova-engine voicecall ...` or the `voice_call` tool
...
```bash
nova-engine plugins install ./extensions/voice-call
