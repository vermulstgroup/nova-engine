...
read_when:
  - You want Zalo Personal (unofficial) support in Nova Engine
...
Zalo Personal support for Nova Engine via a plugin, using `zca-cli` to automate a normal Zalo user account.
...
```bash
nova-engine plugins install @nova-engine/zalouser
```
...
```bash
nova-engine plugins install ./extensions/zalouser
