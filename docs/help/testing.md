...
- Useful overrides:
  - `NOVA_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `NOVA_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Default: **enabled** by `pnpm test:live` (sets `NOVA_LIVE_TEST=1`)
- Scope:
...
- Expectations:
  ...
  - Anthropic key rotation: set `NOVA_LIVE_ANTHROPIC_KEYS="sk-...,sk-..."` (or `NOVA_LIVE_ANTHROPIC_KEY=sk-...`) or multiple `ANTHROPIC_API_KEY*` vars; tests will retry on rate limits
...
- How to enable:
  - `pnpm test:live` (or `NOVA_LIVE_TEST=1` if invoking Vitest directly)
- Set `NOVA_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `NOVA_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet/Haiku 4.5, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.1, Grok 4)
  - `NOVA_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `NOVA_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` (comma allowlist)
- How to select providers:
  - `NOVA_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `NOVA_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
...
- How to select models:
  ...
  - `NOVA_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `NOVA_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
- How to select providers (avoid “OpenRouter everything”):
  - `NOVA_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Token sources (pick one):
  - Profile: `NOVA_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test`
  - Raw token: `NOVA_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
- Model override (optional):
  - `NOVA_LIVE_SETUP_TOKEN_MODEL=anthropic/claude-opus-4-6`
...
- Overrides (optional):
  - `NOVA_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-opus-4-6"`
  - `NOVA_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.3-codex"`
  ...
  - `NOVA_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
  - `NOVA_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `NOVA_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `NOVA_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
- `NOVA_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` to keep Claude Code CLI MCP config enabled (default disables MCP config with a temporary empty file).
...
- Tool calling across several providers:
  - `NOVA_LIVE_GATEWAY_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/minimax-m2.1" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `NOVA_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
`NOVA_LIVE_GATEWAY_MODELS="openai/gpt-5.2,openai-codex/gpt-5.3-codex,anthropic/claude-opus-4-6,google/gemini-3-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,zai/glm-4.7,minimax/minimax-m2.1" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
Include at least one image-capable model in `NOVA_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.
