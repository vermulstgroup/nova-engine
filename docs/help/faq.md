...
2. Copy `$NOVA_STATE_DIR` (default: `~/.openclaw`) from the old machine.
...
Everything lives under `$NOVA_STATE_DIR` (default: `~/.openclaw`):

| Path                                                            | Purpose                                                      |
| --------------------------------------------------------------- | ------------------------------------------------------------ |
| `$NOVA_STATE_DIR/openclaw.json`                             | Main config (JSON5)                                          |
...
OpenClaw reads an optional **JSON5** config from `$NOVA_CONFIG_PATH` (default: `~/.openclaw/openclaw.json`):
...
- a global fallback `.env` from `~/.openclaw/.env` (aka `$NOVA_STATE_DIR/.env`)
...
- If you used profiles (`--profile` / `NOVA_PROFILE`), reset each state dir (defaults are `~/.openclaw-<profile>`).
...
- macOS: `$NOVA_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.openclaw/logs/...`; profiles use `~/.openclaw-<profile>/logs/...`)
