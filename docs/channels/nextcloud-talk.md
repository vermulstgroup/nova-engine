...
```bash
nova-engine plugins install @nova-engine/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
nova-engine plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Nova Engine" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `nova-engine pairing list nextcloud-talk`
  - `nova-engine pairing approve nextcloud-talk <CODE>`
