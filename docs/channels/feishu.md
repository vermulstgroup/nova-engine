...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Nova Engine to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
If you just installed Nova Engine, run the wizard:
...
2. Configuring app credentials in Nova Engine
...
1. You already ran `nova-engine channels add` for Feishu
2. The gateway is running (`nova-engine gateway status`)
...
Edit `~/.nova-engine/nova-engine.json`:
...
```bash
nova-engine pairing approve feishu <CODE>
```
...
2. Run `nova-engine logs --follow` and look for `chat_id`
...
2. Run `nova-engine logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `nova-engine gateway status`  | Show gateway status           |
| `nova-engine gateway install` | Install/start gateway service |
| `nova-engine gateway stop`    | Stop gateway service          |
| `nova-engine gateway restart` | Restart gateway service       |
| `nova-engine logs --follow`   | Tail gateway logs             |
...
4. Check logs: `nova-engine logs --follow`
...
5. Ensure the gateway is running: `nova-engine gateway status`
6. Check logs: `nova-engine logs --follow`
