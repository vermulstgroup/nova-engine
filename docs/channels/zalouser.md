...
read_when:
  - Setting up Zalo Personal for Nova Engine
...
- Install via CLI: `nova-engine plugins install @nova-engine/zalouser`
- Or from a source checkout: `nova-engine plugins install ./extensions/zalouser`
...
- `nova-engine pairing list zalouser`
- `nova-engine pairing approve zalouser <code>`
...
- On startup, Nova Engine resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `nova-engine channels logout --channel zalouser && nova-engine channels login --channel zalouser`
