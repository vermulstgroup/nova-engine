...
read_when:
  - Setting up Twitch chat integration for Nova Engine
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Nova Engine connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
nova-engine plugins install @nova-engine/twitch
```

Local checkout (when running from a git repo):

```bash
nova-engine plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `NOVA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "nova-engine", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use NOVA_TWITCH_ACCESS_TOKEN env var)
...
```bash
NOVA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
