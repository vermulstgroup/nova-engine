...
Matrix is an open, decentralized messaging protocol. Nova Engine connects as a Matrix **user**
...
```bash
nova-engine plugins install @nova-engine/matrix
```

Local checkout (when running from a git repo):

```bash
nova-engine plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `nova-engine plugins install @nova-engine/matrix`
   - From a local checkout: `nova-engine plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Nova Engine calls the same
     login endpoint, stores the access token in `~/.nova-engine/credentials/matrix/credentials.json`,
...
- On first connection, Nova Engine requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Nova Engine logs a warning.
...
`~/.nova-engine/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `nova-engine pairing list matrix`
  - `nova-engine pairing approve matrix <CODE>`
...
- On startup, Nova Engine resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
