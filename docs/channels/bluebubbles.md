...
- Nova Engine talks to it through its REST API (`GET /api/v1/ping`, `POST /message/text`, `POST /chat/:id/*`).
...
3. Run `nova-engine onboard` and select BlueBubbles, or configure manually:
...
```
nova-engine channels add bluebubbles --http-url http://192.168.1.100:1234 --password <password>
```
...
- Approve via:
  - `nova-engine pairing list bluebubbles`
  - `nova-engine pairing approve bluebubbles <CODE>`
...
- **Typing indicators**: Nova Engine sends typing start events; BlueBubbles clears typing automatically on send or timeout (manual stop via DELETE is unreliable).
...
Nova Engine may surface _short_ message IDs (e.g., `1`, `2`) to save tokens.
...
- Direct handles: `+15555550123`, `user@example.com`
  - If a direct handle does not have an existing DM chat, Nova Engine will create one via `POST /api/v1/chat/new`. This requires the BlueBubbles Private API to be enabled.
...
- Pairing codes expire after one hour; use `nova-engine pairing list bluebubbles` and `nova-engine pairing approve bluebubbles <code>`.
- Reactions require the BlueBubbles private API (`POST /api/v1/message/react`); ensure the server version exposes it.
- Edit/unsend require macOS 13+ and a compatible BlueBubbles server version. On macOS 26 (Tahoe), edit is currently broken due to private API changes.
- Group icon updates can be flaky on macOS 26 (Tahoe): the API may return success but the new icon does not sync.
- Nova Engine auto-hides known-broken actions based on the BlueBubbles server's macOS version. If edit still appears on macOS 26 (Tahoe), disable it manually with `channels.bluebubbles.actions.edit=false`.
- For status/health info: `nova-engine status --all` or `nova-engine status --deep`.
