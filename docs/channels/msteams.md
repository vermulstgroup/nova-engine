...
```bash
nova-engine plugins install @nova-engine/msteams
```

Local checkout (when running from a git repo):

```bash
nova-engine plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Nova Engine will offer the local install path automatically.
...
3. Configure Nova Engine with those credentials.
...
- Talk to Nova Engine via Teams DMs, group chats, or channels.
...
- On startup, Nova Engine resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.nova-engine/nova-engine.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Nova Engine, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `nova-engine-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `nova-engine plugins install @nova-engine/msteams`
   - From a local checkout: `nova-engine plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Nova Engine in Teams", "full": "Nova Engine in Teams" },
...
Nova Engine handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Nova Engine only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Nova Engine sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `nova-engine message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.nova-engine/msteams-polls.json`.
...
```bash
nova-engine message send --channel msteams \
...
nova-engine message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
nova-engine message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
nova-engine message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
nova-engine message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
