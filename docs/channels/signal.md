...
- Nova Engine installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Nova Engine"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure Nova Engine and restart the gateway.
5. Send a first DM and approve pairing (`nova-engine pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Nova Engine"` then scan the QR in Signal.
...
4. Configure Nova Engine, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `nova-engine pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Nova Engine at it:
...
This skips auto-spawn and the startup wait inside Nova Engine. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `nova-engine pairing list signal`
  - `nova-engine pairing approve signal <CODE>`
...
- **Typing indicators**: Nova Engine sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Nova Engine forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `nova-engine doctor --fix`.
...
nova-engine pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/nova-engine/nova-engine-$(date +%Y-%m-%d).log" | tail -20
```
