...
```bash
nova-engine channels login --channel whatsapp --account work
```
...
<Note>
Nova Engine recommends running WhatsApp on a separate number when possible. (The channel metadata and onboarding flow are optimized for that setup, but personal-number setups are also supported.)
</Note>
...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for Nova Engine
  ...
  <Accordion title="WhatsApp Web-only channel scope">
    The messaging platform channel is WhatsApp Web-based (`Baileys`) in current Nova Engine channel architecture.
...
- if `messages.responsePrefix` is unset, self-chat replies default to `[{identity.name}]` or `[nova-engine]`
...
<AccordionGroup>
  ...
  <Accordion title="Credential paths and legacy compatibility">
    - current auth path: `~/.nova-engine/credentials/whatsapp/<accountId>/creds.json`
    - backup file: `creds.json.bak`
    - legacy default auth in `~/.nova-engine/credentials/` is still recognized/migrated for default-account flows
  </Accordion>

  <Accordion title="Logout behavior">
    `nova-engine channels logout --channel whatsapp [--account <id>]` clears WhatsApp auth state for that account.
