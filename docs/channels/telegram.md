...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `nova-engine doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `nova-engine logs --follow`
...
- DM messages can carry `message_thread_id`; Nova Engine routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Nova Engine can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, Nova Engine keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), Nova Engine falls back to normal final delivery and then cleans up the preview message.

    `streamMode` is separate from block streaming. When block streaming is explicitly enabled for Telegram, Nova Engine skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Nova Engine retries as plain text.
...
```
    ...
    - `~/.nova-engine/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Nova Engine enqueues system events like:
...
```bash
nova-engine message send --channel telegram --target 123456789 --message "hi"
nova-engine message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `nova-engine channels status` warns when config expects unmentioned group messages.
    - `nova-engine channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `nova-engine logs --follow` for skip reasons
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `nova-engine doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `nova-engine doctor --fix` can resolve legacy `@username` entries to IDs.
