...
summary: Nova Engineをインストールし、数分で最初のチャットを実行しましょう。
title: はじめに
x-i18n:
...
<Info>
最速のチャット方法：Control UIを開く（チャンネル設定は不要）。`nova-engine dashboard`を実行してブラウザでチャットするか、<Tooltip headline="Gatewayホスト" tip="Nova Engine Gatewayサービスを実行しているマシン。">Gatewayホスト</Tooltip>で`http://127.0.0.1:18789/`を開きます。
...
<Steps>
  <Step title="Nova Engineをインストール（推奨）">
    <Tabs>
