...
- When given a `sessionId`, Nova Engine resolves it to the corresponding session key (missing ids error).
...
- After the primary run completes, Nova Engine runs a **reply-back loop**:
  - Round 2+ alternates between requester and target agents.
  - Reply exactly `REPLY_SKIP` to stop the ping‑pong.
  - Max turns is `session.agentToAgent.maxPingPongTurns` (0–5, default 5).
- Once the loop ends, Nova Engine runs the **agent‑to‑agent announce step** (target agent only):
...
- After completion, Nova Engine runs a sub-agent **announce step** and posts the result to the requester chat channel.
...
- When a session is sandboxed and `sessionToolsVisibility="spawned"`, Nova Engine clamps visibility to `tree` even if you set `tools.sessions.visibility="all"`.
