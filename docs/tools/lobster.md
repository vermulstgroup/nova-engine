---
title: Lobster
summary: "Typed workflow runtime for Nova Engine with resumable approval gates."
...
Lobster is a workflow shell that lets Nova Engine run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Nova Engine runs one Lobster tool call and gets a structured result.
...
Nova Engine launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Nova Engine tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Nova Engine Gateway (see the [Lobster repo](https://github.com/nova-engine/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Nova Engine keeps core tools enabled. To restrict core
...
→ nova-engine calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ nova-engine calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Nova Engine tools that do.
