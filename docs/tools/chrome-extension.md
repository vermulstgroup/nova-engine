---
summary: "Chrome extension: let Nova Engine drive your existing Chrome tab"
read_when:
...
The Nova Engine Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate nova-engine-managed Chrome profile.
...
Nova Engine then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Nova Engine release (npm package) as static files. There is no separate “build” step.

After upgrading Nova Engine:

- Re-run `nova-engine browser extension install` to refresh the installed files under your Nova Engine state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Nova Engine ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Nova Engine to control.
...
- `ON`: attached; Nova Engine can drive that tab.
...
Debugging: `nova-engine sandbox explain`
...
`nova-engine browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `nova-engine browser extension install` first to copy the extension to a stable location under your Nova Engine state directory.
...
- **This is not isolated** like the dedicated nova-engine-managed profile.
