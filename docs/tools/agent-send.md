---
summary: "Direct `nova-engine agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `nova-engine agent` (direct agent runs)

`nova-engine agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `nova-engine message --target`).
...
```bash
nova-engine agent --to +15555550123 --message "status update"
...
nova-engine agent --to +15555550123 --message "Summon reply" --deliver
