---
summary: "Nova Engine plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends Nova Engine with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core Nova Engine yet (or you want to keep optional features out of your main
install).
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@nova-engine/msteams` if you use Teams.
...
Nova Engine plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
4. Bundled extensions (shipped with Nova Engine, **disabled by default**)

- `<nova-engine>/extensions/*`

Bundled plugins must be enabled explicitly via `plugins.entries.<id>.enabled`
or `nova-engine plugins enable <id>`. Installed plugins are enabled by default,
but can be disabled the same way.

Each plugin must include a `nova-engine.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `nova-engine.extensions`:
...
Security note: `nova-engine plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `nova-engine.channel` and
install hints via `nova-engine.install`. This keeps the core catalog data-free.
...
Nova Engine can also merge **external channel catalogs** (for example, an MPM
...
Or point `NOVA_PLUGIN_CATALOG_PATHS` (or `NOVA_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "nova-engine": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, Nova Engine uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `nova-engine.plugin.json` (`configSchema`).
...
Nova Engine augments `uiHints` at runtime based on discovered plugins:
...
Plugins may also register their own top‑level commands (example: `nova-engine voicecall`).
...
- Plugin-managed hooks show up in `nova-engine hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `nova-engine hooks`; enable/disable the plugin instead.

## Provider plugins (model auth)

Plugins can register **model provider auth** flows so users can run OAuth or
API-key setup inside Nova Engine (no external scripts needed).
...
- `config`: The current Nova Engine config
...
- Main package: `nova-engine` (this repo)
- Plugins: separate npm packages under `@nova-engine/*` (example: `@nova-engine/voice-call`)

Publishing contract:

- Plugin `package.json` must include `nova-engine.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `nova-engine plugins install <npm-spec>` uses `npm pack`, extracts into `~/.nova-engine/extensions/<id>/`, and enables it in config.
...
- Separately published plugins should run their own CI (lint/build/test) and validate `nova-engine.extensions` points at the built entrypoint (`dist/index.js`).
