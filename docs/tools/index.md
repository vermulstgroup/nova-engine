---
summary: "Agent tool surface for Nova Engine (browser, canvas, nodes, message, cron) replacing legacy `nova-engine-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `nova-engine-*` skills
title: "Tools"
---

# Tools (Nova Engine)

Nova Engine exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `nova-engine-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `nova-engine.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Nova Engine logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:nova-engine`: all built-in Nova Engine tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`nova-engine node run`).
...
- Requires a Brave API key (recommended: `nova-engine configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Nova Engine-managed browser.
...
- Quick smoke: `nova-engine nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `nova-engine gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Nova Engine clamps `tools.sessions.visibility` to `tree`.
