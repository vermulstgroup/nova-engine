...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `nova-engine sandbox explain`.
...
Legacy `agent.*` configs are migrated by `nova-engine doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=nova-engine-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${NOVA_STATE_DIR:-$HOME/.nova-engine}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
