...
- **gateway host** → `nova-engine` process on the gateway machine
...
`~/.nova-engine/exec-approvals.json` directly.

CLI: `nova-engine approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
