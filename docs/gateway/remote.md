...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Nova Engine runs”).
...
- `nova-engine health` and `nova-engine status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `nova-engine gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`NOVA_GATEWAY_PORT`).
