...
Nova Engine has two distinct problems that look similar on the surface:

1. **Operator remote control**: the macOS menu bar app controlling a gateway running elsewhere.
2. **Node pairing**: iOS/Android (and future nodes) finding a gateway and pairing securely.

The design goal is to keep all network discovery/advertising in the **Node Gateway** (`nova-engine gateway`) and keep clients (mac app, iOS) as consumers.
...
- Service types:
  - `_nova-engine-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `nova-engine` entrypoint or binary)
...
- `NOVA_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.nova-engine/nova-engine.json` controls the Gateway bind mode.
- `NOVA_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `NOVA_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `NOVA_CLI_PATH` overrides the advertised CLI path.
