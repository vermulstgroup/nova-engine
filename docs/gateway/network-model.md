...
Most operations flow through the Gateway (`nova-engine gateway`), a single long-running
...
- Loopback first: the Gateway WS defaults to `ws://127.0.0.1:18789`. The wizard generates a gateway token by default, even for loopback. For tailnet access, run `nova-engine gateway --bind tailnet --token ...` because tokens are required for non-loopback binds.
...
- Canvas host is served by the Gateway HTTP server on the **same port** as the Gateway (default `18789`):
  - `/__nova-engine__/canvas/`
  - `/__nova-engine__/a2ui/`
