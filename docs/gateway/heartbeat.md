...
- During heartbeat runs, Nova Engine treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `nova-engine sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Nova Engine skips the heartbeat run entirely (no model call).
...
```bash
nova-engine system event --text "Check for urgent follow-ups" --mode now
```
