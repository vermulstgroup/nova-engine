...
Nova Engine can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Nova Engine requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. Nova Engine verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Nova Engine only treats a request as Serve when it arrives from loopback with
...
- Set `gateway.tailscale.resetOnExit` if you want Nova Engine to undo `tailscale serve`
