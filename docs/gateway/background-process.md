...
Nova Engine runs shell commands through the `exec` tool and keeps long‑running tasks in memory. The `process` tool manages those background sessions.
...
- `NOVA_BASH_PENDING_MAX_OUTPUT_CHARS`: pending stdout/stderr cap per stream (chars)
