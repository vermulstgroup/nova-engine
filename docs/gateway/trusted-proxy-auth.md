...
read_when:
  - Running Nova Engine behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Nova Engine
...
- You run Nova Engine behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Nova Engine checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Nova Engine extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://nova-engine.example.com
    to: http://nova-engine-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
nova-engine.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy nova-engine:18789 {
...
location / {
    ...
    proxy_pass http://nova-engine:18789;
...
`nova-engine security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Nova Engine config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `nova-engine security audit` and review findings
