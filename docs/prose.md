---
summary: "OpenProse: .prose workflows, slash commands, and state in Nova Engine"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In Nova Engine it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
nova-engine plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `nova-engine plugins install ./extensions/open-prose`

Related docs: [Plugins](/tools/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses Nova Engine tools under the hood.
...
## Nova Engine runtime mapping

OpenProse programs map to Nova Engine primitives:

| OpenProse concept         | Nova Engine tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use Nova Engine tool allowlists and approval gates to control side effects.
