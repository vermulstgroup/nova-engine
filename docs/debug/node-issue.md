...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Nova Engine
...
Running Nova Engine via Node with `tsx` fails at startup with:

```
[nova-engine] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `nova-engine status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch nova-engine.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node nova-engine.mjs status` works on Node 25.
