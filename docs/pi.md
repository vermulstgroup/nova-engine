...
This document describes how Nova Engine integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Nova Engine uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Nova Engine directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Nova Engine's local TUI mode)                                             |
...
├── nova-engine-tools.ts              # Nova Engine-specific tools
...
2. **Custom Replacements**: Nova Engine replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Nova Engine Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Nova Engine's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Nova Engine CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Nova Engine wraps this with `guardSessionManager()` for tool result safety.
...
Nova Engine maintains an auth profile store with multiple API keys per provider:
...
Nova Engine loads custom pi extensions for specialized behavior:
...
Nova Engine also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Nova Engine Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Nova Engine tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.nova-engine/agents/<agentId>/sessions/` (or `$NOVA_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-nova-engine-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-nova-engine-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-nova-engine-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-nova-engine-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
