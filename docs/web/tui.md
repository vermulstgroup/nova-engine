...
```bash
nova-engine tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `nova-engine tui --deliver`
...
- Check the Gateway logs: `nova-engine logs --follow`.
- Confirm the agent can run: `nova-engine status` and `nova-engine models status`.
...
- No agents in picker: check `nova-engine agents list` and your routing config.
