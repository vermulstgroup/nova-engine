...
Nova Engine features that can generate provider usage or paid API calls.
...
- `nova-engine status --usage` and `nova-engine channels list` show provider **usage windows**
...
Nova Engine can pick up credentials from:
...
- `nova-engine models status --json`
...
`nova-engine models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
