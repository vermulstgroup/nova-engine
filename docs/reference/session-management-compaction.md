...
This document explains how Nova Engine manages sessions end-to-end:
...
Nova Engine is designed around a single **Gateway process** that owns session state.
...
Nova Engine persists sessions in two layers:
...
- Transcripts: `~/.nova-engine/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Nova Engine resolves these via `src/config/sessions.ts`.
...
Nova Engine intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Nova Engine consumes the events, but Pi decides when to compact).
...
Nova Engine also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Nova Engine bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Nova Engine leaves it alone.
...
- `nova-engine status` (CLI)
- `nova-engine sessions` / `sessions --json`
...
Nova Engine supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Nova Engine strips/suppresses this in the delivery layer.

As of `2026.1.10`, Nova Engine also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Nova Engine uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Nova Engine’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `nova-engine status`.
