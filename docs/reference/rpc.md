...
Nova Engine integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Nova Engine owns lifecycle when `channels.signal.autoStart=true`.
...
- Nova Engine spawns `imsg rpc` as a child process (legacy iMessage integration).
