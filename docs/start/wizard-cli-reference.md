...
read_when:
  - You need detailed behavior for nova-engine onboard
...
This page is the full reference for `nova-engine onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.nova-engine/nova-engine.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `nova-engine doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `nova-engine pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `nova-engine health`.
    - `nova-engine status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.nova-engine/.env` so launchd can read it.
...
- OAuth credentials: `~/.nova-engine/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.nova-engine/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.nova-engine/credentials/oauth.json` (or `$NOVA_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
