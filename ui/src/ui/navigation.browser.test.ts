...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/nova-engine/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/nova-engine");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/nova-engine/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__NOVA_CONTROL_UI_BASE_PATH__ = "/nova-engine";
    const app = mountApp("/nova-engine/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/nova-engine");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/nova-engine/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "nova-engine.control.settings.v1",
