...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".nova-engine");
  const targetPath = path.join(targetDir, "nova-engine.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("nova-engine doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("nova-engine doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("nova-engine doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "nova-engine doctor --fix" to remove these keys.');
    }
  }
