...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.NOVA_UPDATE_IN_PROGRESS);
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("nova-engine update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
