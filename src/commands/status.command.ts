...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Nova Engine status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("nova-engine security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("nova-engine security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.nova-engine.ai/faq");
  runtime.log("Troubleshooting: https://docs.nova-engine.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("nova-engine status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("nova-engine logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("nova-engine status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("nova-engine gateway probe")}`);
  }
}
