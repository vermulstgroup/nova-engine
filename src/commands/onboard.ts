...
export async function onboardCommand(opts: OnboardOptions, runtime: RuntimeEnv = defaultRuntime) {
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    runtime.error(
      [
        ...
        "Read: https://docs.nova-engine.ai/security",
        `Re-run with: ${formatCliCommand("nova-engine onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected — Nova Engine runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://docs.nova-engine.ai/windows",
      ].join("\n"),
    );
  }
