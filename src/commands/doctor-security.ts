    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("nova-engine configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("nova-engine config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("nova-engine doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("nova-engine config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('nova-engine config set session.dmScope "per-channel-peer"') +
