...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("nova-engine models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("nova-engine models auth setup-token")} or ${formatCliCommand(
  ...
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "nova-engine models auth login --provider openai-codex",
    )} or ${formatCliCommand("nova-engine configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("nova-engine configure")}\` or \`${formatCliCommand("nova-engine onboard")}\`.`;
}
