...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("nova-engine models auth setup-token")}\` or \`${formatCliCommand("nova-engine configure")}\`.`
          : `Run \`${formatCliCommand("nova-engine configure")}\` or set an API key env var.`;
