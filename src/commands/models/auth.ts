...
export function resolveRequestedLoginProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("nova-engine plugins list --json")}\`.`,
  );
}
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("nova-engine plugins install")}\`.`,
    );
  }
