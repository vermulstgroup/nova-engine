...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("nova-engine pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`nova-engine pairing approve ${policy.channel} <code>`)}`,
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("nova-engine channels add")}\``,
        },
      ],
