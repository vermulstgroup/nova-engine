...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.NOVA_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.NOVA_GATEWAY_PASSWORD;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from nova-engine.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Nova Engine update wizard" : "Nova Engine configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.nova-engine.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("nova-engine doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.NOVA_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.NOVA_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.NOVA_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.NOVA_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.NOVA_GATEWAY_TOKEN;
