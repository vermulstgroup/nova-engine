...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".nova-engine"].map((dir) => path.resolve(root, entry.name, dir));
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".nova-engine");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.nova-engine out of git; it contains credentials and session history.",
