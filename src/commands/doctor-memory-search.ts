...
export async function noteMemorySearchHealth(cfg: Nova EngineConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("nova-engine config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("nova-engine memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`nova-engine auth add --provider ${resolved.provider}`)}`,
        `- To disable: ${formatCliCommand("nova-engine config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("nova-engine memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("nova-engine auth add --provider openai")}`,
