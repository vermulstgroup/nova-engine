...
const SEARCH_TOOL = "https://docs.nova-engine.ai/mcp.SearchNova Engine";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.nova-engine.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('nova-engine docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.nova-engine.ai/");
      runtime.log(`Search: ${formatCliCommand('nova-engine docs "your query"')}`);
    }
    return;
  }
