...
export async function doctorCommand(
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("nova-engine configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("nova-engine config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("nova-engine setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("nova-engine doctor --fix")}" to apply changes.`);
  }
