...
describe("logger helpers", () => {
  ...
  it("uses daily rolling default log file and prunes old ones", () => {
    ...
    const todayPath = path.join(DEFAULT_LOG_DIR, `nova-engine-${today}.log`);

    // create an old file to be pruned
    const oldPath = path.join(DEFAULT_LOG_DIR, "nova-engine-2000-01-01.log");
...
function pathForTest() {
  const file = path.join(os.tmpdir(), `nova-engine-log-${crypto.randomUUID()}.log`);
