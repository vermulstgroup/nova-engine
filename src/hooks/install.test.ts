...
const fixtureRoot = path.join(os.tmpdir(), `nova-engine-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@nova-engine/test-hooks",
        version: "0.0.1",
        nova-engine: { hooks: ["./hooks/one-hook"] },
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"nova-engine":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@nova-engine/test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(argv).toEqual(["npm", "pack", "@nova-engine/test-hooks@0.0.1", "--ignore-scripts"]);
