...
`;
    ...
    expect(parsed.nova-engine.emoji).toBe("💾");
    expect(parsed.nova-engine.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.nova-engine.emoji).toBe("📝");
    expect(parsed.nova-engine.events).toEqual(["command"]);
    expect(parsed.nova-engine.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.nova-engine.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"nova-engine": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"nova-engine": {"events": ["test"]}}');
  });
...
describe("resolveNova EngineMetadata", () => {
  ...
  it("returns undefined when nova-engine key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        nova-engine: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Nova Engine" },
            { id: "npm", kind: "npm", package: "@nova-engine/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@nova-engine/hook");
  });
...
`;
    ...
    expect(nova-engine?.requires?.config).toEqual(["workspace.dir"]);
    expect(nova-engine?.install?.[0].kind).toBe("bundled");
  });
