...
homepage: https://docs.nova-engine.ai/automation/hooks#command-logger
metadata:
  {
    "nova-engine":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.nova-engine/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.nova-engine/logs/commands.log ~/.nova-engine/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/nova-engine`:
   ```
   /home/username/.nova-engine/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
