...
This directory contains hooks that ship with Nova Engine. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.nova-engine/nova-engine.json`:
...
2. Restart gateway: `pkill -9 -f 'nova-engine.*gateway' && pnpm nova-engine gateway`
3. Enable the hook: `nova-engine hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.nova-engine.ai/automation/hooks
