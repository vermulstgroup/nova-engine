...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses NOVA_HOME for tilde expansion", () => {
    vi.stubEnv("NOVA_HOME", "/srv/nova-engine-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/nova-engine-home", "cron", "jobs.json"));
  });
});
