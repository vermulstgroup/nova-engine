...
vi.mock("../commands/onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "/tmp/nova-engine-workspace",
...
describe("runOnboardingWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-onboard-suite-"));
  });
  ...
  it("exits when config is invalid", async () => {
    readConfigFileSnapshot.mockResolvedValueOnce({
      path: "/tmp/.nova-engine/nova-engine.json",
      exists: true,
      raw: "{}",
      parsed: {},
      valid: false,
