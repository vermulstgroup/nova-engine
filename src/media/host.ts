...
export async function ensureMediaHosted(
  ...
  if (needsServerStart && !opts.startServer) {
    ...
    throw new Error(
      `Media hosting requires the webhook/Funnel server. Start \`${formatCliCommand("nova-engine webhook")}\`/\`${formatCliCommand("nova-engine up")}\` or re-run with --serve-media.`,
    );
  }
  if (needsServerStart && opts.startServer) {
    if (!mediaServer) {
