...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { NOVA_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { NOVA_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports NOVA_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { NOVA_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { NOVA_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { NOVA_SMOKE: "nope" })).toBe("qr");
  });
});
