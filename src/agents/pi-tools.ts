...
function resolveExecConfig(params: { cfg?: NovaEngineConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: NovaEngineConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createNovaEngineCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createNovaEngineReadTool(freshReadTool);
