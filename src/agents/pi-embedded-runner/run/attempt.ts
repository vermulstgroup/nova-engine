...
import { resolveNovaEngineDocsPath } from "../../docs-path.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveNovaEngineAgentDir();

    // Check if the model supports native image input
    const modelHasVision = params.model.input?.includes("image") ?? false;
    const toolsRaw = params.disableTools
      ? []
      : createNovaEngineCodingTools({
          exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveNovaEngineDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/nova-engine/nova-engine/issues/9282
      ...
      // See: https://github.com/nova-engine/nova-engine/issues/8643
      await flushPendingToolResultsAfterIdle({
        agent: session?.agent,
        sessionManager,
      });
