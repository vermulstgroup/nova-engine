...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("nova-engine-ws-", async (workspaceDir) => {
      await withTempDir("nova-engine-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createNova EngineCodingTools({ workspaceDir });
    ...
    await withTempDir("nova-engine-ws-", async (workspaceDir) => {
      await withTempDir("nova-engine-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello nova-engine");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("nova-engine-ws-", async (workspaceDir) => {
      await withTempDir("nova-engine-override-", async (overrideDir) => {
        const tools = createNova EngineCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("nova-engine-sandbox-", async (sandboxDir) => {
      await withTempDir("nova-engine-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createNova EngineCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
