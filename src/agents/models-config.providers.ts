...
type ModelsConfig = NonNullable<Nova EngineConfig["models"]>;
...
export async function resolveImplicitCopilotProvider(params: {
  ...
  // Our Copilot auth lives in Nova Engine's auth-profiles store instead, so we also
  // write a runtime-only auth.json entry for pi-coding-agent to pick up.
  //
  // This is safe because it's (1) within Nova Engine's agent dir, (2) contains the
  // GitHub token (not the exchanged Copilot token), and (3) matches existing
  // patterns for OAuth-like providers in pi-coding-agent.
  // Note: we deliberately do not write pi-coding-agent's `auth.json` here.
  // Nova Engine uses its own auth store and exchanges tokens at runtime.
  // `models list` uses Nova Engine's auth heuristics for availability.
