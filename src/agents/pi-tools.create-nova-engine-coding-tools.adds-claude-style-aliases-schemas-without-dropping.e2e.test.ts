...
describe("createNovaEngineCodingTools", () => {
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "nova-engine-outside.txt");
