...
const SETUP_TOKEN_RAW = process.env.NOVA_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.NOVA_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.NOVA_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const SETUP_TOKEN_MODEL = process.env.NOVA_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "NOVA_LIVE_SETUP_TOKEN did not look like a setup-token. Use NOVA_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set NOVA_LIVE_SETUP_TOKEN_VALUE or NOVA_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureNova EngineModelsJson(cfg, tokenSource.agentDir);
