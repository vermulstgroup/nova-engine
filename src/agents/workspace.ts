...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.NOVA_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".nova-engine", `workspace-${profile}`);
  }
  return path.join(home, ".nova-engine", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".nova-engine";
