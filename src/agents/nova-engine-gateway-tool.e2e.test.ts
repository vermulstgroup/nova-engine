...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const envSnapshot = captureEnv(["NOVA_STATE_DIR", "NOVA_PROFILE"]);
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-test-"));
    ...
    try {
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: nova-engine --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/nova-engine" } }\n}\n';
