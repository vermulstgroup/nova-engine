...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses NOVA_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NOVA_HOME: "/srv/nova-engine-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/nova-engine-home"), ".nova-engine", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".nova-engine", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("nova-engine-workspace-");
