...
function makeMissingToolResult(params: {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: "[nova-engine] missing tool result in session history; inserted synthetic error result for transcript repair.",
      },
    ],
    isError: true,
    timestamp: Date.now(),
...
export function repairToolUseResultPairing(messages: AgentMessage[]): ToolUseRepairReport {
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    // See: https://github.com/nova-engine/nova-engine/issues/4597
