...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-engine-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nova-engine-auth-merge-"));
    const previousAgentDir = process.env.NOVA_AGENT_DIR;
    ...
    try {
      ...
      process.env.NOVA_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.NOVA_AGENT_DIR = previousAgentDir;
      }
