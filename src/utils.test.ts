...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "nova-engine-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-engine-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "nova-engine-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "nova-engine-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.nova-engine when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "nova-engine-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers NOVA_HOME over HOME", () => {
    vi.stubEnv("NOVA_HOME", "/srv/nova-engine-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/nova-engine-home"));
...
describe("shortenHomePath", () => {
  it("uses $NOVA_HOME prefix when NOVA_HOME is set", () => {
    vi.stubEnv("NOVA_HOME", "/srv/nova-engine-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/nova-engine-home")}/.nova-engine/nova-engine.json`)).toBe(
      "$NOVA_HOME/.nova-engine/nova-engine.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $NOVA_HOME replacement when NOVA_HOME is set", () => {
    vi.stubEnv("NOVA_HOME", "/srv/nova-engine-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/nova-engine-home")}/.nova-engine/nova-engine.json`),
    ).toBe("config: $NOVA_HOME/.nova-engine/nova-engine.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/nova-engine")).toBe(path.resolve(os.homedir(), "nova-engine"));
  });
  ...
  it("prefers NOVA_HOME for tilde expansion", () => {
    vi.stubEnv("NOVA_HOME", "/srv/nova-engine-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/nova-engine")).toBe(path.resolve("/srv/nova-engine-home", "nova-engine"));
