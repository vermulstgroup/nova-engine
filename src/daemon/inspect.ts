...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "nova-engine" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["nova-engine", "clawdbot", "moltbot"] as const;

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.NOVA_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["nova-engine_service_marker"];
  const kindKeys = ["nova-engine_service_kind"];
...
function isNova EngineGatewaySystemdService(name: string, contents: string): boolean {
  ...
  if (!name.startsWith("nova-engine-gateway")) {
    return false;
  }
...
function isNova EngineGatewayTaskName(name: string): boolean {
  ...
  return normalized === defaultName || normalized.startsWith("nova-engine gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "nova-engine" || isLegacyLabel(label),
    });
  }
