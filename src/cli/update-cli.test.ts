...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-update-tests-"));
  });
  ...
  beforeEach(() => {
    ...
    readPackageName.mockResolvedValue("nova-engine");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Nova Engine update status");
  });
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("nova-engine-update-wizard");
    const envSnapshot = captureEnv(["NOVA_GIT_DIR"]);
    try {
      setTty(true);
      process.env.NOVA_GIT_DIR = tempDir;
