...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `nova-engine logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("nova-engine doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.nova-engine.ai/cli/logs")}\n`,
    );
