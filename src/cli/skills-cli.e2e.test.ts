...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["NOVA_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-engine-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-engine-bundled-skills-test-"));
    process.env.NOVA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
