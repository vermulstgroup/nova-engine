...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("nova-engine skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendClawHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("nova-engine skills list")}\` to see available skills.`,
      opts.json,
    );
  }
