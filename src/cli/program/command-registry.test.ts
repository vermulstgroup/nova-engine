...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = new Command();
    registerCoreCliCommands(program, testProgramContext, ["node", "nova-engine", "doctor"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = new Command();
    registerCoreCliCommands(program, testProgramContext, ["node", "nova-engine", "vitest"]);

    const prevArgv = process.argv;
    process.argv = ["node", "nova-engine", "status"];
    try {
      program.exitOverride();
      await program.parseAsync(["node", "nova-engine", "status"]);
    } finally {
