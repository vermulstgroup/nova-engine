...
${formatHelpExamples([
  ['nova-engine agent --to +15555550123 --message "status update"', "Start a new session."],
  ['nova-engine agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'nova-engine agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'nova-engine agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['nova-engine agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.nova-engine.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.nova-engine.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['nova-engine agents set-identity --agent main --name "Nova Engine" --emoji "🦞"', "Set name + emoji."],
  ["nova-engine agents set-identity --agent main --avatar avatars/nova-engine.png", "Set avatar path."],
  [
    "nova-engine agents set-identity --workspace ~/.nova-engine/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "nova-engine agents set-identity --identity-file ~/.nova-engine/workspace/IDENTITY.md --agent main",
