...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Nova Engine Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(nova-engine completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# Nova Engine Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(nova-engine completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $NOVA_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "nova-engine") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c nova-engine -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'nova-engine gateway' and not 'nova-engine other gateway', we need to check the sequence.
