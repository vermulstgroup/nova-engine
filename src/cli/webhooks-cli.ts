...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.nova-engine.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Nova Engine hooks")
    ...
    .option("--hook-url <url>", "Nova Engine hook URL")
    .option("--hook-token <token>", "Nova Engine hook token")
