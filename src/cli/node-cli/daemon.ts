...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("nova-engine node install"), formatCliCommand("nova-engine node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("nova-engine node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    NOVA_LOG_PREFIX: baseEnv.NOVA_LOG_PREFIX ?? "node",
