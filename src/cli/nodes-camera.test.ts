...
describe("nodes camera helpers", () => {
  ...
  it("builds stable temp paths when id provided", () => {
    ...
    expect(p).toBe(path.join("/tmp", "nova-engine-camera-snap-front-id1.jpg"));
  });

  it("writes base64 to file", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-test-"));
...
describe("nodes screen helpers", () => {
  ...
  it("builds screen record temp path", () => {
    ...
    expect(p).toBe(path.join("/tmp", "nova-engine-screen-record-id1.mp4"));
  });
});
