...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall Nova Engine and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “Nova Engine Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.nova-engine.ai/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
    ...
    .action(async (_opts, cmd) => {
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Chrome extension is not installed. Run: "${formatCliCommand("nova-engine browser extension install")}"`,
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.nova-engine.ai/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
