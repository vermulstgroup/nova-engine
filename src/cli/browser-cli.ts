...
export function registerBrowserCli(program: Command) {
  const browser = program
    .command("browser")
    .description("Manage Nova Engine's dedicated browser (Chrome/Chromium)")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples(
          ...
          "/cli/browser",
          "docs.nova-engine.ai/cli/browser",
        )}\n`,
    )
    .action(() => {
      browser.outputHelp();
      defaultRuntime.error(
        danger(`Missing subcommand. Try: "${formatCliCommand("nova-engine browser status")}"`),
      );
