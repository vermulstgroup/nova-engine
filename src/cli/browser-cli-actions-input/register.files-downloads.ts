...
export function registerBrowserFilesAndDownloadsCommands(
  ...
  browser
    ...
    .argument(
      "<paths...>",
      "File paths to upload (must be within Nova Engine temp uploads dir, e.g. /tmp/nova-engine/uploads/file.pdf)",
    )
  ...
  browser
    ...
    .argument(
      "[path]",
      "Save path within nova-engine temp downloads dir (default: /tmp/nova-engine/downloads/...; fallback: os.tmpdir()/nova-engine/downloads/...)",
    )
    .option("--target-id <id>", "CDP target id (or unique prefix)")
    .option(
      "--timeout-ms <ms>",
  ...
  browser
    ...
    .argument(
      "<path>",
      "Save path within nova-engine temp downloads dir (e.g. report.pdf or /tmp/nova-engine/downloads/report.pdf)",
    )
    .option("--target-id <id>", "CDP target id (or unique prefix)")
    .option(
      "--timeout-ms <ms>",
