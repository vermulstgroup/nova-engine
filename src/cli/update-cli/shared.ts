...
const NOVA_REPO_URL = "https://github.com/nova-engine/nova-engine.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "nova-engine";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("nova-engine@")) {
    return trimmed.slice("nova-engine@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.NOVA_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveNova EnginePackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", NOVA_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `NOVA_GIT_DIR points at a non-git directory: ${params.dir}. Set NOVA_GIT_DIR to an empty folder or an nova-engine checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`NOVA_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "nova-engine.mjs");
