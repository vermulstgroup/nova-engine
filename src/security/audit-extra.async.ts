...
import type { Nova EngineConfig, ConfigFileSnapshot } from "../config/config.js";
...
export async function collectPluginsCodeSafetyFindings(params: {
  ...
  const entries = await fs.readdir(extensionsDir, { withFileTypes: true }).catch((err) => {
    findings.push({
      ...
      remediation:
        "Check file permissions and plugin layout, then rerun `nova-engine security audit --deep`.",
    });
    return [];
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all nova-engine.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Nova Engine extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (entry.skill.source === "nova-engine-bundled") {
        continue;
      }
      ...
      const summary = await skillScanner.scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `nova-engine security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
