...
/**
 * Nix mode detection: When NOVA_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
/** Build a homedir thunk that respects NOVA_HOME for the given env. */
...
export function resolveStateDir(
  ...
  const override = env.NOVA_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
...
export function resolveCanonicalConfigPath(
  ...
  const override = env.NOVA_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const override = env.NOVA_CONFIG_PATH?.trim();
  if (override) {
    return resolveUserPath(override, env, homedir);
  }
  const stateOverride = env.NOVA_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.NOVA_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (explicit) {
 ...
 *
 * Precedence:
 * - `NOVA_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.NOVA_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
