...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.nova-engine/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/nova-engine/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/nova-engine/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/nova-engine/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/nova-engine/nickserv-password.txt");
