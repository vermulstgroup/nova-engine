...
const ETC_NOVA_DIR = path.join(ROOT_DIR, "etc", "nova-engine");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "nova-engine.json");
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "nova-engine.json"))).toEqual({
      shared: true,
    });
  });
});
