  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.nova-engine/logs/nova-engine.log" },
        hooks: {
          path: "~/.nova-engine/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.nova-engine/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".nova-engine", "logs", "nova-engine.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".nova-engine", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".nova-engine", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".nova-engine", "agents", "main"));
