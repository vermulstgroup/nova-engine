...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-nova-engine",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-nova-engine#general");
  });
  ...
  it("derives session transcripts dir from NOVA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { NOVA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.NOVA_STATE_DIR = "/custom/state";
    try {
