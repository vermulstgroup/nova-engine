...
function warnIfConfigFromFuture(cfg: NovaEngineConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Nova Engine (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  async function writeConfigFile(cfg: NovaEngineConfig, options: ConfigWriteOptions = {}) {
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.NOVA_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.NOVA_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.NOVA_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.NOVA_WATCH_SESSION === "string" &&
        deps.env.NOVA_WATCH_SESSION.trim().length > 0
          ? deps.env.NOVA_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.NOVA_WATCH_COMMAND === "string" &&
        deps.env.NOVA_WATCH_COMMAND.trim().length > 0
          ? deps.env.NOVA_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `NOVA_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NOVA_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.NOVA_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
