...
describe("resolveTelegramFetch", () => {
  ...
  it("honors env enable override", async () => {
    vi.stubEnv("NOVA_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY", "1");
  ...
  it("env disable override wins over config", async () => {
    vi.stubEnv("NOVA_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY", "0");
    vi.stubEnv("NOVA_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY", "1");
    globalThis.fetch = vi.fn(async () => ({})) as unknown as typeof fetch;
    resolveTelegramFetch(undefined, { network: { autoSelectFamily: true } });
