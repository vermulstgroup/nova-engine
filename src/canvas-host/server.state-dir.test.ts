...
describe("canvas host state dir defaults", () => {
  ...
  it("uses NOVA_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Nova Engine Canvas");
    } finally {
      await handler.close();
