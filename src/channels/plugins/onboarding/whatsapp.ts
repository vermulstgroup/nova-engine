...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Nova Engine can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppAllowFrom(
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Nova Engine" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("nova-engine channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
