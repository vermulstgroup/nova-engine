...
import { scheduleGatewaySigusr1Restart, triggerNova EngineRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Nova Engine in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerNova EngineRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Nova Engine via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
