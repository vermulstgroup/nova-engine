...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-commands-"));
...
function buildParams(commandBody: string, cfg: Nova EngineConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/nova-engine-session-store.json" },
    } as Nova EngineConfig;
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes Slack DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    const written = writeConfigFileMock.mock.calls[0]?.[0] as Nova EngineConfig;
