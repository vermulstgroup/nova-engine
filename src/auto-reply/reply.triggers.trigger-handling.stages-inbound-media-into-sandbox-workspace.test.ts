...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(async (home) => await fn(home), { prefix: "nova-engine-triggers-" });
}
...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withTempHome(async (home) => {
      const inboundDir = join(home, ".nova-engine", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "nova-engine"),
      });
