...
const RELAY_AUTH_HEADER = "x-nova-engine-relay-token";
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Nova Engine-Extension-Relay",
          revision: "0",
          userAgent: "Nova Engine-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "Nova Engine/extension-relay",
