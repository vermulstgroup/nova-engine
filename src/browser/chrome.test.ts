...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "nova-engine-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    expect(def.name).toBe(DEFAULT_NOVA_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_NOVA_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".nova-engine-profile-decorated"),
      "utf-8",
    );
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_NOVA_BROWSER_PROFILE_NAME);
  });
});
