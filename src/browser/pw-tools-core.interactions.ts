  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-nova-engine-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-nova-engine-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-nova-engine-labels", "1");
          ...
          tag.setAttribute("data-nova-engine-labels", "1");
