...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    ...
    // Start with only nova-engine profile
    ...
    // 2. Simulate adding a new profile to config (like user editing nova-engine.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles.nova-engine = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles.nova-engine?.cdpPort).toBe(19999);
  });
