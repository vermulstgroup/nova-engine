...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const nova-engine = resolveProfile(resolved, "nova-engine");
    expect(nova-engine?.driver).toBe("nova-engine");
    expect(nova-engine?.cdpPort).toBe(18800);
    expect(nova-engine?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from NOVA_GATEWAY_PORT when unset", () => {
    ...
    try {
      ...
      const nova-engine = resolveProfile(resolved, "nova-engine");
      expect(nova-engine?.cdpPort).toBe(19012);
      expect(nova-engine?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const nova-engine = resolveProfile(resolved, "nova-engine");
      expect(nova-engine?.cdpPort).toBe(19022);
      expect(nova-engine?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.NOVA_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "nova-engine");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        nova-engine: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("nova-engine");
  });
});
