...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("nova-engine");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const nova-engine = ctx.forProfile("nova-engine");

    const opened = await nova-engine.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("nova-engine")?.lastTargetId).toBe("CREATED");
  });
});
