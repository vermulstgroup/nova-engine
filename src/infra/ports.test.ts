...
describe("ports helpers", () => {
  ...
  it("prints an Nova Engine-specific hint when port details look like another Nova Engine instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js nova-engine gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another Nova Engine instance is already running");
  });
  ...
  it("classifies ssh and gateway listeners", () => {
    ...
    expect(
      classifyPortListener(
        {
          commandLine: "node /Users/me/Projects/nova-engine/dist/entry.js gateway",
        },
        18789,
      ),
    ).toBe("gateway");
