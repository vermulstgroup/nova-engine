...
describe("delivery-queue", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-engine-dq-test-"));
  });
...
describe("resolveOutboundSessionRoute", () => {
  const baseConfig = {} as Nova EngineConfig;
  ...
  it("treats Telegram usernames as DMs when unresolved", async () => {
    const cfg = { session: { dmScope: "per-channel-peer" } } as Nova EngineConfig;
    const route = await resolveOutboundSessionRoute({
      cfg,
      channel: "telegram",
      agentId: "main",
