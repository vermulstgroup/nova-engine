...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.NOVA_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Nova Engine. You can keep running the web gateway without it: \`${formatCliCommand("nova-engine gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
