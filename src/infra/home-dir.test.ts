...
describe("resolveEffectiveHomeDir", () => {
  it("prefers NOVA_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/nova-engine-home"),
    );
  });
  ...
  it("expands NOVA_HOME when set to ~", () => {
    const env = {
      NOVA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for NOVA_HOME", () => {
    const result = resolveRequiredHomeDir(
      { NOVA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when NOVA_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ NOVA_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { NOVA_HOME: "/srv/nova-engine-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/nova-engine-home")}/x`);
  });
