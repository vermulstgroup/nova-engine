...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.NOVA_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.NOVA_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.NOVA_TEST_TAILSCALE_BINARY;
    } else {
      process.env.NOVA_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
