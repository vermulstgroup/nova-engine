...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      domain: "nova-engine.internal.",
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (Nova Engine)",
    ...
    expect(txt).toContain(`$ORIGIN nova-engine.internal.`);
    ...
    expect(txt).toContain(`_nova-engine-gw._tcp IN PTR studio-london._nova-engine-gw._tcp`);
    expect(txt).toContain(`studio-london._nova-engine-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Nova Engine)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/nova-engine`);
  });
