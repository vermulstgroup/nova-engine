...
describe("shell env fallback", () => {
  it("is disabled by default", () => {
    ...
    expect(shouldEnableShellEnvFallback({ NOVA_LOAD_SHELL_ENV: "0" })).toBe(false);
    expect(shouldEnableShellEnvFallback({ NOVA_LOAD_SHELL_ENV: "1" })).toBe(true);
  });
  ...
  it("resolves timeout from env with default fallback", () => {
    ...
    expect(resolveShellEnvFallbackTimeoutMs({ NOVA_SHELL_ENV_TIMEOUT_MS: "42" })).toBe(42);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        NOVA_SHELL_ENV_TIMEOUT_MS: "nope",
      }),
    ).toBe(15000);
  });
