...
const DEFAULT_PACKAGE_NAME = "nova-engine";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "nova-engine.mjs");
    ...
    const doctorStep = await runStep(
      step("nova-engine doctor", doctorArgv, gitRoot, { NOVA_UPDATE_IN_PROGRESS: "1" }),
    );
