...
describe("runGatewayUpdate", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-update-"));
  });
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "nova-engine.mjs"), "export {};\n", "utf-8");
  });
  ...
  function createStableTagRunner(params: {
    ...
    const calls: string[] = [];
    let uiBuildCount = 0;
    const doctorKey = `${process.execPath} ${path.join(tempDir, "nova-engine.mjs")} doctor --non-interactive`;
  ...
  it("skips git update when worktree is dirty", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "nova-engine", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "nova-engine", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "nova-engine.mjs")} doctor --non-interactive`]: {
        stdout: "",
      },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "nova-engine", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const pkgRoot = path.join(nodeModules, "nova-engine");
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === params.expectedInstallCommand) {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "nova-engine", version: "2.0.0" }),
          "utf-8",
        );
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g nova-engine@latest",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g nova-engine@beta",
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "nova-engine");
    const staleDir = path.join(nodeModules, ".nova-engine-stale");
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g nova-engine@latest") {
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "nova-engine");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.writeFile(
        path.join(pkgRoot, "package.json"),
        JSON.stringify({ name: "nova-engine", version: "1.0.0" }),
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g nova-engine@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "nova-engine", version: "2.0.0" }),
            "utf-8",
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g nova-engine@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a nova-engine checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-nova-engine-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when nova-engine.mjs is missing", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "nova-engine", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
    ...
    await fs.rm(path.join(tempDir, "nova-engine.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("nova-engine doctor entry");
  });
