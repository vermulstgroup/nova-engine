...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.NOVA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*nova-engine-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "nova-engine";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "nova-engine");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "nova-engine-gw");
  ...
  records.push(`_nova-engine-gw._tcp IN PTR ${instanceLabel}._nova-engine-gw._tcp`);
  records.push(`${instanceLabel}._nova-engine-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._nova-engine-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; nova-engine-content-hash: ${contentHash}\n${contentBody}`;
}
