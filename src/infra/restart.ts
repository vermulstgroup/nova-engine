...
export function triggerNova EngineRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.NOVA_SYSTEMD_UNIT,
        process.env.NOVA_PROFILE,
      );
  ...
  const label =
    process.env.NOVA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.NOVA_PROFILE);
