...
function candidateBinDirs(opts: EnsureNovaEnginePathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `nova-engine` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "nova-engine");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.NOVA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "nova-engine"))) {
