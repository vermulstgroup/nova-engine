...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".nova-engine");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.NOVA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$NOVA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via NOVA_STATE_DIR.
