...
import { ensureNova EngineCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.NOVA_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.NOVA_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "NOVA_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "NOVA_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("nova-engine doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("nova-engine doctor")}" to repair, then retry.`,
    );
  }
