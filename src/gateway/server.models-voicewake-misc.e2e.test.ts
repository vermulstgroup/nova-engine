...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.NOVA_STATE_DIR = path.join(homeDir, ".nova-engine");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["nova-engine", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".nova-engine", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    const envSnapshot = captureEnv(["NOVA_CANVAS_HOST_PORT", "NOVA_GATEWAY_TOKEN"]);
    try {
      process.env.NOVA_GATEWAY_TOKEN = "secret";
      testTailnetIPv4.value = "100.64.0.1";
      testState.gatewayBind = "lan";
      const canvasPort = await getFreePort();
      testState.canvasHostPort = canvasPort;
      process.env.NOVA_CANVAS_HOST_PORT = String(canvasPort);
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.NOVA_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing NOVA_CONFIG_PATH");
    }
