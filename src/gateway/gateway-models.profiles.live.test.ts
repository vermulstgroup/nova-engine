...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.NOVA_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.NOVA_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.NOVA_LIVE_GATEWAY_PROVIDERS);
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-live-state-"));
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-live-"));
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Nova Engine live tool probe (local, safe): " +
    ...
    process.env.NOVA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.NOVA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.NOVA_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.nova-engine-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
