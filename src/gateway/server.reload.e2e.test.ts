...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.NOVA_SKIP_CHANNELS;
    prevSkipGmail = process.env.NOVA_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.NOVA_SKIP_PROVIDERS;
    process.env.NOVA_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.NOVA_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.NOVA_SKIP_GMAIL_WATCHER;
    } else {
      process.env.NOVA_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.NOVA_SKIP_PROVIDERS;
    } else {
      process.env.NOVA_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
