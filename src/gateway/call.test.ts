...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["NOVA_GATEWAY_TOKEN", "NOVA_GATEWAY_PASSWORD"]);
    loadConfig.mockReset();
    resolveGatewayPort.mockReset();
    pickPrimaryTailnetIPv4.mockReset();
    pickPrimaryLanIPv4.mockReset();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    ...
    process.env.NOVA_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["NOVA_GATEWAY_PASSWORD"]);
    loadConfig.mockReset();
    resolveGatewayPort.mockReset();
    pickPrimaryTailnetIPv4.mockReset();
    pickPrimaryLanIPv4.mockReset();
  ...
  it("prefers env password over local config password", async () => {
    process.env.NOVA_GATEWAY_PASSWORD = "from-env";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
...
describe("callGateway token resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["NOVA_GATEWAY_TOKEN"]);
    loadConfig.mockReset();
    resolveGatewayPort.mockReset();
    pickPrimaryTailnetIPv4.mockReset();
    pickPrimaryLanIPv4.mockReset();
