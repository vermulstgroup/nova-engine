...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.NOVA_LIVE_TEST);
const CLI_LIVE = isTruthyEnvValue(process.env.NOVA_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.NOVA_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.NOVA_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("NOVA_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const rawModel = process.env.NOVA_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `NOVA_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.NOVA_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `NOVA_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "NOVA_LIVE_CLI_BACKEND_ARGS",
        process.env.NOVA_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`NOVA_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "NOVA_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.NOVA_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.NOVA_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.NOVA_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "NOVA_LIVE_CLI_BACKEND_IMAGE_MODE requires NOVA_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-live-cli-"));
    const disableMcpConfig = process.env.NOVA_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    const tempConfigPath = path.join(tempDir, "nova-engine.json");
    ...
    try {
      ...
      if (previous.configPath === undefined) {
        ...
        process.env.NOVA_CONFIG_PATH = previous.configPath;
      }
      if (previous.token === undefined) {
        delete process.env.NOVA_GATEWAY_TOKEN;
      } else {
        process.env.NOVA_GATEWAY_TOKEN = previous.token;
      }
      if (previous.skipChannels === undefined) {
        delete process.env.NOVA_SKIP_CHANNELS;
      } else {
        process.env.NOVA_SKIP_CHANNELS = previous.skipChannels;
      }
      if (previous.skipGmail === undefined) {
        delete process.env.NOVA_SKIP_GMAIL_WATCHER;
      } else {
        process.env.NOVA_SKIP_GMAIL_WATCHER = previous.skipGmail;
      }
      if (previous.skipCron === undefined) {
        delete process.env.NOVA_SKIP_CRON;
      } else {
        process.env.NOVA_SKIP_CRON = previous.skipCron;
      }
      if (previous.skipCanvas === undefined) {
        delete process.env.NOVA_SKIP_CANVAS_HOST;
      } else {
        process.env.NOVA_SKIP_CANVAS_HOST = previous.skipCanvas;
      }
