...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  ...
  const dir = await mkdtemp(path.join(os.tmpdir(), "nova-engine-plugin-http-auth-test-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.NOVA_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.NOVA_DISABLE_CONFIG_CACHE;
    } else {
      process.env.NOVA_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
