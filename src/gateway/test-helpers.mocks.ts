...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `nova-engine-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.NOVA_CONFIG_PATH = path.join(root, "nova-engine.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "nova-engine.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "nova-engine-gateway-test"),
        ...fileDefaults,
