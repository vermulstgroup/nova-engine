...
describe("gateway server chat", () => {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-engine-gw-"));
