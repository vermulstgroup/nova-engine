...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.NOVA_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.NOVA_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.NOVA_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.NOVA_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.NOVA_GATEWAY_TOKEN;
