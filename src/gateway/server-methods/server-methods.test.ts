...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "nova-engine-logs-"));
    const older = path.join(tempDir, "nova-engine-2026-01-20.log");
    const newer = path.join(tempDir, "nova-engine-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "nova-engine-2026-01-22.log") });
