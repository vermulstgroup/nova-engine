...
export async function resolveGatewayRuntimeConfig(params: {
  ...
  const canvasHostEnabled =
    process.env.NOVA_SKIP_CANVAS_HOST !== "1" && params.cfg.canvasHost?.enabled !== false;
  ...
  if (tailscaleMode === "funnel" && authMode !== "password") {
    throw new Error(
      "tailscale funnel requires gateway auth mode=password (set gateway.auth.password or NOVA_GATEWAY_PASSWORD)",
    );
  }
  ...
  if (!isLoopbackHost(bindHost) && !hasSharedSecret && authMode !== "trusted-proxy") {
    throw new Error(
      `refusing to bind gateway to ${bindHost}:${params.port} without auth (set gateway.auth.token/password, or set NOVA_GATEWAY_TOKEN/NOVA_GATEWAY_PASSWORD)`,
    );
  }
